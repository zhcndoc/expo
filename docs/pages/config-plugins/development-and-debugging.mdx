...
> 使用 [modifier 预览](https://github.com/expo/vscode-expo#expo-preview-modifier) 实时调试你的插件结果。
...
（这取决于使用该模块的开发者所使用的包管理器中模块提升的实现细节），或者根本无法导入该模块（如果使用诸如 Yarn Berry 或 pnpm 等包管理器的“即插即用”功能）。
...
<!-- @info 包含 `xmlns:android="..."` 以便在清单中使用 `android:name` 之类的 `android:*` 属性。 -->
...
此时，已有 [React Native 项目](/bare/overview/) 的用户可以通过在本地 **strings.xml** 文件中创建一个字符串来配置这个值（前提是他们也已设置 `expo-modules-core` 支持）：
...
借助 [Continuous Native Generation (CNG)](/workflow/continuous-native-generation/)，我们可以通过 Expo config plugin 安全地暴露这个功能：
...
重新运行 `npx expo prebuild -p`（`eas build -p android`，或 `npx expo run:ios`）后，用户现在就可以看到这些更改，并且它们已安全地应用到自己的 CNG 项目中！
...
作为一个现实世界中的例子，`eas-cli` 会使用内省来确定 CNG 项目中最终的 iOS entitlements 将是什么，以便在构建之前将它们与 Apple Developer Portal 同步。内省也可以作为一个便捷的调试和开发工具。
...
# @info 使用 `withGradleProperties()` mod 安全修改。 #
...
project.ext.react = [/* @info 这段代码会预先添加到模板中，但也可以通过 `withAppBuildGradle()` 使用正则表达式添加 */ enableHermes: findProperty('expo.react.jsEngine') ?: 'jsc' /* @end */]
```
...
通常，你应该只通过 Expo [自动链接](/more/glossary-of-terms/#autolinking) 与 Gradle 文件交互，这会为项目文件提供一个程序化接口。

### iOS AppDelegate

某些模块可能需要向项目的 AppDelegate 添加委托方法。可以通过使用 [AppDelegate 订阅器](/modules/appdelegate-subscribers/) 安全地完成，也可以通过 `withAppDelegate` mod 以危险方式完成（_强烈不推荐_）。
使用 AppDelegate 订阅器可以让原生 Expo 模块以安全且可靠的方式响应重要事件。

下面是一些 AppDelegate 订阅器实际工作的示例。此外，你还可以在 GitHub 上的社区仓库中找到许多示例（[其中一个示例](https://github.com/bamlab/react-native-app-security/blob/c1a861cbd348f404ec18ffae90d1c9bdc66bc00d/ios/RNASAppLifecyleDelegate.swift)）。
...
通常，你应该只通过 Expo [自动链接](/more/glossary-of-terms/#autolinking) 与 Podfile 交互，这会为项目文件提供一个程序化接口。
...
> 此示例使用 `tsx` 来简单地支持本地 TypeScript，但这并非严格必要。请参阅[在 `app.config.js` 中使用 TypeScript](/guides/typescript/#appconfigjs)。
...
export function withAppDelegateHeaderBaseMod(config) {
  return BaseMods.withGeneratedBaseMods<'appDelegateHeader'>(config, {
    platform: 'ios',
    providers: {
      // 添加一条自定义规则，为 mods.ios.appDelegateHeader 上的 mods 提供 AppDelegate 头文件数据
...
export const withSimpleAppDelegateHeaderMod = config => {
  return withAppDelegateHeader(config, config => {
    console.log('修改头文件：', config.modResults);
    return config;
  });
};
```
