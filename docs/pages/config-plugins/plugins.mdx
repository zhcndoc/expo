...
你的配置插件可以接受从应用配置传入的参数。为此，你需要在配置插件函数中读取该参数，然后在应用配置中将包含该参数的对象与配置插件函数一起传入。
...
const withAndroidPlugin: ConfigPlugin<AndroidProps> = (
  config,
  /* @info 将 `options` 对象传递给插件。 */ options = {}/* @end */
) => {
...
const withIosPlugin: ConfigPlugin<IosProps> = (config, /* @info 将 `options` 对象传递给插件。 */ options = {}/* @end */) => {
   /* @info 使用提供的消息，或回退到默认值 */
  const message = options.message || 'Hello world, from Expo plugin!';
  /* @end */
  /* @hide ... 其余示例保持不变*/ /* @end */
...
要动态地向插件传递值，你可以在应用配置中向插件传入一个带有 `message` 属性的对象：
...
> **info** **提示**：对于每个包含配置插件的 Expo 库，你都可以在该库的 API 参考中找到更多相关信息。例如，[`expo-camera` 库包含一个配置插件部分](/versions/latest/sdk/camera/#configuration-in-app-config)。

运行 `npx expo prebuild` 时，系统会编译 [`mods`](/config-plugins/introduction/#mod)，并更改原生文件。
