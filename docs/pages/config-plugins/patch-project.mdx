...
`patch-project` 是一个 Expo 配置插件和命令行界面（CLI）工具，用于生成和应用补丁，以便在运行 `npx expo prebuild` 后保留原生更改。对于希望保留自定义内容、但不需要了解如何编写配置插件的原生应用开发者来说，这个工具非常有用，因为它可以有效地生成一种与 [Continuous Native Generation（CNG）](/workflow/continuous-native-generation/) 配合使用的自动化解决方案。
