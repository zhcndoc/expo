...
1. 客户端库 MUST 使用请求头中指定的约束条件，为最新的更新和指令发起一次[请求](#request)。
2. 如果收到[响应](#response)，客户端库 MUST 处理其内容：
   - 对于包含 _update_ 的响应，客户端库 SHALL 继续发起其他请求，以下载并存储清单中指定的任何新资源。清单和资源共同构成一个新的 _update_。客户端库将编辑其本地状态，以反映本地存储中已添加新的更新。它还将使用响应[请求头](#common-response-headers)中找到的新的 `expo-manifest-filters` 和 `expo-server-defined-headers` 更新本地状态。
   - 对于包含 _directive_ 的响应，客户端库将根据指令类型使用该指令，并相应地编辑其本地状态。
...
4. 任何前一个响应的[服务器定义的请求头](#response)所规定的请求头。
...
配置为执行[代码签名](#code-signing)验证的符合规范的客户端库 MUST 发送 `expo-expect-signature` 请求头，以表明它期望符合规范的服务器在 manifest 响应中包含 `expo-signature` 请求头。`expo-expect-signature` 是一个 [Expo SFV](/technical-specs/expo-sfv-0/) 字典，其中 MAY 包含以下任意键值对：
...
符合规范的客户端库还 MUST 包含 [`assetRequestHeaders`](#extensions-body) 中为此资源键指定的任何标头（键、值）对。

## 资源响应

位于特定 URL 的资源自创建以来不得被更改或移除，因为客户端库可能会在任何更新时随时获取资源。符合规范的客户端必须验证资源的 base64url 编码 SHA-256 哈希值与清单中该资源的 `hash` 字段匹配。

### 资源响应标头

资源必须使用客户端根据请求的 `accept-encoding` 标头所支持的压缩格式进行编码。服务器可以提供未压缩的资源。响应必须包含一个 `content-type` 标头，其中包含该资源的 MIME 类型。
例如：
...
资源应能够以 [Gzip](https://www.gnu.org/software/gzip/) 和 [Brotli](https://github.com/google/brotli) 压缩方式提供。
