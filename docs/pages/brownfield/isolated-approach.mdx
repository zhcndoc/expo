...
> **信息** 如果你想了解另一种将 React Native 直接集成到原生项目中的方案，请参阅 [集成式方案指南](/brownfield/integrated-approach/)。
...
有关所有可用选项的详细信息，请参阅 [`expo-brownfield` API 参考](/versions/latest/sdk/brownfield/)。
...
有关构建选项的更多详情，请参阅 [API 参考](/versions/latest/sdk/brownfield/)，例如仅构建 debug 或 release、指定自定义输出目录等。
...
有关构建选项的更多详情，请参阅 [`expo-brownfield` API 参考](/versions/latest/sdk/brownfield/)，例如仅构建 debug 或 release、指定自定义输出目录等。
...
该标志接受一个可选名称。如果省略，包默认命名为 **\{TargetName\}Artifacts**。生成的目录是一个完整的 Swift 包，其结构如下：
...
然后，在你的 target 的 **常规** 选项卡中，找到 **Frameworks, Libraries, and Embedded Content**，确保这两个 framework 都设置为 **Embed & Sign**。
