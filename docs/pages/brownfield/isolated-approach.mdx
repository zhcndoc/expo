...
<Prerequisites>
  ...
  <Requirement title="Yarn">
    安装 [Yarn](https://yarnpkg.com/) 作为 JavaScript 依赖的包管理器。
  </Requirement>
</Prerequisites>
...
有关所有可用选项的详细信息，请参阅 [`expo-brownfield` API reference](/versions/latest/sdk/brownfield/)。
...
有关构建选项的更多详情，请参阅 [API reference](/versions/latest/sdk/brownfield/)，例如仅构建 debug 或 release、指定自定义输出目录等。
...
有关构建选项的更多详情，请参阅 [`expo-brownfield` API reference](/versions/latest/sdk/brownfield/)，例如仅构建 debug 或 release、指定自定义输出目录等。

### 将产物作为 Swift Package 发布

传入 `--package [name]` 标志，可以将构建输出打包为一个独立的 Swift Package，而不是分别生成 **.xcframework** 目录。然后，你可以在 Xcode 中将其作为本地依赖添加到宿主应用，而不是手动将 framework 拖入项目。

<Terminal cmd={['$ npx expo-brownfield build:ios --release --package MyAppPackage']} />

该标志接受一个可选名称。如果省略，包默认命名为 **\{TargetName\}Artifacts**。生成的目录是一个完整的 Swift Package，其结构如下：
...
当 `build:ios` 生成 Swift Package 时（例如 **./artifacts/MyAppPackage-release**），在 Xcode 中将其作为本地依赖添加到宿主应用，并选择该包目录。Xcode 会通过聚合库产品自动链接捆绑的 **.xcframework** 文件。
...
<BoxLink
  ...
  description="探索用于通信、导航等功能的完整 JavaScript API。"
