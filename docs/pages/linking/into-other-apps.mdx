...
你可以使用 [`Linking.createURL`](/versions/latest/sdk/linking/#linkingcreateurlpath-namedparameters) 创建一个可用于打开应用或重定向回应用的 URL。此方法会解析为以下内容：

- **生产版和开发构建**：`myapp://`，其中 `myapp` 是应用配置中定义的[自定义 scheme](/linking/into-your-app/#add-a-custom-scheme-in-app-config)
