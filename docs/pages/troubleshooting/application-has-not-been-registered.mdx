...
在看到这条错误信息之前，请先查看你的日志，看看可能是什么原因导致了它。一个常见原因是某个本地模块依赖有多个版本，而该模块会自行注册为一个视图——例如，[这个 Stack Overflow 讨论串](https://stackoverflow.com/questions/67543844/invariant-violation-main-has-not-been-registered-while-running-react-native-a/67550379)中，提问者的依赖里包含了多个版本的 `react-native-safe-area-context`。
...
在使用 [Continuous Native Generation (CNG)](/workflow/continuous-native-generation/) 的项目中，默认行为是使用 `"main"` 作为 `AppKey`。这会自动为你处理好，只要你没有把 **package.json** 中的 `"main"` 字段改成默认值以外的内容，就一切正常。如果你想自定义应用入口点，请参阅 [registerRootComponent](/versions/latest/sdk/expo/#registerrootcomponentcomponent) API 参考。
...
这个错误在另外一些场景下也可能出现，不过这些情况更难预测，而且修复方式也更依赖于你的项目。例如，还有以下几种情况：
