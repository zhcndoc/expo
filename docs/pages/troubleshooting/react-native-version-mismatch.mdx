...
- 如果这是一个 [现有的 React Native 项目](/bare/overview/)，并且在升级 React Native 版本后立刻出现此错误，你应该仔细检查是否正确完成了每一个升级步骤。

- 最后：
  - 通过运行 `rm -rf node_modules && npm cache clean --force && npm install && watchman watch-del-all && rm -rf $TMPDIR/haste-map-* && rm -rf $TMPDIR/metro-cache && npx expo start --clear` 清除打包器缓存
    - 如果你使用的是 npm，可在[这里](clear-cache-macos-linux)找到相关命令。
    - 如果你使用的是 Windows，可在[这里](clear-cache-windows)找到相关命令。
  - 如果这是一个现有的 React Native 项目，请运行 `npx pod-install`，然后重新构建你的原生项目（运行 `yarn android` 重新构建 Android，运行 `yarn ios` 重新构建 iOS）
