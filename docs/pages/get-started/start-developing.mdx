...
如果你使用的是 Android 模拟器或 iOS 模拟器，可以分别按 <kbd>A</kbd> 或 <kbd>I</kbd> 来打开应用。

<Collapsible summary="遇到问题？">

请确保你的电脑和设备连接到同一个 Wi‑Fi 网络。

如果还是不行，可能是路由器配置导致的——这在公共网络中很常见。你可以在启动开发服务器时选择 **隧道** 连接类型，然后再次扫描二维码来解决这个问题。
...
> 使用 **隧道** 连接类型会使应用重新加载的速度明显慢于 **局域网** 或 **本地**，因此在可能的情况下最好避免使用隧道。如果需要通过网络中的另一台设备访问你的机器，你可能需要安装并使用模拟器或仿真器来加快开发速度。
...
- 如果你看到已启用 **快速刷新**，请将其切换。如果你看到 **禁用快速刷新**，请关闭开发者菜单。现在再试着做一次更改。
