...
要在 EAS 服务器上创建新的环境变量，你可以在项目仪表盘中导航到 **项目设置** > [**环境变量**](https://expo.dev/accounts/[account]/projects/[project]/environment-variables)，然后点击 **添加变量** 按钮。
...
使用[环境变量创建表单](https://expo.dev/accounts/[account]/projects/[project]/environment-variables)页面来设置名称、值、环境、可见性以及可选描述。
...
- `SENTRY_AUTH_TOKEN` 变量是一个敏感环境变量。它用于在构建和更新后对 Sentry 进行身份验证以上传源映射，并且必须可在 EAS 服务器之外访问。
...
使用 `eas env:set` 添加变量，并使用 `eas env:list` 验证已设置的变量。

<Terminal
  cmd={[
    '# 示例：为 preview 环境创建新的纯文本环境变量',
...
> **警告** 不要把密钥放进 `EXPO_PUBLIC_` 变量中。客户端包中的所有内容最终都可被终端用户读取。

### 构建时和应用配置

其他不带 `EXPO_PUBLIC_` 前缀的变量可以在 [应用配置](/workflow/configuration) 解析期间使用，以配置应用行为。例如，`APP_VARIANT` 变量用于确定所选 [应用变体](/build-reference/variants/) 的应用名称、包名和 bundle 标识符：

在动态应用配置中使用非前缀变量。如果你需要在本地解析配置，可见性至少保持为 **sensitive**（plain text 和 sensitive 可通过 EAS CLI 读取；secrets 会保留在服务器上）。
...
export default {
  /* @info 使用 `getAppName()` 作为 "name" 属性 */
  name: getAppName(),
  /* @end */
  /* @hide ... */ /* @end */
  ios: {
    /* @info 使用 `getUniqueIdentifier()` 作为 "bundleIdentifier" 属性 */
    bundleIdentifier: getUniqueIdentifier(),
    /* @end */
    /* @hide ... */ /* @end */
  },
  android: {
    /* @info 使用 `getUniqueIdentifier()` 作为 "package" 属性 */
...
像 `GOOGLE_SERVICES_JSON` 这样的环境变量是一个 secret 文件变量，无法在 EAS 服务器之外读取，用于向 EAS Build 任务提供被 git 忽略的 **google-services.json** 文件。要在应用配置中使用它，你可以使用 `process.env` 变量，并在变量未设置时提供一个回退值（用于本地开发时，通常它位于项目仓库中）：
...
你可以使用项目或账户中的 [EAS 仪表盘](https://expo.dev/accounts/[account]/projects/[project]/environment-variables) 来创建、更新和删除环境变量。
...
# GOOGLE_SERVICES_JSON=*****（机密变量不可读取）
SENTRY_AUTH_TOKEN=token
```

> **info** **提示：** 请将生成的 **.env** 文件保留在 **.gitignore** 中，以避免泄漏以及本地与云端任务之间的优先级冲突。

你也可以在 EAS 仪表盘中使用 **导出** 选项下载文件，并将其存储到你的项目中。
...
> **重要** 创建自定义环境适用于 [Enterprise 和 production](/billing/plans/#plans) 套餐。
...
- 在你的项目中，导航到 **项目设置** > [**环境变量**](https://expo.dev/accounts/[account]/projects/[project]/environment-variables)，然后点击 **添加变量** 按钮。
- 在 **环境** 下，点击 **加号 (+) 图标** 以输入自定义环境的名称。
