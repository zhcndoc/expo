...
jobs:
  build_app:
    ...
    params:
      ...
      profile: string # 可选 - 默认值为 production
      message: string # 可选
...
<Collapsible summary="使用刷新后的 ad hoc 配置文件进行 iOS 内部构建">
...
jobs:
  submit_to_store:
    ...
    params:
      ...
      groups: string[] # 可选
    hooks:
      before_submit: step[] # 可选 - 在提交开始前运行的步骤。
      after_submit: step[] # 可选 - 在提交完成后运行的步骤。
```
...
- `before_submit`：在提交开始前运行的步骤。
- `after_submit`：在提交完成后运行的步骤。
...
jobs:
  publish_update:
    ...
    params:
      ...
      upload_sentry_sourcemaps: boolean # 可选 - 默认为“尝试上传，但如果失败则不让作业失败”
```
...
jobs:
  run_maestro_tests:
    ...
    hooks:
      before_maestro_tests: step[] # 可选 - tests start 前运行的步骤。
      after_maestro_tests: step[] # 可选 - tests complete 后运行的步骤。
```
...
- `before_maestro_tests`：在 tests start 之前运行的步骤。
- `after_maestro_tests`：在 tests complete 之后运行的步骤。
...
jobs:
  run_maestro_tests:
    ...
    hooks:
      before_maestro_cloud: step[] # 可选 - 在 Maestro Cloud 上传之前运行的步骤。
      after_maestro_cloud: step[] # 可选 - 在 Maestro Cloud 上传之后运行的步骤。
```
...
以下 hook 支持用于 Maestro Cloud 作业：

- `before_maestro_cloud`：在 Maestro Cloud 上传之前运行的步骤。
- `after_maestro_cloud`：在 Maestro Cloud 上传之后运行的步骤。
...
暂停一次工作流运行，直到某台 iOS 设备为特定 [Apple Team](https://expo.fyi/apple-team) 完成注册，并且团队成员在 [expo.dev](https://expo.dev) 上批准该注册。使用此作业可在 EAS Workflows 中自动化注册用于 [内部分发](/build/internal-distribution/) 的设备。
...
| 参数 | 类型 | 描述 |
| --------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| apple_team_identifier | string | 可选。Apple Team ID（例如 `ABCDE12345`）。如果你省略此参数且你的 Expo 账号恰好只有一个 Apple Team，则会使用该团队。如果你的账户没有 Apple Teams，或者有两个及以上 Apple Teams，则必须设置此参数。提供后，EAS 会根据该标识符解析或创建团队。 |
...
<Collapsible summary="结合指纹和重打包的持续部署">
