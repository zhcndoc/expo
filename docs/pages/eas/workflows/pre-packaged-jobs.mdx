...
Prepackaged actions are ready-to-use workflow tasks that help you automate common tasks such as building, submitting, and testing applications. They provide a standardized way to handle these operations without having to write custom task configurations from scratch. This guide will introduce the available prepackaged actions and how to use them in your workflows.
...
jobs:
  run_maestro_tests:
    ...
    params:
      ...
      flows: string # 必需 - 要运行的 Maestro flow 文件或包含 flows 的目录路径。对应 `maestro cloud` 的 `--flows` 参数。
      maestro_api_key: string # 可选 - 默认值为 `$MAESTRO_CLOUD_API_KEY`
      ...
      device_locale: string # 可选 - 用于测试的设备区域设置。将作为 `--device-locale` 传递给 Maestro。
      device_model: string # 可选 - 用于测试的设备型号。运行 `maestro list-cloud-devices` 可查看支持的值列表。
      device_os: string # 可选 - 用于测试的设备操作系统。将作为 `--device-os` 传递给 Maestro。运行 `maestro list-cloud-devices` 可查看支持的值列表。
...
| js_bundle_only                          | boolean | 可选。是否仅重新打包 JavaScript bundle。默认值为 false，这意味着将更新整个应用元数据。                                                                                                                            |
| message                                 | string  | 可选。附加到构建上的自定义消息。对应于运行 `eas build` 时的 `--message` 标志。                                                                                                                                   |
| repack_version                          | string  | 可选。要使用的 `@expo/repack-app` 版本。默认使用最新版本。                                                                                                                                                        |
| ios_signing_use_source_app_entitlements | boolean | 可选。是否使用 fastlane resign 的 `use_app_entitlements` 行为：提取应用包代码签名 entitlements，并将其与新 provisioning profile 中的 entitlements 合并。默认值为 false。 |
