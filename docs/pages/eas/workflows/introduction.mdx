...
- **为 React Native/Expo 预打包**：提供可直接使用的任务类型（`build`、`submit`、`update`、`maestro`、`deploy` 等），抽象掉实现复杂性
- **无需管理基础设施**：在带有 macOS 和 Linux 工作节点的 EAS 上运行，因此你无需维护 CI 服务器或配置 Android Studio/Xcode
- **统一的制品管理**：所有构建制品、更新和日志都会显示在 EAS 仪表盘上
- **GitHub 集成**：可在 push、pull request、label 或 ref 删除事件时，结合分支和路径过滤自动触发工作流
- **更快的迭代**：组合 Fingerprint、Get Build 和 Update 任务，避免重复的原生构建，并在可能时发布 OTA（空中）更新
- **内置 E2E 测试**：直接在工作流中于 Android 模拟器和 iOS 模拟器上运行 Maestro 测试
- **Slack 通知**：当工作流成功运行或失败时，向 Slack 频道发送通知
- **Repack**：重用现有构建的元数据和 JavaScript bundle，更快创建兼容构建
...
当 GitHub 分支或标签被删除时运行。支持使用 glob 模式进行分支和标签过滤。对于清理与 Git 分支同名的 EAS Update 分支很有用。

参见 [`on.ref_delete` 语法参考](/eas/workflows/syntax/#onref_delete)。
