...
- **灵活的触发器**：可在 [GitHub 事件](/eas/workflows/syntax/#on)（push、pull request、label、分支或标签删除）、cron 计划、[App Store Connect 事件](/eas/workflows/syntax/#onapp_store_connect)上运行工作流，也可以使用 `eas workflow:run` 手动运行，或通过 [REST API](/eas/workflows/rest-api/) 触发。
