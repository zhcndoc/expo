...
jobs:
  my_job:
    ...
    params:
      ...
      flows: string # 必填. Maestro flow 文件或包含待运行 flow 的目录路径。对应 `maestro cloud` 的 `--flows` 参数。
      maestro_api_key: string # 可选. 用于 Maestro 项目的 API key。默认会使用 `MAESTRO_CLOUD_API_KEY` 环境变量。对应 `maestro cloud` 的 `--api-key` 参数。
      include_tags: string | string[] # 可选. 要包含在测试中的标签。会作为 `--include-tags` 传递给 Maestro。
      exclude_tags: string | string[] # 可选. 要从测试中排除的标签。会作为 `--exclude-tags` 传递给 Maestro。
      maestro_version: string # 可选. 要用于测试的 Maestro 版本。如果未提供，将使用最新版本。
      maestro_config: string # 可选. 用于测试的 Maestro `config.yaml` 文件路径。会作为 `--config` 传递给 Maestro。
      device_locale: string # 可选. 用于测试的设备区域设置。会作为 `--device-locale` 传递给 Maestro。
      device_model: string # 可选. 用于测试的设备型号。会作为 `--device-model` 传递给 Maestro。运行 `maestro list-cloud-devices` 可查看支持的值列表。
      device_os: string # 可选. 用于测试的设备操作系统。会作为 `--device-os` 传递给 Maestro。运行 `maestro list-cloud-devices` 可查看支持的值列表。
      name: string # 可选. Maestro Cloud 上传名称。对应 `maestro cloud` 的 `--name` 参数。
      branch: string # 可选. 覆盖 Maestro Cloud 上传来源分支。默认情况下，如果 workflow 运行是从 GitHub 触发的，则会使用该 workflow 运行所在的分支。对应 `maestro cloud` 的 `--branch` 参数。
      async: boolean # 可选. 异步运行 Maestro Cloud 测试。如果为 true，job 的状态只表示上传是否成功，而*不表示*测试是否成功。对应 `maestro cloud` 的 `--async` 参数。
```
