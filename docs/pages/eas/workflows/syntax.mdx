...
工作流文件使用 YAML 语法，文件扩展名必须是 `.yml` 或 `.yaml`。如果你是 YAML 新手并想进一步了解，请参见 [一分钟学 YAML](https://learnxinyminutes.com/docs/yaml/)。
...
| `description` | `string`   | <NoIcon />                       | 输入的说明。                                                  |
| `required`    | `boolean`  | <NoIcon />                       | 是否需要该输入。默认为 `false`。                         |
| `default`     | varies     | <NoIcon />                       | 输入的默认值。必须与输入类型匹配。                     |
| `options`     | `string[]` | <YesIcon /> (适用于 `type: choice`) | 可用于 choice 输入的选项。                                        |
...
为该 job 设置环境变量。此属性适用于所有在 VM 上运行的 job（除预打包的 `apple-device-registration-request`、`require-approval`、`doc`、`get-build` 和 `slack` job 外的所有 job）。
...
一些预打包的 job 支持 hooks，可在主要 job 动作之前或之后运行额外步骤（例如 `maestro-cloud`、`maestro` 和 `submit`）。hook 名称是特定于 job 的。有关可用的 hook 键，请参见该 job 的文档。
...
应为此工作流配置中定义的作业使用的特定工具版本。有关可用值，请遵循各工具的文档。
...
并发控制配置。目前只允许为同分支工作流设置 `cancel_in_progress`。
...
| 属性                 | 类型     | 描述                                                                                                                                             |
| -------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cancel_in_progress` | `true`   | 如果为 true，GitHub 新启动的工作流运行将取消同一分支上当前正在进行的运行。                                                |
| `group`              | `string` | 我们暂不支持自定义并发组。请设置此占位值，以便在未来支持自定义组时，你的工作流仍能保持兼容。 |

## 控制流

你可以通过 `needs` 和 `after` 关键字控制某个作业何时运行。另外，你还可以使用 `if` 关键字基于条件控制作业是否应运行。

### `jobs.<job_id>.needs`

在此作业运行之前必须成功完成的作业 ID 列表。
...
在此作业运行之前必须完成（成功或失败均可）的作业 ID 列表。
...
jobs:
  ...
  notify:
    # @info #
    after: [build] # 此作业将在 build 完成后运行（无论 build 成功还是失败）
    # @end #
```

### `jobs.<job_id>.if`

`if` 条件用于决定作业是否应运行。当 `if` 条件满足时，作业将运行；当条件不满足时，作业将被跳过。被跳过的作业不会算作成功完成，并且任何后续作业如果在其 `needs` 列表中包含该作业，也不会运行。
...
```json
{
  ...
  /* @info 每个作业会生成不同的一组输出。详情请参见特定作业的文档。 */
  "outputs": {}
  /* @end */
}
```
...
大多数预打包作业会暴露特定输出。你可以使用 [`set-output` 函数](#jobsjob_idoutputs) 在自定义作业中设置输出。
...
当前作业中所有步骤的记录。每个步骤提供其通过 [`set-output`](#set-output) 函数设置的输出。

> **注意：** `steps` 上下文仅在作业的 steps 内可用，而不在工作流级别可用。要将某个步骤的输出暴露给其他作业，请使用 [`set-output` 函数](#set-output) 和作业的 [`outputs` 配置](#jobsjob_idoutputs)。
...
手动通过 [`workflow_dispatch`](#onworkflow_dispatchinputs) 触发工作流时提供的输入记录。在通过 `eas workflow:run` 命令并带有输入参数触发工作流时可用。
...
如果工作流运行是通过 `eas workflow:run` 启动的，则其 `event_name` 为 `workflow_dispatch`，其余所有属性都将为空。
...
jobs:
  notify_slack:
    ...
    params:
      message: |
        工作流运行已完成：${{ workflow.name }}
        查看详情：${{ workflow.url }}
```

#### `app_store_connect`

与工作流运行相关联的 App Store Connect 实体信息。此上下文仅适用于由 App Store Connect 事件触发的工作流。参见 [`on.app_store_connect`](#onapp_store_connect)。
...
`app_store_connect.build_upload` 对象仅在工作流由 `build_upload` 事件域触发时存在。
...
> **注意：** `env` 上下文仅在作业的上下文中可用，而不在工作流级别可用。
...
> **注意：** `hashFiles` 函数仅在作业的 steps 内可用，而不在工作流级别可用。
...
jobs:
  my_job:
    ...
    params:
      ...
      retry_failed_only: boolean # 可选，默认值为 true。为 true 时，在适用情况下，重试将仅重新运行上一次尝试中失败的 flows。
      record_screen: boolean # 可选，默认值为 false。为 true 时，会上传测试的屏幕录制。
...
jobs:
  my_job:
    ...
    params:
      ...
      flows: string # 必填。Maestro flow 文件的路径，或包含要运行的 flows 的目录。对应 `maestro cloud` 的 `--flows` 参数。
      maestro_api_key: string # 可选。用于 Maestro 项目的 API key。默认使用 `MAESTRO_CLOUD_API_KEY` 环境变量。对应 `maestro cloud` 的 `--api-key` 参数。
      include_tags: string | string[] # 可选。要包含在测试中的标签。将作为 `--include-tags` 传递给 Maestro。
      exclude_tags: string | string[] # 可选。要从测试中排除的标签。将作为 `--exclude-tags` 传递给 Maestro。
      maestro_version: string # 可选。用于测试的 Maestro 版本。如果未提供，将使用最新版本。
      ...
      device_model: string # 可选。用于测试的设备型号。将作为 `--device-model` 传递给 Maestro。运行 `maestro list-cloud-devices` 可查看受支持的值列表。
      device_os: string # 可选。用于测试的设备操作系统。将作为 `--device-os` 传递给 Maestro。运行 `maestro list-cloud-devices` 可查看受支持的值列表。
      name: string # 可选。Maestro Cloud 上传的名称。对应 `maestro cloud` 的 `--name` 参数。
      branch: string # 可选。覆盖 Maestro Cloud 上传来源的分支。默认情况下，如果工作流运行是从 GitHub 触发的，将使用该工作流运行所在的分支。对应 `maestro cloud` 的 `--branch` 参数。
      async: boolean # 可选。异步运行 Maestro Cloud 测试。如果为 true，任务状态只表示上传是否成功，而不表示测试是否成功。对应 `maestro cloud` 的 `--async` 参数。
    hooks:
...
在一台 iOS 设备向 Apple Team 注册并且团队成员批准该注册之前，暂停工作流。有关详细信息和示例，请参阅 [Apple 设备注册请求任务文档](/eas/workflows/pre-packaged-jobs#apple-device-registration-request)。
...
| 工作器       | 效率核心 | 统一内存（GiB RAM） | SSD (GiB) | 备注                                |
| ------------ | ---------------- | ------------------------ | --------- | ------------------------------------ |
...
| `extensions` | string[]  | <NoIcon />  | `["apk", "aab", "ipa", "app"]` | 要在下载的制品或归档中查找的文件扩展名列表。                           |
...
| 属性              | 类型      | 是否必填 | 描述                                                                                                                                              |
| ----------------- | --------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `key`             | `string`  | <YesIcon /> | 要恢复的缓存 key。你可以使用 `${{ hashFiles('package-lock.json') }}` 之类的表达式，根据文件哈希创建动态 key。                                    |
| `restore_keys`    | `string`  | <NoIcon />  | 如果找不到完全匹配的 key，则使用的备用 key 或前缀。如果提供了该值，缓存系统会查找任何以前缀开头的缓存条目。                                      |
| `path`            | `string`  | <YesIcon /> | 应恢复缓存的路径。该路径应与保存缓存时使用的路径一致。                                                                                           |
...
| 属性     | 类型      | 是否必填 | 描述                                                                                                                                                                                                       |
| -------- | --------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `key`    | `string`  | <YesIcon /> | 要将缓存保存到的 key。你可以使用 `${{ hashFiles('package-lock.json') }}` 之类的表达式，根据文件哈希创建动态 key。该值应与恢复缓存时使用的 key 一致。                                                        |
| `path`   | `string`  | <YesIcon /> | 要缓存的目录或文件路径。该路径应与恢复缓存时使用的路径一致。                                                                                                                                               |
...
| 属性           | 类型   | 是否必填 | 描述                                                                      |
| ------------- | ------ | -------- | ------------------------------------------------------------------------- |
| `name`        | string | <NoIcon /> | 要下载的制品名称。如果未提供 `artifact_id`，则为必填。                      |
| `artifact_id` | string | <NoIcon /> | 要下载的制品 ID。如果未提供 `name`，则为必填。                            |
