...
[模型上下文协议（MCP）](https://modelcontextprotocol.io/) 是一种标准协议，允许 AI 模型与外部数据源集成，从而提供更丰富的上下文，以获得更精准的响应。它使诸如 agent 之类的 AI 辅助工具能够更深入地了解你的开发环境，从而更好地协助你的代码库。
...
<Prerequisites>
  ...
  <Requirement title="最新 SDK 的 Expo 项目">
    使用 `npx create-expo-app@latest --template default@sdk-56` 创建一个项目，或者确保你现有的项目已安装最新的 `expo` 包。
  </Requirement>
...
<Terminal
  cmd={{
    npm: [
      ...
      '',
      '# 确保你已使用与用于',
      '# 通过 MCP 服务器进行身份验证的相同账户登录 Expo CLI',
...
1. 本地 Expo 开发服务器从你的模拟器中捕获截图。
2. 截图发送到 Expo MCP Server。
3. Expo MCP Server 将截图返回给你的本地 MCP 客户端或 AI 辅助工具。
