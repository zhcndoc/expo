...
应用配置中的大多数配置都可以在运行时通过 JavaScript 代码访问，使用 [`Constants.expoConfig`](/versions/latest/sdk/constants/#nativeconstants)。你**不应**在应用配置中包含任何敏感信息（以下列出的少数字段会被过滤掉）。
...
> **warning** 你还应避免直接在 JavaScript 代码中导入 **app.json** 或 **app.config.js**，因为这会导入整个文件，而不是经过处理的版本。相反，请使用 [`Constants.expoConfig`](/versions/latest/sdk/constants/#nativeconstants) 访问配置。
...
- 注释、变量和单引号。
- **app.config.js** 和 **app.config.ts** 中都支持 ESM `import` 语法，包括导入其他 JavaScript 文件。
- TypeScript 支持（**app.config.ts**），包括空值合并和可选链。导入其他 TypeScript 文件或自定义语言特性需要使用 [`tsx`](/guides/typescript/#appconfigjs)。
- 每当 Metro 打包器重新加载时都会更新。
- 向你的应用提供环境信息。
- 不支持 Promise。

> **信息** **注意**：也会发现 **app.config.mts**、**app.config.cts**、**app.config.mjs** 和 **app.config.cjs** 文件。默认情况下，配置会被转译为 CommonJS，并且 `.js` 和 `.ts` 文件可以混合使用 ESM 和 CommonJS 语法。当这种混合使用导致导入或 require 问题时，请使用显式扩展名之一，将配置锁定为单一模块格式。
