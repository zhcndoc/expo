...
对于大多数团队来说，使用 EAS 比起投入工程时间和资源去获取、搭建并维护其他基础设施上的这些服务更合理。此外，EAS 在各项服务之间提供了深度集成，例如用于监控应用版本采用情况的部署页面、将更新分配给特定构建，以及逐步推送这些更新——这又与通过 [EAS Insights](/eas-insights/introduction/) 进行监控相呼应。
...
开发构建是一个包含 `expo-dev-client` 库的应用调试版本。它可以帮助你尽可能快速地迭代，并且相比 Expo Go 提供更灵活、更可靠、更完整的开发环境。你可以安装任意原生库，并使用[应用配置](/workflow/configuration/)或创建[配置插件](/config-plugins/introduction/)来配置或应用对[原生项目](#android-and-ios-native-projects)的更改。你可以[在本地](/guides/local-app-development/#local-builds-with-expo-dev-client)创建开发构建，也可以使用 [EAS Build](/develop/development-builds/introduction/?buildenv=build-with-eas#how-would-you-like-to-build-your-development-build) 在云端创建构建。
...
使用 CNG 可以让升级到新版本 React Native 变得容易得多。它可以简化项目维护，并促进设置复杂功能，例如 [App Clips](https://github.com/bndkt/react-native-app-clip)、[共享扩展](https://github.com/timedtext/expo-config-plugin-ios-share-extension) 和 [错误报告](https://github.com/getsentry/sentry-react-native)。这一切都要归功于 [配置插件](/config-plugins/introduction/)。了解更多关于 [CNG](/workflow/continuous-native-generation/) 的信息。
...
你可以在项目中运行 `npx expo prebuild`，然后直接修改 **android** 和 **ios** 目录，而不是使用 Expo 应用配置。如果你决定这么做，那么请记住，你将不再能够使用 prebuild 重新生成项目——在直接做出原生更改后再次运行 prebuild 会覆盖所有这些修改。

请注意，你可以使用 [配置插件](/config-plugins/introduction/) 来修改原生项目配置，而无需直接修改原生项目，并且如果你之后决定回到 CNG，也可以这样做。
...
在大多数情况下，你会为项目创建并使用开发构建。你需要安装 [`expo-dev-client`](/develop/development-builds/introduction/) 库。可以使用 EAS Build 创建开发构建，也可以在本地机器上创建：

<BoxLink
  title="使用 EAS 创建开发构建"
  description="了解如何使用 EAS 为项目创建开发构建。"
...
- #### 更新应用配置
  ...
  这包括使用应用配置文件（**app.json** 或 **app.config.js**）修改应用的配置。它包括更新应用名称、图标、启动画面和其他属性。这些更改并不会全部直接影响原生项目。不过，如果你所做的更改会影响原生项目，你可以使用 [应用配置](/workflow/configuration/) 来修改原生项目配置，或者创建或使用 [配置插件](/config-plugins/introduction/)。有关 app config 文件中可用属性的完整列表，请参见 [app config 参考](/versions/latest/config/app/)。

- #### 编写原生代码或修改原生项目配置

  这包括直接编写原生代码或修改原生代码配置。你需要访问原生代码项目目录来进行这些更改，也可以使用[本地 Expo 模块](/modules/get-started/#add-a-new-module-to-an-existing-application)编写原生代码。

- #### 安装需要修改原生代码的库

  这包括某个库需要对原生代码项目配置进行更改。这个库要么提供了配置插件，要么提供了更新应用配置的步骤。与前一个活动一样，这也要求你创建开发构建。
...
要将应用发布到应用商店，你可以使用 [EAS Submit](/deploy/submit-to-app-stores/)。有关使用 EAS Submit 的更多信息，请参阅[提交到 Google Play 商店](/submit/android/)和[提交到 Apple App Store](/submit/ios/)。
