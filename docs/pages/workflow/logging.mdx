---
title: 查看日志
description: 了解如何在使用 Expo CLI 时查看日志、在 Android Studio 和 Xcode 中查看原生日志，以及查看系统日志。
---

import { Terminal } from '~/ui/components/Snippet';

在 React Native 应用中记录信息的方式与在网页浏览器中类似。你可以使用 `console.log`、`console.warn` 和 `console.error`。不过，有时你可能希望深入查看，以获取有关应用中发生情况的更有用信息。为此，你可以使用 **原生日志** 和 **系统日志**。

## 控制台日志

当你运行 `npx expo start` 并连接设备时，控制台日志会显示在终端进程中。这些日志会从运行时通过 WebSocket 发送到 Expo CLI，这意味着其结果与直接将开发工具连接到引擎相比，保真度更低。

你可以通过使用 [Hermes](/guides/using-hermes) 创建开发构建，并[连接检查器](/guides/using-hermes#javascript-debugger)，来查看**高保真**日志并使用 `console.table` 等高级日志函数。

### 在生产模式下查看控制台日志

当你使用 [`npx expo start --no-dev`](/workflow/development-mode/#production-mode) 启动应用时，控制台日志不会显示在 Expo CLI 终端中。将日志转发到 Expo CLI 的运行时代码仅在开发模式下运行。你无法在生产模式下重新启用它。不过，`console.log` 仍会将日志写入设备上的原生日志。要查看这些日志，请在 Android 上使用 `adb logcat`，或在 iOS 上使用 Console 应用。有关分步说明，请参阅[查看原生日志](/debugging/runtime-issues/#viewing-native-logs)。

## 原生日志

你可以通过在本地编译原生应用，在 Android Studio 和 Xcode 中查看原生运行时日志。有关更多信息，请参阅[原生调试](/debugging/runtime-issues/#native-debugging)。

## 系统日志

虽然通常没有必要，但如果你想查看设备上发生的所有日志，例如，甚至包括其他应用和操作系统的日志，你可以使用以下命令：

<Terminal
  cmd={{
    npm: [
      '# 显示使用 adb logcat 的 Android 设备系统日志',
      '$ npx react-native log-android',
      '# 显示 iOS 设备系统日志',
      '$ npx react-native log-ios',
    ],
    yarn: [
      '# 显示使用 adb logcat 的 Android 设备系统日志',
      '$ yarn dlx react-native log-android',
      '# 显示 iOS 设备系统日志',
      '$ yarn dlx react-native log-ios',
    ],
    pnpm: [
      '# 显示使用 adb logcat 的 Android 设备系统日志',
      '$ pnpm dlx react-native log-android',
      '# 显示 iOS 设备系统日志',
      '$ pnpm dlx react-native log-ios',
    ],
    bun: [
      '# 显示使用 adb logcat 的 Android 设备系统日志',
      '$ bunx react-native log-android',
      '# 显示 iOS 设备系统日志',
      '$ bunx react-native log-ios',
    ],
  }}
/>
