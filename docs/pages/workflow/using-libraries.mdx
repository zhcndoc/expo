...
如果你安装了特定版本的第三方库，并希望在 `npx expo install`、`npx expo-doctor` 或 `npx expo start` 执行版本检查时将其排除，请在 **package.json** 文件中使用 [`expo.install.exclude`](/versions/latest/config/package-json/#installexclude) 属性。
