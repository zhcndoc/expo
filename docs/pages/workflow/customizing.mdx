...
如果你使用的是 [Expo Go](https://expo.dev/go)，[你只能访问 Expo SDK 中包含的原生库](/versions/latest/sdk/third-party-overview/)，或不包含任何自定义原生代码的库（[了解有关第三方库的更多信息](/workflow/using-libraries/#third-party-libraries)）。[创建开发构建](/develop/development-builds/introduction/)后，你就可以像在其他原生应用中一样修改原生代码或配置。
...
- 如果该库包含[配置插件](/config-plugins/introduction/)，你可以在应用配置中指定首选配置。
- 创建新的开发构建（可以[在本地](/guides/local-app-development/)创建，也可以使用 [EAS](/develop/development-builds/introduction/?buildenv=build-with-eas#how-would-you-like-to-build-your-development-build)创建）。
...
如果你打算在单个应用中使用你的原生模块（你以后随时可以改变主意），我们建议你[使用一个“本地”Expo 模块](/modules/get-started/#create-the-local-expo-module)来编写自定义原生代码。本地 Expo 模块的工作方式与库开发者以及 Expo SDK 中使用的 [Expo Modules](/modules/overview) 类似，例如 `expo-camera`，但它们不会发布到 npm。相反，你可以直接在项目中创建它们。
