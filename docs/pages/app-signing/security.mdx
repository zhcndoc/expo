...
构建用于发布到 Play 商店的应用时，需要 keystore 和 keystore 密码。它们会使用 KMS 加密，并在静态存储时额外加密。在应用首次提交到 Google Play 商店后，之后更新该应用时必须使用相同的 keystore 对应用重新签名。它证明 APK 来自拥有该 keystore 的开发者。仅有 keystore 本身并不能让你向 Google Play 提交应用——你的 Google 账户还需要拥有 Google Play Console 的访问权限。
