...
要签署发布到 Play Store 的构建，需要 keystore 和 keystore 密码。这些内容会使用 KMS 加密，并且在静态存储时也会额外加密。签名可以证明更新来自拥有该 keystore 的开发者。Google Play 只接受使用为你的应用注册的密钥签名的更新。对于使用 [App signing by Google Play](/app-signing/app-credentials/#app-signing-by-google-play) 的应用（新应用的默认设置），Google 会持有应用签名密钥，而你的 keystore 是上传密钥。仅凭 keystore 无法让你提交到 Google Play，你的 Google 账户还需要拥有 Google Play Console 的访问权限。

### 如果泄露的后果

如果你的 Google Play Developer 账户安全，恶意行为者无法使用你的 keystore 和 keystore 密码更新你的应用。如果你的应用使用 App signing by Google Play，你还可以创建新的 keystore 并[请求 Google 重置你的上传密钥](/app-signing/app-credentials/#lost-your-keystore-learn-how-to-reset)。重置后，Google Play 将不再接受使用被泄露密钥签署的构建。

### 如果丢失的后果

如果你的应用使用 App signing by Google Play，丢失的 keystore 可以恢复。创建新的 keystore 并[请求 Google 重置你的上传密钥](/app-signing/app-credentials/#lost-your-keystore-learn-how-to-reset)。在 Google 完成重置之前，你无法更新应用。如果你的应用不使用 App signing by Google Play，丢失的 keystore 本身就是应用签名密钥。在这种情况下，你将无法在 Google Play 上更新应用。我们建议将 keystore 和 keystore 密码下载并备份到安全的位置。
...
仅凭这些凭证，恶意行为者几乎无能为力——如果没有你的 Apple Developer 账户凭证，他们将无法提交任何应用。你可以在 Apple Developer 网站上撤销分发证书和配置文件。
...
在创建独立应用构建或上传到 App Store 时，系统会提示你提供 Apple Developer 账户凭证。我们不会将这些信息存储在我们的服务器上——EAS CLI 只会在本地使用它们。只有你的电脑会为 Expo 服务器配置并发送分发证书和 auth key；你的开发者凭证不会发送到 Expo 服务器。Apple 还强制要求更高一层的安全保护，因为他们要求所有 Apple Developer 账户都启用双重认证。
