...
可以！所有 Expo 工具和服务都能在任何 React Native 应用中出色运行。例如，你可以使用 [Expo SDK](/versions/latest)、[`expo-dev-client`](/develop/development-builds/introduction/#how-would-you-like-to-build-your-development-build) 的任何部分，以及 EAS Build、Submit 和 Update——它们都能很好地运行！详细了解如何[在项目中安装 `expo`](/bare/installing-expo-modules)、[采用预构建](/guides/adopting-prebuild)以及[设置 EAS Build](/build/introduction)。
...
准备就绪后，你可以创建生产构建（**.aab** 和 **.ipa**）并将其提交到应用商店。你可以使用 [EAS Build](/build/introduction) 通过一条命令构建应用，并使用 [EAS Submit](/deploy/submit-to-app-stores/) 将其提交到应用商店。
...
传统上，开发 iOS 应用需要 macOS，不过，您可以使用 [EAS Build](/build/introduction) 在云端构建应用。您还可以使用 [EAS Submit](/deploy/submit-to-app-stores/) 将应用提交到应用商店。您可以使用 [Expo Go](https://expo.dev/go) 或[开发版本](/develop/development-builds/introduction/)在实体 iOS 设备上进行测试。
...
许多流行的 Web 包，例如 three.js，都可以与 Expo 和 React Native 一起使用。更多信息请参见 [Expo 示例](https://github.com/expo/examples)。
...
React Native 使用 JavaScript 解释器（JSC、V8 或 Hermes）来运行你的应用代码。请直接参考 [Google Play 政策中心](https://play.google/developer-content-policy/) 和 [Apple Developer Program 许可协议](https://developer.apple.com/support/terms/apple-developer-program-license-agreement) 获取最新政策信息。
...
Expo CLI 提供了与 React Native Community CLI（也称为“React Native CLI”）相同的核心功能，并额外提供了[自动配置 TypeScript](/guides/typescript)、[Web 支持](/workflow/web)、[自动安装兼容的库](/more/expo-cli/#install)、[改进的原生构建命令](/more/expo-cli#compiling)、[隧道连接](/more/expo-cli#tunneling)、[Prebuild](/more/glossary-of-terms/#prebuild)以及[更多 Expo CLI 功能](/more/expo-cli)。
