...
首先，你需要创建一个 Expo 模块，或使用 React Native 模块 API 在库中集成 Expo modules API。请参阅[什么是 Expo Modules API](/modules/overview/#what-is-the-expo-modules-api)。
...
现在你可以通过向订阅者类添加委托函数来订阅事件。有关你可以订阅的完整函数列表，请参阅在 [`ExpoAppDelegate.swift`](https://github.com/expo/expo/blob/main/packages/expo/ios/AppDelegates/ExpoAppDelegate.swift) 中被重写的函数。对于在提供时可能产生副作用的应用委托函数，目前尚不支持（例如 [`application(_:viewControllerWithRestorationIdentifierPath:coder:)`](https://developer.apple.com/documentation/uikit/uiapplicationdelegate/1623062-application)）。
