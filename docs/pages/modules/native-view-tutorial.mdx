...
> **info** 由于这是一个示例库，不会发布，因此在所有提示中按 <kbd>Return</kbd> 键即可接受默认值。
...
使用 [Prop 定义组件](/modules/module-api/#prop) 来定义该 prop。在 prop setter 块中，你可以同时访问视图和 prop。指定 URL 的类型为 `URL` — Expo 模块 API 会将字符串转换为原生 `URL` 类型。
...
[视图回调](/modules/module-api/#view-callbacks) 允许开发者监听组件上的事件。它们通常通过组件上的 props 注册，例如：`<Image onLoad={...} />`。使用 [事件定义组件](/modules/module-api/#events) 为你的 WebView 定义一个事件。将它命名为 `onLoad`。
...
<ContentSpotlight
  alt="围绕我们的 WebView 构建的简单网页浏览器 UI"
