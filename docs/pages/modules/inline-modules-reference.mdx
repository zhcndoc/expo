...
> **warning** 内联模块是 [实验性功能](/more/release-statuses/#experimental)，并且可在 Expo SDK 56 及更高版本中使用。该 API 可能会发生破坏性变更。
...
配置将内联模块文件添加到哪些 Xcode 目标中。如果省略此配置，内联模块只会添加到应用的主目标中。
...
每个条目都是 Xcode 项目中（以及 **ios/Podfile** 的 `target` 块中）显示的目标名称。只使用目标名称 — 即使目标嵌套在 `abstract_target` 中，也不要包含抽象目标的名称。

</PaddedAPIBox>

> **警告** 更改[应用配置](/workflow/configuration/)后，需要运行 `npx expo prebuild`，更改才能生效。
