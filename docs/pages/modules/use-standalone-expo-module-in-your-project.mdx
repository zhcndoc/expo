...
> **提示** 要了解如何将项目配置为 monorepo，请查看 [使用 monorepo](/guides/monorepos/) 指南。
...
- **创建 tarball**：使用 `npm pack` 为模块创建 tarball，然后通过运行 `npm install /path/to/tarball` 将其安装到项目中。此方法有助于在发布之前本地测试模块，或与无法访问 npm 仓库的其他人共享模块。
- **运行本地 npm 仓库**：使用 [Verdaccio](https://verdaccio.org/) 等工具托管本地 npm 仓库。你可以从该仓库安装模块，这对于管理公司或组织内部包很有用。
