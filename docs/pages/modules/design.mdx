...
[Android lifecycle listeners](/modules/android-lifecycle-listeners/) 和 [iOS AppDelegate subscribers](/modules/appdelegate-subscribers/) 是一项强大的功能，允许你接入应用的生命周期，而无需将模块的代码分散到 `MainActivity` 和 `AppDelegate` 类中，也不需要你的库用户这样做。这对于与 [Continuous Native Generation](/workflow/continuous-native-generation/) 的平滑集成尤其有用，因为它为库提供了一种以可组合方式接入应用生命周期事件的机制——而无需担心其他库可能正在做什么。
