...
要响应与应用相关的某些 Android 系统事件，例如传入链接和配置更改，需要在 **MainActivity.java** 和/或 **MainApplication.java** 中重写相应的生命周期回调。
...
首先，你需要创建一个 Expo 模块，或者使用 React Native 模块 API 在库中集成 Expo 模块 API。请参阅[什么是 Expo 模块 API](/modules/overview/#what-is-the-expo-modules-api)。
...
export function App() {
  ...
  return (
    <View style={styles.container}>
      <Text>初始 URL：{initialUrl || '无'}</Text>
      <Text>当前 URL：{url || '无'}</Text>
      {event && (
        <View style={styles.textContainer}>
          <Text>最新深度链接：</Text>
          <Text>协议：{event.scheme}</Text>
          <Text>主机：{event.host}</Text>
          <Text>路径：{event.path}</Text>
        </View>
      )}
    </View>
  );
