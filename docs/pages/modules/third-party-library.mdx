...
> **info** **提示**：如果你不打算发布这个库，请在终端窗口中的所有提示处按下 <kbd>Return</kbd> 以接受默认值。
...
<Terminal
  cmd={{
    npm: [
      '# 确保在运行下面的命令之前你已位于 expo-radial-chart 目录中',
      '$ npm run build',
    ],
    yarn: [
      '# 确保在运行下面的命令之前你已位于 expo-radial-chart 目录中',
...
要在应用中使用该模块，请定义这些 props 的类型。它接受一个系列列表——每个系列都包含一个颜色和一个百分比值。
...
2. 定义一个接受 `Series` 对象列表的 `setChartData` 函数。你可以遍历该列表，为每个系列创建一个 `PieEntry`，并将颜色存储在一个单独的列表中。
...
3. 创建一个接受系列列表的 `setChartData` 函数，使用数据创建一个 `PieChartDataSet` 实例，并将其赋值给 `PieChartView` 实例的 `data` 属性。
