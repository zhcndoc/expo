...
> **注意：** 在 SDK 57 过渡期间，不使用 `--template` 标志的 `create-expo-app@latest` 会创建一个 SDK 54 项目。如果你计划在实体设备上使用 Expo Go，请使用 SDK 54 项目。否则，请使用 `--template default@sdk-57` 来创建 SDK 57 项目。
...
使用[受支持的 Node 包管理器](#node-package-managers-support)运行 `create-expo-app`，会使用默认模板初始化并设置新的 Expo 项目。
...
- 运行 `npx create-expo-app --example` 会显示一个可交互的可用示例列表供你选择
- 运行 `npx create-expo-app --example with-router` 会设置一个使用 Expo Router 库的项目
- 运行 `npx create-expo-app --example with-react-navigation` 会设置一个类似默认模板的项目，但配置的是普通的 React Navigation 库

#### 面向 AI 代理的 Expo Skills

如果你使用 AI 代理，请安装 [Expo Skills](/skills/)，以便让它学习示例仓库中的规范模式：
...
使用 `create-expo-app` 创建新项目时，还会处理特定 Node 包管理器所需的额外配置。
