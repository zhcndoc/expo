...
> **注意：** 在 SDK 56 过渡期间，未使用 `--template` 标志的 `create-expo-app@latest` 会创建一个 SDK 54 项目。如果你计划在实体设备上使用 Expo Go，请使用 SDK 54 项目。否则，请使用 `--template default@sdk-56` 来创建一个 SDK 56 项目。
...
使用 [Node 包管理器](#node-package-managers-支持) 运行 `create-expo-app` 会使用默认模板初始化并设置一个新的 Expo 项目。
...
使用 `create-expo-app` 创建新项目时，也会处理为特定 Node 包管理器所需的额外配置。
