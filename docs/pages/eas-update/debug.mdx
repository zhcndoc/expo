...
title: EAS 更新调试
sidebar_title: 调试
description: 了解如何使用基本的调试技术来解决更新问题。
hasVideoLink: true
---
...
如果部署的渠道不符合预期，这意味着我们的构建不是使用正确的渠道构建的。要修复此问题，请[配置我们的渠道](#configure-channel)并重新构建应用。
...
如果部署的运行时版本不符合预期，这意味着我们的构建不是使用正确的运行时版本构建的。要修复此问题，请[配置我们的运行时版本](#configure-runtime-version)并重新构建应用。
...
如果部署关联到了不符合预期的分支，我们需要[将我们的渠道映射到正确的分支](#map-channel-to-branch)。
...
所显示的部署没有任何更新。要修复此问题，请[向该分支发布更新](#publish-update)。如果更新已经发布，请检查[更新页面](https://expo.dev/accounts/[account]/projects/[project]/updates)，确保它与我们构建的运行时版本匹配。
...
所显示的部署具有正确的渠道，但未链接到任何分支。要修复此问题，请[将渠道映射到正确的分支](#map-channel-to-branch)。

### 缺少部署

如果我们的部署没有显示出来，这意味着我们的构建没有为 EAS Update 正确配置。要修复此问题，请[配置我们的渠道](#configure-channel)、[配置我们的运行时版本](#configure-runtime-version)，并验证我们的[通用配置](/eas-update/debug/#verifying-app-configuration)。在进行这些更改后，我们需要重新构建应用。

### 更新崩溃时自动回滚

如果部署页面上一切看起来都正确，但你的应用仍然显示之前的更新或构建中嵌入的代码，那么新更新中的代码可能发生了崩溃。这种情况可能发生在应用下载并应用新更新后，新更新在根组件渲染之前崩溃。
...
如果你的用户看到“无法加载所有资源”错误，这意味着应用能够下载清单，但无法下载更新运行所需的所有资源。如果某个资源不在原始构建中，就需要下载它。常见原因包括：
...
- [检查日志条目](/versions/latest/sdk/updates/#updatesreadlogentriesasyncmaxage)，这些日志条目来自原生层。
- 手动获取并[加载更新](/versions/latest/sdk/updates/#example-check-for-updates-manually)。
...
Branch 详情页面会显示一个 update 列表及其 runtime version 和 platform。根据这个列表，我们应该能够通过将构建的 runtime version 和 platform 与 update 的 runtime version 和 platform 进行匹配，来判断某个构建应应用哪个 update。最新的兼容 update 将可供构建下载并执行。

<ContentSpotlight
  alt="Branch 详情页面上的 update 列表"
...
`expo-updates` 库导出了各种函数，用于在应用已经运行后与 updates 进行交互。在某些情况下，调用函数获取 update 并查看错误消息，可以帮助我们缩小根本原因的范围。我们可以为项目创建一个模拟器构建，并手动检查是否有可用的 updates，或者在获取 updates 时是否出现错误。请参阅代码示例以[手动检查 updates](/versions/latest/sdk/updates/#example-check-for-updates-manually)。
...
查看 update bundle 中包含哪些资产可能会有所帮助。我们可以在 [Updates Detail](https://expo.dev/accounts/[account]/projects/[project]/updates) 页面中查看命名资产列表：
