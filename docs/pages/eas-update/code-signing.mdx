...
> **信息** EAS 更新代码签名仅适用于订阅了 EAS Production 或 Enterprise 计划的账户。请参阅 [EAS 定价计划](https://expo.dev/pricing)。

`expo-updates` 库支持使用[公钥密码学](https://en.wikipedia.org/wiki/Public-key_cryptography)进行端到端代码签名。代码签名允许开发者使用自己的密钥对更新进行加密签名。随后会在客户端应用更新之前验证这些签名，这确保了互联网服务提供商、CDN、云服务提供商，甚至 EAS 本身都无法篡改由应用运行的更新。
...
- 证书有效期是一个可根据应用安全需求而变化的设置。
  - 较短的有效期会要求更频繁地进行[密钥轮换](#key-rotation)，但通常被认为是更好的做法，因为一旦私钥泄露，其过期时间会更早，从而限制影响范围。
...
从应用中移除代码签名的过程与[密钥轮换](#key-rotation)类似，可以将其视为将密钥轮换到一个 `null` 密钥。
