...
你可以通过在应用处于后台时检查并获取更新来下载**后台更新**，这也称为_后台获取_。你可以使用 [`expo-background-task`](/versions/latest/sdk/background-task/) 在后台任务中运行与前台相同的 `Updates.checkForUpdateAsync()` 和 `Updates.fetchUpdateAsync()` 方法。即使用户有一段时间没有打开应用，这也是确保用户始终使用最新版本应用的好方法。
...
你可以在运行时使用 `Updates.setUpdateURLAndRequestHeadersOverride()` 方法覆盖更新 URL 和请求头。如果你希望加载特定更新，或在应用运行时更改更新通道，这会很有用。请参阅[如何在运行时覆盖更新配置](/eas-update/override/)。
