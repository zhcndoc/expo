...
如果 `expo-updates` 捕获到了一个 JS 错误，接下来会发生什么取决于 React Native 是否已经触发了原生的“内容已出现”事件（Android 上是 `ReactMarkerConstants.CONTENT_APPEARED`，iOS 上是 `RCTContentDidAppearNotification`）——对于这个特定更新来说，这大致对应于你的应用的首个视图是在本次启动还是上一次启动中已经渲染到屏幕上。
