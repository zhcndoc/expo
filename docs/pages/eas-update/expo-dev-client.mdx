...
本指南将介绍如何使用 **扩展** 选项卡或构建特定的更新 URL，在开发构建中加载并预览已发布的更新。

<Prerequisites>
  <Requirement title="已安装开发构建">
    [创建开发构建并将其安装
    到你的设备、Android 模拟器或 iOS 模拟器上](/develop/development-builds/introduction/?buildenv=build-with-eas#how-would-you-like-to-build-your-development-build)。
  </Requirement>
  <Requirement
    title={
      <>
        已安装 <CODE>expo-updates</CODE>
      </>
    }>
    确保你的开发构建已[安装 `expo-updates` 库](/eas-update/getting-started/#configure-your-project)。
  </Requirement>
</Prerequisites>
...
构造好 URL 后，将其直接复制并粘贴到开发构建启动器屏幕中的 **手动输入 URL** 下。
