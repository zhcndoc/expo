...
加载错误会在父路由中通过 [`ErrorBoundary`](/router/error-handling/#error-handling) 导出项进行处理。
...
<Terminal
  cmd={{
    npm: ['$ npx expo start --clear', '', '# 或在导出时', '$ npx expo export --clear'],
    yarn: ['$ yarn expo start --clear', '', '# 或在导出时', '$ yarn expo export --clear'],
    pnpm: ['$ pnpm expo start --clear', '', '# 或在导出时', '$ pnpm expo export --clear'],
    bun: ['$ bun expo start --clear', '', '# 或在导出时', '$ bun expo export --clear'],
  }}
/>
...
异步路由代表了我们未来计划支持 [React Server Components](https://react.dev/blog/2023/03/22/react-labs-what-we-have-been-working-on-march-2023#react-server-components) 的早期预览。因此，有一些注意事项需要了解：

- 异步路由目前还不支持原生生产应用。
- 在开发环境中，运行时 JavaScript 会以懒加载方式打包，因此你可能会遇到 HTML 与可用 JavaScript 不匹配的情况。
- 自定义的 [`SuspenseFallback`](/router/error-handling/#loading-states-with-suspense-fallback) 导出不适用于异步路由。
