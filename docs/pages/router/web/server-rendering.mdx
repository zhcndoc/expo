...
服务器端渲染（SSR）会在每次请求时动态生成 HTML，而 [静态渲染](/router/web/static-rendering) 则是在构建时预渲染 HTML。本指南将引导你为 Expo Router 应用启用服务器渲染。
...
这会创建一个包含服务端渲染应用的 **dist** 目录。与静态渲染不同，它不会预先生成 HTML 文件。相反，输出会包含如下所示的类似目录结构：
...
来自 `expo-router/html` 的 [`useServerDocumentContext`](/versions/latest/sdk/router/#useserverdocumentcontext) hook 提供了服务器渲染器注入到文档中的元数据和资源节点。你必须将这些值展开到 HTML 中，以确保响应中包含元数据、字体和 CSS：
...
> **信息** 在创建自定义 **+html.tsx** 模板时，你必须使用 [`useServerDocumentContext`](/versions/latest/sdk/router/#useserverdocumentcontext) 返回给你的所有属性。否则，你的服务端渲染 HTML 可能会显示异常，或者应用可能无法正常工作。
...
- 它不会在客户端重新 hydration，并且只能使用 [`useServerDocumentContext`](/versions/latest/sdk/router/#useserverdocumentcontext) React hook
...
路由可以导出一个 [`generateMetadata`](/versions/latest/sdk/server/#generatemetadatafunctionrequest-params) 函数，用于定义每个页面的元数据，例如 title、description 和 [Open Graph](https://ogp.me/) 标签。该函数在服务器上于渲染开始前运行，其结果会通过 [Root HTML](#root-html) 组件中 [`useServerDocumentContext`](/versions/latest/sdk/router/#useserverdocumentcontext) 提供的 `headNodes` 注入到 HTML 文档的 `<head>` 中。

从你的路由文件中导出一个 [`generateMetadata`](/versions/latest/sdk/server/#generatemetadatafunctionrequest-params) 函数，并返回一个 [`Metadata`](/versions/latest/sdk/server/#metadata) 对象。该函数会接收传入的请求和路由参数，你可以利用它们动态生成元数据：
...
`generateMetadata` 函数在服务器上执行，并会像 [数据加载器](/router/web/data-loaders) 一样从客户端包中移除。支持的元数据字段完整列表请参见 `expo-server` API 参考中的 [`Metadata`](/versions/latest/sdk/server/#metadata) 类型。
...
| 动态路由           |     | 需要 [`generateStaticParams`](/router/web/static-rendering/#generatestaticparams) | 自动运行             |
| 元数据             |     | [`<Head>`](#using-head-with-server-rendering) 组件                               | [`generateMetadata`](#metadata) |
| 需要服务器         |     | <NoIcon />                                                                            | <YesIcon />                     |
| 首字节时间         |     | 最快（已缓存）                                                                      | 较慢（每次请求时渲染）   |
| 托管               |     | 任意静态托管服务                                                                       | 需要服务器运行时         |
