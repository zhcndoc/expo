...
在你的 [应用配置](/versions/latest/config/app/) 中的 `expo-router` 插件里配置头信息：
...
<Terminal
  cmd={{
    npm: ['$ npx expo start', '', '# 或导出用于生产', '$ npx expo export -p web'],
    yarn: ['$ yarn expo start', '', '# 或导出用于生产', '$ yarn expo export -p web'],
    pnpm: ['$ pnpm expo start', '', '# 或导出用于生产', '$ pnpm expo export -p web'],
    bun: ['$ bun expo start', '', '# 或导出用于生产', '$ bun expo export -p web'],
  }}
/>
...
某些 Web API，例如 [`SharedArrayBuffer`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/SharedArrayBuffer)，需要特定的跨域头信息。这对于像 [`expo-sqlite` 在 web 上](/versions/latest/sdk/sqlite/#web-setup) 这样的功能是必需的。
