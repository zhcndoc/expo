...
> **info** 使用静态渲染时，[数据加载器](/router/web/data-loaders) 会在构建过程中执行，并将其结果嵌入到输出的 HTML 文件中。

## 设置

<Step label="1">

在你项目的 [应用配置](/versions/latest/config/app/) 中启用静态渲染：
...
export async function generateStaticParams(): Promise<Record<string, string>[]> {
  /* @info 任何导出 `generateStaticParams` 的动态子路由都会针对数组中的每一项调用一次。 */
...
> **注意**：全局上下文提供者应放在 [根布局](/router/basics/navigation-layouts/#root-layout) 组件中，而不是根 HTML 组件中。
...
- 不要将全局 CSS 导入到这个文件中。应改为使用 [根布局](/router/basics/navigation-layouts/#root-layout)。Expo Router 会从根布局开始遍历依赖图，因此在其他地方导入 CSS 可能会导致意外的加载顺序，使 **node_modules** 中的 CSS 优先于你的自定义样式。
...
- `ScrollViewStyleReset`：对于带有根 `<ScrollView />` 的全屏 [React Native Web 应用](https://necolas.github.io/react-native-web/docs/setup/#root-element)，应使用根样式重置，以确保与原生端保持一致。
...
> **警告** 某些路径（例如 `/assets`）是 Metro 预留的。请避免将文件放在 **public/assets/** 或其他预留路径中。完整列表请参见 [预留路径](/router/reference/reserved-paths/)。
...
> **提示** **仅限 Web**：静态资源可以在运行时代码中通过相对路径访问。例如，**logo.png** 可以通过 `/logo.png` 访问：
