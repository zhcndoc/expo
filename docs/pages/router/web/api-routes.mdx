...
在开发环境中，相对路径的 fetch 请求会自动相对于开发服务器源地址发起；而在生产环境中，可以在 **app.json** 中通过 `origin` 字段进行配置：
...
> **important** 服务器运行时 API 和 `expo-server` 在 SDK 54 及更高版本中可用，并且在生产环境中使用时需要已部署的服务器。
...
- [TypeScript](/guides/typescript) — 类型和 [**tsconfig.json** 路径别名](/guides/typescript/#path-aliases-optional)。
- [Environment variables](/guides/environment-variables) — 服务器路由可以访问所有环境变量，而不只是以 `EXPO_PUBLIC_` 为前缀的那些。
- Node.js 标准库 — 确保你在本地使用的 Node.js 版本与服务器环境所需版本一致。
- **babel.config.js** 和 **metro.config.js** 支持 — 配置可同时作用于客户端和服务器代码。
...
如果你想导出 API 路由并跳过生成应用的网站版本，可以使用以下命令，它将生成一个仅包含项目服务端代码的 **dist** 目录。
...
console.log(`Bun 服务器正在运行于 http://localhost:${port}`);
```
...
app.listen(port, () => {
  console.log(`Express 服务器监听端口 ${port}`);
});
```
...
如果你正在使用任何环境变量或 **.env** 文件，请将它们添加到 Netlify。你可以通过进入 **站点设置** 并将它们添加到 **构建与部署** 部分来完成。
