...
如果你使用 AI 代理，请安装 [Expo Skills](/skills/)，以教会它包括数据加载器在内的数据获取模式：
...
> **信息** [`useLoaderData`](/versions/latest/sdk/router/#useloaderdata) 钩子不需要在路由组件本身中调用。它可以在路由组件树中的任何子组件里调用。
...
> **警告** 在使用静态渲染时，`request` 参数为 `undefined`，因为在构建时没有 HTTP 请求。
...
| 适用场景             | 博客、营销页面、文档                  | 个性化内容、依赖身份验证的页面                                      |
...
对于只需要路由参数的路由，请使用 [`createStaticLoader`](/versions/latest/sdk/server/#createstaticloaderfn)。回调只会接收路由参数，并且可以安全地用于静态渲染和服务器渲染：
...
对于需要访问传入 HTTP 请求的路由，请使用 [`createServerLoader`](/versions/latest/sdk/server/#createserverloaderfn)。回调会接收 [`ImmutableRequest`](/versions/latest/sdk/server/#immutablerequest) 和路由参数作为参数：
...
> **警告** 如果在静态站点生成（SSG）期间调用 `createServerLoader`，它会抛出错误，因为构建时没有 HTTP 请求。使用静态渲染时请改用 `createStaticLoader`。

### 直接使用 `LoaderFunction`

你也可以使用 `expo-router/server` 中的 [`LoaderFunction`](/versions/latest/sdk/server/#loaderfunctionrequest-params) 类型直接为 loader 添加类型。这让你可以完全控制函数签名，包括 `request` 和 `params`：
