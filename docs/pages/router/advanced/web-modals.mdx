...
> **重要** Web 模态框处于 [alpha](/more/release-statuses/#alpha) 阶段，并可在 SDK 54 及更高版本中使用。要使用此功能，你必须在项目中设置 `EXPO_UNSTABLE_WEB_MODAL=1` 环境变量。
...
> **提示** 要使用新的 Web 模态功能，你必须在开发和 [导出](/deploy/web/#export-your-web-project) 构建中都设置 `EXPO_UNSTABLE_WEB_MODAL=1` 环境变量。你可以通过将其添加到项目根目录的 **.env** 文件中，或者在命令前加上前缀来实现，例如：`EXPO_UNSTABLE_WEB_MODAL=1 npx expo start`。
...
> **提示** **注意：** `webModalStyle` 属性仅适用于 web 平台。在移动端，模态会自动适配为适合触控交互的 sheet 行为。
...
<Collapsible summary="全屏模态示例">

如果你想为占据最大空间的内容创建一个全屏模态，可以在模态路由的 `Stack.Screen` 选项中使用 `webModalStyle` 属性：
...
当在移动设备上运行 Web 应用时，如果你想避免显示全屏模态，可以将 `sheetAllowedDetents` 设置为 `fitToContents` 或自定义值：
...
<Collapsible summary="紧凑模态示例">

对于较小的交互，你可以创建一个会根据内容自适应的紧凑模态：
...
<Collapsible summary="透明模态示例">

当你想显示一个遮罩层并保留底层屏幕的视觉上下文时，可以将 `presentation` 选项设置为 `transparentModal`：
...
<Collapsible summary="自定义停靠点示例">

你可以使用 `sheetAllowedDetents` 来定义模态可停留的高度：
...
对于你的 Web 应用，如果项目使用了 [全局 CSS](https://docs.expo.dev/versions/latest/config/metro/#global-css) 文件，你还可以覆盖宽度、高度、边框和遮罩层变量。
...
/* @info 可以在任意堆栈的 `_layout.tsx` 文件中设置 `unstable_settings`。它用于定义堆栈的初始路由名称，从而确保用户拥有一致的起始点，尤其是在深度链接时。 */
export const unstable_settings = {
  ...
  return (
    <Stack>
      ...
      <Stack.Screen
        name="modal"
        options={{
          /* @info 将 `presentation` 模式设置为模态路由的 `transparentModal`。*/
          presentation: 'transparentModal',
          /* @end */
          /* @info （可选）将 `animation` 设置为 `fade`。*/
          animation: 'fade',
          /* @end */
