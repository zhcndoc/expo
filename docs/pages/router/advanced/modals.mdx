...
<VideoBoxLink
  ...
  description="了解如何以不同方式在应用的其余部分之上显示内容。"
  className="mb-6"
/>
  ...
  return (
    <Stack>
      ...
      <Stack.Screen
        name="modal"
        options={{
          /* @info 将 `presentation` 模式设置为模态路由的 `modal`。 */
          presentation: 'modal',
          /* @end */
        }}
      />
...
export default function Home() {
  return (
    <View style={styles.container}>
      ...
      /* @info 使用 `Link` 组件导航到模态屏幕。`href` 属性是模态屏幕的路由名称。 */
...
import { Link, /* @info 导入 `router` 对象，它用于以命令式方式进行导航。 */ router /* @end */} from 'expo-router';
import { StyleSheet, Text, View } from 'react-native';

export default function Modal() {
  /* @info 使用 `router.canGoBack()` 来检查模态框是否作为独立屏幕呈现。如果屏幕是重新加载的或直接导航到的，那么模态框应以全屏方式呈现。你可能需要调整 UI 以适应这一点。 */
  ...
  return (
    <View style={styles.container}>
      <Text>模态屏幕</Text>
      /* @info 在 Web 上，使用 `../` 作为导航到根目录的一种简单方式。这与 `goBack` 并不相同。*/
...
export default function Modal() {
  return (
    <View style={styles.container}>
      <Text>模态屏幕</Text>
      /* @info 使用 `Platform.OS` 检查当前平台是否为 iOS，然后使用 `StatusBar` 组件更改外观。 */
  ...
  return (
    <Stack>
      ...
      <Stack.Screen
        name="modal"
        options={{
          /* @info 将 `presentation` 模式设置为 `formSheet`。 */
...
在 Android 上，form sheet 使用平台的底部抽屉行为。原生堆栈标题和嵌套堆栈导航器不支持在 form sheet 屏幕内使用，因此诸如 `headerShown`、`title` 和标题栏按钮之类的选项不会在 sheet 内渲染。如果你的 form sheet 需要标题或操作，请改为将它们作为 sheet 内容的一部分进行渲染。
