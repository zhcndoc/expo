...
Android 推荐使用 [`@expo/material-symbols`](https://www.npmjs.com/package/@expo/material-symbols) 库来提供图标。它将 Google 的 [Material Symbols](https://fonts.google.com/icons) 作为独立的资源子路径提供，因此 Metro 只会打包你实际导入的图标。
...
`Stack.Toolbar.Button` 的 `icon` 属性既接受 `ImageSourcePropType`（Android），也接受 SF Symbol 名称（iOS）。如果要在两个平台上使用同一个组件，请根据 `process.env.EXPO_OS` 分支，并传入适合平台的值。Metro 会在构建时将 `process.env.EXPO_OS` 替换为字符串字面量，然后对不匹配当前平台的分支进行 tree-shaking——因此 Material Symbols XML 可绘制对象不会出现在 iOS 包中，SF Symbol 名称也不会出现在 Android 包中：
...
> **信息** 徽标仅适用于 iOS 标题栏位置（`left` 或 `right`），不适用于底部工具栏，也不适用于 Android。
...
要修复此问题，请使用合适的主题将根布局包裹在来自 `expo-router` 的 `<ThemeProvider>` 中：
...
<Collapsible summary="Android 图标必须是图像源">
...
在 Android 上，`Stack.Toolbar.Button` 只会渲染图标——`Stack.Toolbar.Badge` 和 `Stack.Toolbar.Label` 子元素会被忽略。如果你需要在 Android 上实现类似徽标的界面，可以使用 [`Stack.Toolbar.View`](/versions/latest/sdk/router/stack/#stacktoolbarview) 嵌入自定义组件。
...
有关完整的 API 文档，包括所有可用的属性，请参阅 [`Stack.Toolbar` API 参考](/versions/latest/sdk/router/stack/#stacktoolbar)。
