...
Apple 接力（Handoff）是一项功能，它使用户能够在另一台设备上继续浏览您的应用或网站。Expo Router 为此功能自动处理所有运行时路由。不过，一次性配置必须手动设置。
...
- Handoff 需要配置 [通用链接](/linking/into-your-app/)，至少在 iOS 上，并且包含 `activitycontinuation` 对象。
...
您可以使用以下命令，根据您的 [应用配置](/versions/latest/config/app/) 生成 **apple-app-site-association** 文件：
...
请参阅 [测试深度链接](/linking/into-your-app/#test-the-deep-link) 指南，以在开发环境中测试接力（Handoff）。

### Expo Head 设置

请确保在 `app.config.tsx` 文件中使用 `expo-router` 配置插件设置 Handoff 源地址。此 URL 将在用户切换到您的应用时用作 `webpageUrl`。
...
> 在测试接力（Handoff）到原生端时，不要使用仅限开发环境的 `?mode=developer` 后缀。
...
在任何您想支持接力（Handoff）的路由中，使用来自 `expo-router/head` 的 `Head` 组件：
...
| Meta 标签                | 描述                                                                                                                                                                                                                                               |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `expo:handoff`           | 设置为 `true` 以为当前路由启用接力（Handoff）。默认值为 `false`。（仅 iOS）                                                                                                                                                                                |
| `og:title` 和 `<title>` | 为 `NSUserActivity` 设置标题；在接力（Handoff）中不使用此项。                                                                                                                                                                                             |
| `og:description`         | 为 `NSUserActivity` 设置描述；在接力（Handoff）中不使用此项。                                                                                                                                                                                           |
