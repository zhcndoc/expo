...
在原生应用中，重写 URL 的另一种方式是在 [`redirectSystemPath`](#using-redirectsystempath) 方法中处理。这种方式对于某些使用场景来说更加简单，但也存在一些缺点：
...
不建议在新项目或集成中使用此 API。它的使用方式与服务器端路由和[静态渲染](/router/web/static-rendering/)不兼容，并且在离线或网络连接状况较差的环境中可能难以管理。
