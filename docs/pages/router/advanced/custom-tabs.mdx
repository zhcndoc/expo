...
<FileTree files={['_layout.tsx', ['(one,two)/route.tsx', '一个共享组中的路由']]} />
...
<Tabs>
  ...
  {/* 自定义标签栏 */}
  <View>
    /* @info 一个自定义标签栏。`TabList` 外部的 `TabTrigger` 不需要 `href` 属性。 */
    <View>
...
所有组件也都有对应的 hook 版本，让你可以控制渲染树。有关可用 hooks 的完整列表，请参阅 [Router UI 参考](/versions/latest/sdk/router/ui/)。
...
`TabSlot` 接受一个 `renderFn` 属性。该函数可用于覆盖屏幕的渲染方式，使你能够实现动画或屏幕持久化/卸载等高级功能。有关更多信息，请参阅 [Router UI 参考](/versions/latest/sdk/router/ui/)。
...
你可以向 `TabSlot` 提供一个自定义渲染器，以自定义其渲染屏幕的方式。你可以利用它来检测屏幕何时获得焦点，并相应地执行动画。
