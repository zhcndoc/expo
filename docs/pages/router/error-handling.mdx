...
Expo Router 会将每个路由包裹在一个 [React Suspense](https://react.dev/reference/react/Suspense) 边界中。你可以从布局文件中导出一个 [`SuspenseFallback`](/versions/latest/sdk/router/#suspensefallback) 组件，以自定义任何子路由挂起时显示的加载界面：
...
[`SuspenseFallback`](/versions/latest/sdk/router/#suspensefallback) 组件接收 `route` 和 `params` 属性。你可以使用 `params` 来为动态路由显示上下文相关的加载状态，例如：
