...
description: Expo Router 是一个用于使用 Expo 构建的通用 React Native 应用程序的开源路由库。
sidebar_title: 简介
searchRank: 15
---
...
Expo Router 将来自 web 的最佳文件系统路由理念带到了通用应用中——让你的路由在所有平台上都能正常工作。当你向 **app** 目录添加一个文件时，该文件会自动成为你导航中的一个路由。

## 快速开始

<Step label="1">

我们建议使用 `create-expo-app` 创建一个新的 Expo 应用，这样你就能得到一个已经安装并配置好 Expo Router 库的项目：
...
> **注意：** 在 SDK 57 过渡期间，未使用 `--template` 标志的 `create-expo-app@latest` 会创建一个 SDK 54 项目。如果你计划在实体设备上使用 Expo Go，请使用 SDK 54 项目。否则，请使用 `--template default@sdk-57` 创建 SDK 57 项目。
...
- 若要在移动设备上查看你的应用，我们建议从 [Expo Go](/get-started/set-up-your-environment/#how-would-you-like-to-develop) 开始。随着应用复杂度增加并且你需要更多控制时，可以创建一个 [开发构建](/develop/development-builds/introduction/)。
- 在终端 UI 中按 <kbd>W</kbd> 可在 web 浏览器中打开项目。按 <kbd>A</kbd> 可打开 Android（需要 Android Studio），或按 <kbd>I</kbd> 可打开 iOS（需要安装 Xcode 的 macOS）。
...
如果你使用 AI 代理，请安装 [Expo Skills](/skills/)，教会它基于文件的导航模式：

<RelatedSkills names={['expo-router']} />

## 主要特性

- **原生**：构建于 [React Native Screens](https://github.com/software-mansion/react-native-screens) 之上，Expo Router 导航默认就是真正的原生体验，并针对平台进行了优化。
- **可分享**：应用中的每个屏幕都会自动支持 [深度链接](/linking/overview/)，使应用中的任何路由都可以通过链接分享。
- **离线优先**：应用会被缓存并以离线优先方式运行，在你发布新版本时会自动更新。无需网络连接或服务器即可处理所有进入的原生 URL。
- **已优化**：路由会通过 [生产环境中的懒求值](/router/web/async-routes/) 自动优化，并在开发环境中进行延迟打包。
- **快速迭代**：在 Android、iOS 和 Web 上都能实现统一的 Fast Refresh，同时在打包器中进行产物记忆化，以便在规模扩大时仍然能够保持快速迭代。
- **通用**：Android、iOS 和 Web 共享统一的导航结构，并且可以在路由级别下沉到平台特定 API。
- **可发现**：Expo Router 支持 Web 端的构建时 [静态渲染](/router/web/static-rendering/)，以及面向原生端的 [通用链接](/linking/overview/)。这意味着你的应用内容可以被搜索引擎索引。
...
<Collapsible summary="Expo Router 与 Expo 以及 React Native CLI 有什么区别？">

从历史上看，React Native 并没有规定应用应该如何构建，这有点像在没有现代 web 框架的情况下使用 React。Expo Router 是一个面向 React Native 的有主见框架，类似于 Remix 和 Next.js 之于仅面向 web 的 React。
...
<BoxLink
  title="手动安装"
  Icon={BookOpen02Icon}
  description="关于如何开始以及将 Expo Router 添加到现有应用的详细说明。"
  href="/router/installation/"
/>
<BoxLink
  title="路由器 101"
  Icon={BookOpen02Icon}
  description="如需了解核心概念、表示模式、导航布局以及常见导航模式，请从路由器 101 部分开始。"
