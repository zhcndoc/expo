...
1. 通过按下终端窗口中正在运行的开发服务器里的 <kbd>J</kbd> 来启动 React Native DevTools
2. 通过点击齿轮图标打开 **Settings**
3. 在 **Extensions** 下，点击 **Restore defaults and reload**
4. 再次打开 **Settings**，然后转到 **Ignore List** 选项卡
...
<Terminal
  cmd={[
    '$ 第 11 行无效调用：process.env.EXPO_ROUTER_APP_ROOT require.context 的第一个参数应为字符串。',
  ]}
/>
...
如果你设置了一个模态框或其他预期应有返回按钮的界面，那么你需要在路由的布局中添加 [`unstable_settings`](/router/advanced/router-settings/)，以确保初始路由已正确配置。初始路由在某种程度上对移动应用来说比较特殊，并且在系统中的契合方式略显别扭——后续会继续改进。
