...
export default function Route() {
  const { everything } = useLocalSearchParams<{
    /* @info **everything** 将是路径段数组，即使只有一个也一样 */
    everything: string[];
    /* @end */
  }>();
...
export default function Route() {
  const { everything } = useLocalSearchParams<{
    /* @info **everything** 将是路径段数组，即使只有一个也是如此 */
    everything: string[];
    /* @info **query** 是一个可选的搜索参数 */
    query?: string;
    /* @info **query2** 是一个可选的搜索参数 */
    query2?: string;
    /* @end */
  }>();
  const user = everything[0];
...
export default function Page() {
  ...
  return (
    <TextInput
      ...
      onChangeText={search => {
        setSearch(search);
        /* @info 将搜索参数 **query** 设置为文本输入 **search** */
...
URL [哈希](https://developer.mozilla.org/en-US/docs/Web/API/URL/hash) 是 URL 中跟在 `#` 符号后面的字符串。它通常用于网站中链接到页面的特定部分，但也可以用于存储数据。Expo Router 将哈希视为一个名为 `#` 的特殊搜索参数。可以使用与 [搜索参数](#local-versus-global-url-parameters) 相同的 hooks 和 API 来访问和修改它。
