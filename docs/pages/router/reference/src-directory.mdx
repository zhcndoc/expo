...
在 **tsconfig.json** 文件中更新 [TypeScript 路径别名](/guides/typescript#path-aliases-optional)，将其指向 **src** 目录，而不是根目录。如果你使用默认的 `@/*` 别名，请将其设置为 **./src/\***：
...
- 你可以考虑更新任何[类型别名](/guides/typescript#path-aliases-optional)，将其指向 **src** 目录，而不是根目录。
