...
使用 Expo Router 时，你不需要在代码中定义初始路由或第一个屏幕。相反，当你打开应用时，Expo Router 会查找与 `/` URL 匹配的第一个 **index.tsx** 文件。在[默认模板](/router/introduction/#quick-start)中，该文件是 **src/app/index.tsx**。如果应用用户默认应从导航树中更深层的位置开始，你可以使用一个[路由组](/router/basics/notation/#parentheses)（即名称放在圆括号中的目录），该目录不会计入 URL 的一部分。如果你希望第一个屏幕是一组标签页，可以将所有标签页页面放在 **src/app/(tabs)** 目录中，并将默认标签页定义为 **index.tsx**。通过这种结构，`/` URL 会将用户直接带到 **src/app/(tabs)/index.tsx** 文件。
...
- **src/app/index.tsx** 是初始路由，当你打开应用或导航到 Web 应用的根 URL 时，它会首先显示。
- **src/app/home.tsx** 是路由为 `/home` 的页面，因此在浏览器中可以通过类似 `yourapp.com/home` 的 URL 导航到该页面，或者在原生应用中通过 `yourapp://home` 导航到该页面。
- **src/app/\_layout.tsx** 是根布局。之前可能放在 **App.jsx** 中的任何初始化代码，都应放在这里。
- **src/app/profile/friends.tsx** 是路由为 `/profile/friends` 的页面。
- **src/components/app-tabs.native.tsx** 和 **src/components/app-tabs.tsx** 是[平台特定](/router/advanced/platform-specific-modules/)的标签页组件。**app-tabs.native.tsx** 文件用于 Android 和 iOS，而 **app-tabs.tsx** 文件用于 Web。根布局会导入这些文件来渲染标签页导航器。
- **src/components/text-field.tsx** 和 **src/components/toolbar.tsx** 不在 **src/app** 目录中，因此不会被视为页面。它们不会拥有 URL，也不能作为导航操作的目标。不过，它们可以作为 **src/app** 目录中页面的组件使用。
