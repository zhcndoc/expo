...
一旦你的应用中有了几个页面并完成了它们的布局设置，就该开始在它们之间进行导航了。Expo Router 中的每个页面默认都有一个 URL，因此你可以使用链接以及与 Web 上相同的 URL 模式在页面之间导航。

## 使用 `useRouter` 的原生导航基础

和 React Navigation 一样，你可以在 `onPress` 处理函数中调用一个函数来导航到另一个页面。在 Expo Router 中，你可以使用 `useRouter` 钩子来访问导航函数：
...
<BoxLink
  title="路由器命令式导航 API 参考"
  description="了解如何使用命令式导航提供的所有函数。"
...
export default function Page() {
  return (
    /* @info 导航到 `/other` 的 `onPress` 事件会传递给 `Pressable`。 */
...
{/* prettier-ignore */}
```tsx src/app/index.tsx
/* @info 从 `expo-router` 导入 `Link` React 组件和 `router`，以便通过命令式方式进行导航。 */
...
if (navigation.isFocused()) {
  navigation.setOptions({ title: '已更新标题' });
}
```
