...
/* @info 设置 `initialRouteName` 可确保直接深度链接到堆栈中的某个页面时，仍然会先将 index 路由压入堆栈。 */
...
指向属于另一个标签页的 URL 的 `Link` 会切换标签页，此时返回按钮会弹出该标签页内部的堆栈。在上面的例子中，从 Settings 标签页打开 `/feed/123` 会进入 Feed 标签页中的帖子。返回时会回到 `feed/index`，而不是 Settings。

使用 [native tabs](/router/advanced/native-tabs/) 时，标签导航器会在挂载后立即加载每个标签页。包含堆栈的标签页在你链接进入之前，就已经停留在该堆栈的第一个屏幕上。无论你是否传递 `withAnchor`，这个屏幕都会位于你链接到的屏幕下方。

如果你希望返回按钮回到你所在的屏幕，请将详情路由移出 tabs，改为从外层 stack 推入：

<FileTree
  files={[
    ['src/app/\_layout.tsx', '包含 tabs 的 stack'],
    ['src/app/(tabs)/\_layout.tsx'],
    ['src/app/(tabs)/index.tsx'],
    ['src/app/(tabs)/feed/\_layout.tsx'],
    ['src/app/(tabs)/feed/index.tsx'],
    ['src/app/(tabs)/settings.tsx'],
    ['src/app/feed/[postId].tsx', '推入标签栏上方'],
  ]}
/>
...
现在，`/feed/123` 是根 stack 中的屏幕，而不是 Feed 标签页中的屏幕。路由器会将它推入当前打开的标签页上方，返回时会回到该标签页。这也是 iOS 应用中详情屏幕通常的行为。
...
export default function Layout() {
  ...
  return (
    <SafeAreaView>
      <Stack />
      <Modal visible={!isAuthenticated}>{/* 登录 用户体验 */}</Modal>
    </SafeAreaView>
  );
}
```
