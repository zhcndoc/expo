...
> **info** 本指南适用于 **SDK 56 及更高版本**。在 SDK 56 中，Expo Router 停止接受来自 `@react-navigation/*.` 的应用代码导入。它们现在来自 `expo-router/\*` 入口点。如果你正在将现有的 Expo Router 应用从 SDK 55 或更早版本升级，请按照 [SDK 55 到 56 迁移指南](/router/migrate/sdk-55-to-56) 进行操作。下面的示例已经使用了 SDK 56 及更高版本的导入路径。
...
在 Expo Router 中，搜索参数只能序列化顶层值，例如 `number`、`boolean` 和 `string`。React Navigation 没有相同的限制，因此用户有时会传入无效参数，例如函数、对象、映射等。
...
要访问 [`navigation.navigate`](https://reactnavigation.org/docs/navigation-object/#navigate)，请从 [`useNavigation`](/versions/latest/sdk/router/#usenavigationparent) hook 中导入 `navigation` prop。
...
function Header() {;
  ...
  return (
    <View>
      ...
      <Link
        href="/profile"
        style={/* @info 使用 `pathname` 来判断链接是否处于激活状态。 */[pathname === '/profile' && { color: 'blue' }]/* @end */}>
        个人资料
      </Link>
...
你可以使用 React Navigation 库中的 [`reset`](https://reactnavigation.org/docs/navigation-actions/#reset) 操作来重置导航状态。它通过 Expo Router 的 [`useNavigation`](/versions/latest/sdk/router/#usenavigationparent) hook 分发，以访问 `navigation` 属性。
...
```tsx src/app/_layout.tsx
/* @info 从 React Navigation 直接导入主题 API。 */
import { ThemeProvider, DarkTheme, DefaultTheme, useTheme } from 'expo-router/react-navigation';
/* @end */
import { Slot } from 'expo-router';
...
[React Navigation Elements](https://reactnavigation.org/docs/elements/) 库提供了一组 UI 元素和辅助工具，可用于构建导航 UI。这些组件设计为可组合且可定制的。你可以复用该库中的默认功能，或在此基础上构建你自己的导航器 UI。
