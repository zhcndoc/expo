...
> **警告** `@expo/webpack-config` 已[弃用](/more/release-statuses/#deprecated)，并且不再接收任何新功能更新。
...
<Terminal
  cmd={{
    npm: [
      ...
      '',
      '? 你的 Web 应用根目录是什么（也就是你要部署哪个目录）？ dist/',
      '? 你希望预缓存哪些文件类型？ js, html, ttf, ico, json',
      '? 你希望将 service worker 文件保存到哪里？ dist/sw.js',
      '? 你希望将这些配置选项保存到哪里？ workbox-config.js',
      "? 你的 Web 应用清单中的 'start_url' 是否包含除 'utm_' 或 'fbclid' 之外的查询参数（例如 '?source=pwa'）？ 否",
    ],
    yarn: [
