...
Expo 使用 `metro-file-map` 的一个分支版本（[`@expo/metro-file-map`](https://github.com/expo/expo/tree/main/packages/@expo/metro-file-map)）来发现、监听并按需解析源文件。

Metro 会在启动时预先遍历 `watchFolders` 中列出的每个目录。在 [monorepo](/guides/monorepos) 中，这通常意味着会遍历每个工作区。启用按需文件系统访问后，文件映射可以在解析器请求时，按需读取 `watchFolders` 之外的文件。这使你能够安全地减少 `watchFolders` 条目的数量，以牺牲一部分文件监听覆盖范围来换取更快的启动速度，而不会破坏指向项目根目录之外的导入。

按需遍历还允许在 monorepo 根目录之外解析符号链接，这为 Bun 和 pnpm 等包管理器增加了 [Global Virtual Store 支持](https://pnpm.io/global-virtual-store)。

此功能默认启用，可通过在你的 [app config](/versions/latest/config/app/) 中将 `experiments.onDemandFilesystem` 设置为 `false` 来禁用：
...
When importing assets, a virtual module is created to represent the data required to import that asset.

On native platforms, the asset will be a numeric ID: `1`, `2`, `3`, etc., which can be looked up via `require("@react-native/assets-registry/registry").getAssetByID(<NUMBER>)`. On Web and server platforms, the asset will vary depending on the file type. If the file is an image, the asset will be `{ uri: string, width?: number, height?: number }`; otherwise, the asset will be a `string` representing the remote URL of that asset. Starting from SDK 55, you can use `String(asset)` to get the public URL of any asset on Web, excluding React Server Component environments that cannot contain the `toString` function.

Assets can be used as follows:
...
In API routes, you can always assume that the asset type will not be a number:
...
export async function GET(req: Request) {
  const ImageData = await fetch(
    new URL(
      ...
      asset.uri,
      // Append to the origin of the current request URL.
      req.url
    )
...
默认情况下，React Native 只支持使用根目录下的 `index.js` 文件作为入口文件（或者平台特定的变体，例如 `index.ios.js`）。Expo 项目允许使用任意入口文件，但这在现有 React Native 项目中需要额外设置。
