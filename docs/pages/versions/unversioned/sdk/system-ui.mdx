...
如果你的项目中使用了配置插件（[Continuous Native Generation (CNG)](/workflow/continuous-native-generation/)），你可以使用 `expo-system-ui` 内置的 [配置插件](/config-plugins/introduction/) 来进行配置。该插件允许你从 [app config](../config/app) 中配置 Android 上的 [`userInterfaceStyle`](../config/app/#userinterfacestyle) 和 iOS 上的 [`backgroundColor`](../config/app/#backgroundcolor) 属性。这些属性无法在运行时设置，且需要构建新的应用二进制文件后才会生效。如果你的应用**没有**使用 CNG，那么你需要手动配置该库。
...
如果你没有使用 Continuous Native Generation（[CNG](/workflow/continuous-native-generation/)），或者你正在手动使用原生 **android** 和 **ios** 项目，那么你需要向你的原生项目添加以下配置：
