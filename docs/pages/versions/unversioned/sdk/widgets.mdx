...
> **重要** 此库当前处于 [alpha](/more/release-statuses/#alpha) 阶段，可能会发生破坏性变更。它在 Expo Go 应用中不可用，请使用 [development builds](/develop/development-builds/introduction) 进行试用。
...
如果您的项目中使用了 config plugins（[Continuous Native Generation (CNG)](/workflow/continuous-native-generation/)），您可以通过其内置的 [config plugin](/config-plugins/introduction/) 来配置 `expo-widgets`。该插件允许您配置各种无法在运行时设置、且需要构建新的应用二进制文件后才会生效的属性。
