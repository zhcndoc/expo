...
title: 路由分屏视图
sidebar_title: 分屏视图
description: 一个 Expo Router 子模块，提供原生分屏布局。
...
> **重要** SplitView 是一个 [alpha](/more/release-statuses/#alpha) API，仅在 **Expo SDK 55** 及更高版本中可用于 **iOS**。该 API 可能会有破坏性变更，目前尚未准备好用于生产环境。

`expo-router/unstable-split-view` 是 `expo-router` 的一个子模块，导出用于使用 [平台原生系统分栏视图](https://developer.apple.com/design/human-interface-guidelines/split-views) 构建分栏布局的组件。
...
<Collapsible summary="不能在其他导航器中使用">

`SplitView` 不能在另一个导航器中使用（`Slot` 除外）。它必须用于根布局层级。

</Collapsible>

<Collapsible summary="只允许特定的子组件">

`SplitView` 仅接受 `SplitView.Column` 和 `SplitView.Inspector` 作为直接子组件。其他组件会被忽略，并伴随警告。
...
> **信息** **注意：** 我们正在积极开发 `SplitView` 并希望获得反馈。你可以在 [Discord](https://chat.expo.dev) 上分享你的想法，或在 [GitHub 上提交 issue](https://github.com/expo/expo/issues)，也可以使用本页底部的 **反馈** 按钮。
