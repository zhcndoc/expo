...
// 在模块作用域中定义任务，以便后台运行时可以找到它。
// 需要在全局作用域中调用，而不是在 React 组件中。
...
> **info** **注意：** [`TaskManager.defineTask`](#taskmanagerdefinetasktaskname-taskexecutor) 必须在模块作用域中调用，而不是在 React 组件内部。上面的示例将其放在屏幕文件的模块作用域中，这对非路由应用有效。在 Expo Router 项目中，请将其移到单独的文件中（例如 **tasks.ts**），并在根布局（**app/\_layout.tsx**）顶部导入该文件，这样在任何导航运行之前就会注册该任务。
