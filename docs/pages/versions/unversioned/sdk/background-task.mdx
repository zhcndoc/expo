...
> **信息** 在 Android 上，行为因设备厂商而异。例如，有些实现会将从最近任务列表中移除应用视为结束应用。了解更多这些差异： [https://dontkillmyapp.com](https://dontkillmyapp.com)。

## 平台差异

### Android&ensp;<PlatformTags platforms={['android']} />

在 Android 上，[`WorkManager`](https://developer.android.com/topic/libraries/architecture/workmanager) API 允许为任务指定最小执行间隔（至少 15 分钟）。在满足指定条件的前提下，任务会在该间隔过去之后的某个时间执行。

### iOS&ensp;<PlatformTags platforms={['ios']} />

在 iOS 上，[`BGTaskScheduler`](https://developer.apple.com/documentation/backgroundtasks/bgtaskscheduler) API 会决定启动后台任务的最佳时间。系统会综合考虑电池电量、网络可用性以及用户使用模式来确定任务何时运行。你仍然可以为任务指定最小执行间隔，但系统可能会选择在稍后执行该任务。

## 已知限制

### iOS&ensp;<PlatformTags platforms={['ios']} />

iOS 模拟器不提供 [`后台任务`](https://developer.apple.com/documentation/backgroundtasks) API。它仅在物理设备上运行时可用。

## 安装

<APIInstallSection />

## app 配置中的设置&ensp;<PlatformTags platforms={['ios']} />

为了能够在 iOS 上运行后台任务，你需要在应用的 **Info.plist** 文件中添加以下内容：

- 将 `processing` 值添加到 `UIBackgroundModes` 数组中。这样可以启用后台处理能力。
- 使用 `com.expo.modules.backgroundtask.processing` 标识符添加 `BGTaskSchedulerPermittedIdentifiers` 数组。这将注册允许的后台任务标识符。

**如果你正在使用 [CNG](/workflow/continuous-native-generation/)**，所需的 `UIBackgroundModes` 和 `BGTaskSchedulerPermittedIdentifiers` 配置都会在 prebuild 过程中自动应用。
...
// 注册并创建任务，以便即使后台任务界面
//（本示例后面定义的一个 React 组件）不可见时也能使用。
// 注意：这需要在全局作用域中调用，而不是在 React 组件中。
TaskManager.defineTask(BACKGROUND_TASK_IDENTIFIER, async () => {
  try {
    const now = Date.now();
    console.log(`在以下日期收到了后台任务调用：${new Date(now).toISOString()}`);
  } catch (error) {
...
// 2. 在应用的某个时刻通过提供相同的名称来注册任务
// 注意：这不需要在全局作用域中，可以在你的 React 组件中使用！
...
要排查或调试 Android 上后台任务的问题，请使用 Android SDK 中附带的 `adb` 工具检查已调度任务，并将 `<package-name>` 替换为应用配置中的 [Android 包名](/versions/latest/config/app/#package)：

<Terminal
  cmd={['$ adb shell dumpsys jobscheduler | grep -A 40 -m 1 -E "JOB #.* <package-name>"']}
/>

该命令的输出将显示应用的已调度任务，包括它们的状态、约束以及其他信息。查找 `JOB` 行以在输出中找到作业 ID 和其他详细信息：
