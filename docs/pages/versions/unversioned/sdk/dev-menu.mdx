...
`expo-dev-menu` 可作为任何 Expo 项目中的 **独立库** 使用。对于不需要完整 [`expo-dev-client`](/versions/latest/sdk/dev-client/) 启动器界面的 [brownfield 应用](/versions/latest/sdk/brownfield/) 来说，它尤其有用。
...
> **注意：** 之后对 `registerDevMenuItems` 的调用将覆盖所有先前的条目。

## 与 expo-dev-client 一起使用

如果你正在使用 [开发构建](/develop/development-builds/introduction/)，请改为安装 `expo-dev-client`。它包含了 `expo-dev-menu`，以及额外的开发工具：
