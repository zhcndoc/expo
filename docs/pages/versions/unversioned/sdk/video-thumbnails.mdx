...
> **warning** **[已弃用](/more/release-statuses/#deprecated):** Video Thumbnails 库已被弃用，改为使用 [`expo-video`](video.mdx) 中的 [`generateThumbnailsAsync`](video.mdx#generatethumbnailsasynctimes-options)。`expo-video-thumbnails` 不再接收补丁更新，并将在 SDK 56 中移除。
