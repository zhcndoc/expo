---
title: 选择器
description: 与 @react-native-picker/picker 兼容的选择器。
...
一个与 `@react-native-picker/picker` API 兼容的 `Picker` 组件。它在 iOS 上使用 SwiftUI 的轮式 `Picker`，在 Android 上使用 Material 3 的 `ExposedDropdownMenuBox`，在 web 上使用原生 `<select>` 元素。

在底层，该组件封装了平台特定的 `@expo/ui` 原语：

- **Android**: [Jetpack Compose ExposedDropdownMenuBox](../jetpack-compose/exposeddropdownmenubox)
- **iOS**: [SwiftUI Picker](../swift-ui/picker) 并使用 `pickerStyle('wheel')`

如果你需要更低层级的控制，请直接使用这些原语。
...
- 在 `Picker.Item` 上，`style` 属性仅适用于 `color`、`backgroundColor`、`fontFamily` 和 `fontSize`。顶层的 `color` 和 `fontFamily` 属性仍然受支持，并作为对应 `style` 值的别名。
- `Picker.Item` 上的 `enabled` 仅适用于 Android。
- `ref` 的 `focus()` 和 `blur()` 方法仅在 Android 上生效（打开/关闭下拉菜单）。在 iOS 上，轮式选择器始终可见。
...
向 `Picker.Item` 传递一个 `style`，可以分别控制每一项的 `color`、`backgroundColor`、`fontFamily` 和 `fontSize`，并可使用 `enabled={false}` 在 Android 上禁用特定项。

`fontFamily` 在 iOS 上接受 iOS 字体名称（例如 `'Menlo'`），在 Android 上接受 Compose 通用字体族（`'monospace'`、`'serif'`、`'sansSerif'`、`'cursive'`）或使用 [`expo-font`](../../font) 加载的字体。
...
使用 ref 可以在 Android 上以编程方式打开和关闭下拉菜单。在 iOS 上，这些方法没有效果，因为轮式选择器始终可见。
