...
通用的 `Picker` 独立于 [`@expo/ui/community/picker`](../drop-in-replacements/picker)，后者仍然是 `@react-native-picker/picker` 的兼容适配层。新代码建议优先使用这个通用 `Picker`，除非你明确需要 RN-Picker 的 API 表面。
