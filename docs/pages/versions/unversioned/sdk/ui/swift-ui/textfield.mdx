...
> **信息** 有关跨平台用法，请参见通用的 [`TextInput`](../universal/textinput) —— 它会针对每个平台渲染相应的原生组件。
...
将一个 [`useNativeState`](usenativestate) 可观察对象绑定到 `text`。文本框会自行跟踪用户输入，你可以从 `textState.value` 读取当前值。
...
传入一个 `onTextChange` worklet，用于转换或校验输入，并将结果写回 [`useNativeState`](usenativestate) 可观察状态。下面的示例会在输入时将文本转换为大写。
...
当 `onTextChange` 使用 `'worklet'` 指令标记时，它会在 UI 线程上同步运行，因此在回调中对 [`useNativeState`](usenativestate) 可观察对象的写入会在下一帧之前生效。输入文本和遮罩后的文本之间不会出现闪烁。下面的示例会在用户输入时对电话号码进行遮罩，并从 worklet 中同时写入 `text` 和 `selection`，以保持光标位于格式化值的末尾。

> **注意:** Worklet 需要安装 [`react-native-reanimated`](https://docs.swmansion.com/react-native-reanimated/) 和 [`react-native-worklets`](https://docs.swmansion.com/react-native-worklets/)。`selection` 属性需要 iOS 18.0+ / tvOS 18.0+。在较旧版本上，worklet 仍然可以更新文本，但无法进行光标定位。
