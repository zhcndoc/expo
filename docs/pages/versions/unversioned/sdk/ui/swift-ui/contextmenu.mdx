...
<ContentSpotlight
  ...
  alt="长按后的磁贴以放大的预览形式显示，下方有“分享”、“收藏”、“删除”菜单"
/>
...
在菜单 `Button` 上使用 [`disabled(true)`](modifiers/#disableddisabled) 修饰符，可将其渲染为灰显且不可交互。
...
`ContextMenu.Items` 中的 SwiftUI [`Toggle`](toggle) 在 `isOn` 为 `true` 时，会自动渲染为带左侧 SF Symbol 和右侧勾选标记的行。
...
export default function CheckmarkContextMenuItemExample() {
  ...
  return (
    <Host matchContents>
      <ContextMenu>
        <ContextMenu.Items>
          <Toggle isOn={pinned} label="置顶" systemImage="pin" onIsOnChange={setPinned} />
        </ContextMenu.Items>
        <ContextMenu.Trigger>
          <Text>长按我</Text>
        </ContextMenu.Trigger>
