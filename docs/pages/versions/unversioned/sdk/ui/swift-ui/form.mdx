...
Expo UI Form 与官方 SwiftUI [Form API](https://developer.apple.com/documentation/swiftui/form) 保持一致。它提供了一个容器，用于分组数据输入所使用的控件，例如设置页面或检查面板。
...
<ContentSpotlight
  ...
  alt="带有两个分组 Section 的 Toggle 表单以及底部说明文字"
/>
...
使用 [`Section`](section) 组件在表单内对相关控件进行分组。
...
使用 [`scrollDisabled`](modifiers/#scrolldisableddisabled) 修饰器来阻止表单滚动。
