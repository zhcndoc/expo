...
> **info** 有关跨平台用法，请参见通用的 [`Checkbox`](../universal/checkbox) — 它会为每个平台渲染相应的原生组件。
...
将 `toggleable` 修饰符应用到每个 `Row`，使整行（复选框 + 标签）都可点击，并具备正确的可访问性语义。在行上使用 `toggleable` 时，请从复选框本身省略 `onCheckedChange`/`onClick`，以避免重复处理。
