...
> **信息** 有关跨平台用法，请参见通用的 [`Slider`](../universal/slider) —— 它会根据平台渲染相应的原生组件。

Expo UI Slider 与官方的 Jetpack Compose [Slider API](https://developer.android.com/develop/ui/compose/components/slider) 保持一致，并允许从有界范围中选择数值。
...
同时使用 `Slider.Thumb` 和 `Slider.Track` 插槽即可实现完全自定义的滑块外观。
