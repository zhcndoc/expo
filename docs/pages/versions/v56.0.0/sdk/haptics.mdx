---
title: Haptics
description: 一个库，可在 Android 上访问系统的振动效果、在 iOS 上访问触觉引擎、以及在 Web 上访问 Web Vibration API。
...
在 iOS 上，如果用户设备满足以下任一条件，Taptic engine 将不会起作用：

- 已启用低电量模式。可通过 [`expo-battery`](./battery/) 检测。
- 用户在设置中禁用了 Taptic Engine。
- iOS 相机处于活动状态（以防止不稳定）。
- iOS 语音听写处于活动状态（以免干扰麦克风输入）。
...
在 Android 上，该库需要权限来控制设备振动。`VIBRATE` 权限会自动添加。
