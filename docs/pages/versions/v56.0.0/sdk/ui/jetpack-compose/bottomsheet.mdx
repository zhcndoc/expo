---
title: 模态底部弹出层
description: 一个用于从屏幕底部展示内容的 Jetpack Compose ModalBottomSheet 组件。
...
> **info** 有关跨平台用法，请参阅通用的 [`BottomSheet`](../universal/bottomsheet) — 它会针对不同平台渲染相应的原生组件。
...
当 `initialFullyExpanded` 为 `true` 时，抽屉在首次组合时会直接以完全展开状态打开，同时仍然保留可达的部分展开状态。与 `skipPartiallyExpanded` 不同，用户仍然可以向下拖动到部分展开状态。`partialExpand()` 方法也仍然可用。
    ...
    <Host matchContents>
      ...
      {visible && (
        <ModalBottomSheet
          ...
          <Column verticalArrangement={{ spacedBy: 12 }} modifiers={[paddingAll(24)]}>
            <Text>此抽屉已完全展开打开。</Text>
            <Text>你仍然可以将其向下拖动到部分展开状态。</Text>
...
使用 `RNHostView` 在抽屉中嵌套可滚动的 React Native 列表，例如 `FlatList`、`ScrollView`，或高性能列表如 [FlashList](https://shopify.github.io/flash-list/) 或 [Legend List](https://github.com/LegendApp/legend-list)。在可滚动组件上设置 `nestedScrollEnabled`，使其优先滚动自身内容。到达顶部边缘后，剩余的拖动会移动抽屉。不设置 `nestedScrollEnabled` 时，列表会吞掉手势，抽屉将保持不动。
