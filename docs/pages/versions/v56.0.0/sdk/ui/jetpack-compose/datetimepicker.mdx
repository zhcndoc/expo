---
title: 日期时间选择器
description: 一个用于选择日期和时间的 Jetpack Compose 日期时间选择器组件。
...
> **注意：** 日期变体会渲染 Material 的日历网格和输入字段，这两者都会在内部水平滚动。父级 `Host` 必须在水平方向提供有限宽度，请将 `matchContents={{ vertical: true }}` 与 `style={{ width: '100%' }}` 一起使用（或任何有限宽度）。有关详细信息，请参阅 [Host 参考中的匹配内容](host/#match-contents)。
