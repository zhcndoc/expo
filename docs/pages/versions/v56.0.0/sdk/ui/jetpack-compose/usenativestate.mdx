...
> **注意：** 在使用 [React Compiler](https://react.dev/learn/react-compiler) 时，你应避免直接访问和修改 `value` 属性。相反，请使用符合 React Compiler 标准的 [`get`](#observablestate) 和 [`set`](#observablestate) 方法。
...
> **注意：** 使用 worklet 需要在你的项目中安装 [`react-native-worklets`](https://docs.swmansion.com/react-native-worklets/)。`useNativeState` 本身不依赖它即可工作，但下面展示的同步 UI 线程更新依赖 worklet 运行时。
