...
<ContentSpotlight
  ...
  alt="使用 Material 3 Icon 组件渲染的 Wi‑Fi、蓝牙、邮件和个人图标"
/>
...
`@expo/material-symbols` 默认提供 **outlined** 样式及默认轴。当你需要不同样式（`rounded`、`sharp`）、filled 变体，或自定义 weight、grade、optical size 时，请使用其 CLI 直接从 Google Fonts 获取特定的可绘制对象并放入你的项目中。
