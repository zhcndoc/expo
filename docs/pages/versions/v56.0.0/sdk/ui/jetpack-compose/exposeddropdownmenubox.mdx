...
Expo UI `ExposedDropdownMenuBox` 与官方 Jetpack Compose 的 [`ExposedDropdownMenuBox`](https://kotlinlang.org/api/compose-multiplatform/material3/androidx.compose.material3/-exposed-dropdown-menu-box.html) 保持一致。请在锚点内容（通常是只读的 `TextField`）上使用 `menuAnchor()` 修饰符，并使用 `ExposedDropdownMenu` 来包裹 `DropdownMenuItem` 子项。
