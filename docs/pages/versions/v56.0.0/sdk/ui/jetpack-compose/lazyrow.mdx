...
> **info** `LazyRow` 目前还不是真正的懒加载：原生端只会组合可见项，但 React 仍会预先创建每个子项，因此大型列表在挂载时可能会很慢。我们正在改进这一点。对于大型列表，我们建议使用 [FlashList](https://shopify.github.io/flash-list/) 或 [Legend List](https://github.com/LegendApp/legend-list)。
...
const items = Array.from({ length: 100 }, (_, i) => `项 ${i + 1}`);
