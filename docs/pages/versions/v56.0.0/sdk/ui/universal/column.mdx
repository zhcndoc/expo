...
一种垂直布局容器，将其子项从上到下排列。在 iOS 上委托给 SwiftUI 的 [`VStack`](../swift-ui/vstack)，在 Android 上委托给 Jetpack Compose 的 [`Column`](../jetpack-compose/column)，在 web 上委托给 flex `View`。
...
使用 [`alignment`](#alignment) 沿交叉（水平）轴定位子项。
