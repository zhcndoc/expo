---
title: DisclosureGroup
description: 一个用于展示可展开内容的 SwiftUI DisclosureGroup 组件。
sourceCodeUrl: 'https://github.com/expo/expo/tree/sdk-56/packages/expo-ui'
packageName: '@expo/ui'
platforms: ['ios', 'expo-go']
---

import APISection from '~/components/plugins/APISection';
import { APIInstallSection } from '~/components/plugins/InstallSection';
import { ContentSpotlight } from '~/ui/components/ContentSpotlight';

Expo UI DisclosureGroup 与官方 SwiftUI [DisclosureGroup API](https://developer.apple.com/documentation/swiftui/disclosuregroup) 保持一致，并显示一个披露指示器，用于显示或隐藏内容。

<ContentSpotlight
  variant="component"
  aspect="landscape"
  src="/static/images/expo-ui/disclosuregroup/ios-light.webp"
  darkSrc="/static/images/expo-ui/disclosuregroup/ios-dark.webp"
  alt="Form 内展开的 DisclosureGroup"
/>

## 安装

<APIInstallSection />

## 使用

### 基础折叠组

`DisclosureGroup` 最常用于 [`Form`](form) 内，因此它会带有带有 chevron 指示器的标准 iOS 列表样式。

```tsx BasicDisclosureGroupExample.tsx
import { useState } from 'react';
import { DisclosureGroup, Form, Host, Section, Text } from '@expo/ui/swift-ui';

export default function BasicDisclosureGroupExample() {
  const [isExpanded, setIsExpanded] = useState(true);
  return (
    <Host style={{ flex: 1 }}>
      <Form>
        <Section>
          <DisclosureGroup
            label="高级设置"
            isExpanded={isExpanded}
            onIsExpandedChange={setIsExpanded}>
            <Text>自动更新应用</Text>
            <Text>应用下载</Text>
            <Text>卸载未使用的应用</Text>
          </DisclosureGroup>
        </Section>
      </Form>
    </Host>
  );
}
```

### 初始展开

将 `isExpanded` 初始设为 `true`，即可默认显示内容。

```tsx InitiallyExpandedExample.tsx
import { useState } from 'react';
import { Host, DisclosureGroup, Text } from '@expo/ui/swift-ui';

export default function InitiallyExpandedExample() {
  const [isExpanded, setIsExpanded] = useState(true);

  return (
    <Host matchContents>
      <DisclosureGroup label="详情" isExpanded={isExpanded} onIsExpandedChange={setIsExpanded}>
        <Text>此内容默认可见。</Text>
      </DisclosureGroup>
    </Host>
  );
}
```

## API

```tsx
import { DisclosureGroup } from '@expo/ui/swift-ui';
```

<APISection packageName="expo-ui/swift-ui/disclosuregroup" apiName="DisclosureGroup" />
