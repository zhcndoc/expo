---
title: 滑块
description: 一个用于从范围中选择数值的 SwiftUI Slider 组件。
...
> **info** 有关跨平台用法，请参见通用的 [`Slider`](../universal/slider) — 它会根据平台渲染相应的原生组件。

Expo UI Slider 与官方 SwiftUI [Slider API](https://developer.apple.com/documentation/swiftui/slider) 保持一致，允许从有界范围中选择数值。
...
> **注意：** `Slider` 是一个可变宽度组件，它会扩展以填充可用的水平空间，并且没有内在宽度。当在 `Host` 上使用 `matchContents` 时，你应该在 `Slider` 上应用 [`frame`](modifiers/#frameparams) 修饰符，为其提供明确的宽度。或者，也可以通过 `style` 为 `Host` 指定明确的尺寸（例如，`style={{ width: 300 }}` 或 `style={{ flex: 1 }}`），或者将 `Slider` 放在像 `Form` 这样提供宽度约束的 SwiftUI 容器中。
