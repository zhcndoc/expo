...
> **info** `LazyHStack` 目前还不是真正的懒加载：原生端只会构建可见项，但 React 仍会一次性创建所有子元素，因此大型列表在挂载时可能会很慢。我们正在改进这一点。对于大型列表，我们推荐使用 [FlashList](https://shopify.github.io/flash-list/) 或 [Legend List](https://github.com/LegendApp/legend-list)。
