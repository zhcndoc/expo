...
> **info** 有关跨平台用法，请参阅通用的 [`Row`](../universal/row) — 它会根据平台渲染相应的原生组件。

Expo UI HStack 与官方 SwiftUI [HStack API](https://developer.apple.com/documentation/swiftui/hstack) 保持一致，并将其子元素水平排列。
...
<ContentSpotlight
  ...
  alt="在 HStack 中水平排列的四个带编号的彩色圆角方块"
/>
...
`alignment` 属性控制子元素的垂直对齐方式。可选项有：`top`、`center`、`bottom`、`firstTextBaseline` 和 `lastTextBaseline`。
