...
> **info** 关于跨平台使用，请参见通用的 [`Switch`](../universal/switch) —— 它会根据平台渲染相应的原生组件。
...
使用 `toggleStyle` 修饰符来更改切换控件的外观。可用样式有：`automatic`、`switch` 和 `button`。
...
使用 [`labelsHidden`](modifiers/#labelshidden) 修饰符隐藏标签，同时保留其可访问性。
