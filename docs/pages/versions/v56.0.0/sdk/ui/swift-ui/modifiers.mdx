---
title: 修饰器
description: 用于自定义组件外观和行为的 SwiftUI 视图修饰器。
...
修饰符通过带数组语法的 `modifiers` 属性应用到组件上。你可以组合多个修饰符来创建复杂的样式和行为。
...
> **info** 你还可以创建适用于任何 Expo UI 组件的自定义修饰符。详情请参阅 [使用 SwiftUI 扩展](/guides/expo-ui-swift-ui/extending/) 指南。
