...
- 在 `Picker.Item` 上，`style` 属性只应用于 `color`、`backgroundColor`、`fontFamily` 和 `fontSize`。顶层的 `color` 和 `fontFamily` 属性仍然受支持，它们只是对应 `style` 值的别名。
- `Picker.Item` 上的 `enabled` 仅适用于 Android。
- `ref` 的 `focus()` 和 `blur()` 方法仅在 Android 上生效（打开/关闭下拉菜单）。在 iOS 上，轮盘式 picker 始终可见。
