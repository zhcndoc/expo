...
`expo-brownfield` 包提供了一个[配置插件](/config-plugins/introduction/)，可在使用 [Continuous Native Generation (CNG)](/workflow/continuous-native-generation/) 时用于配置 brownfield 集成。此插件允许你自定义 Expo 项目的打包方式，以及它如何集成到现有的原生应用中。
  ...
  properties={[
    {
      ...
      platform: 'ios',
      description:
        '用于 brownfield 集成的 Xcode 目标名称。这会在你的 Xcode 项目中创建一个单独的目标来承载 React Native 代码。',
    ...
    },
    {
      ...
      platform: 'android',
      description:
        '生成的 Android 库的 Maven group ID。发布库到 Maven 仓库时会用到它。',
    ...
    },
    {
      ...
      platform: 'android',
      description:
        '生成的 Android 库的版本字符串。发布到 Maven 仓库时会用到它。',
      default: '"1.0.0"',
    },
    {
