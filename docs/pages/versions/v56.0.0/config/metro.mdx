...
Expo CLI can load environment variables from **.env** files. To learn more about how to use environment variables in Expo CLI, see the [Environment Variables guide](/guides/environment-variables/).

EAS CLI uses a different environment variable mechanism, unless it invokes Expo CLI during compilation and bundling. To learn more about [environment variables in EAS](/eas/environment-variables/).

If you are migrating an older project, you should ignore local env files by adding the following to your **.gitignore**:
...
### Disable dotenv files

You can completely disable Expo CLI from loading dotenv files by enabling the `EXPO_NO_DOTENV` environment variable before calling any Expo CLI command.

<Terminal
  cmd={[
    '# All users can run cross-env, then run an Expo CLI command',
    '$ npx cross-env EXPO_NO_DOTENV=1 expo start',
    '# Or, macOS and Linux users can define the environment variable first, then run npx, then run an Expo CLI command',
    '$ EXPO_NO_DOTENV=1 npx expo start',
  ]}
/>

### Disable client environment variables prefixed with `EXPO_PUBLIC_`

Environment variables prefixed with `EXPO_PUBLIC_` are exposed to the app at build time. For example, `EXPO_PUBLIC_API_KEY` will be accessible via `process.env.EXPO_PUBLIC_API_KEY`.

You can disable client environment variable inlining by using the `EXPO_NO_CLIENT_ENV_VARS=1` environment variable, which must be defined before running any bundling.
...
Metro 在启动时会预先遍历 `watchFolders` 中列出的每个目录。在 [monorepo](/guides/monorepos) 中，这通常意味着会遍历每个工作区。启用按需文件系统访问后，文件映射可以在解析器请求时，按需读取 `watchFolders` 之外的文件。这样你就可以安全地减少 `watchFolders` 条目的数量，在不破坏访问项目根目录之外导入的前提下，以减少文件监听覆盖范围来换取更快的启动时间。

按需遍历还允许将符号链接解析到 monorepo 根目录之外，这为 Bun 和 pnpm 等包管理器增加了对 [Global Virtual Store 的支持](https://pnpm.io/global-virtual-store)。

此功能默认启用，你可以在 [应用配置](/versions/latest/config/app/) 中将 `experiments.onDemandFilesystem` 设置为 `false` 来禁用它：
...
Expo CLI will automatically split the web bundle into multiple chunks in production based on asynchronous imports. This feature requires installing and importing `@expo/metro-runtime` somewhere in the entry bundle (available by default in Expo Router).

Shared dependencies in asynchronous bundles will be merged into a single chunk to reduce the number of requests. For example, if you have two asynchronous bundles that import `lodash`, the library will be merged into one initial chunk.

The chunk splitting heuristics cannot be customized. For example:
...
When you run `npx expo export -p web`, the bundle will be split into multiple files, and the entry bundle will be added to the main HTML file. `@expo/metro-runtime` adds the runtime code used to load and execute asynchronous bundles.
...
默认情况下，React Native 仅支持将根目录下的 `index.js` 文件作为入口文件（或平台特定变体，如 `index.ios.js`）。Expo 项目允许使用任意入口文件，但这在现有 React Native 项目中需要额外配置。
