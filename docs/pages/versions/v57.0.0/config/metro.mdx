...
在 [自定义 Metro 指南](/guides/customizing-metro/) 中查看更多关于 **metro.config.js** 的信息。

## 环境变量

Expo CLI 可以从 **.env** 文件中加载环境变量。了解更多关于如何在 Expo CLI 中使用环境变量的信息，请参阅 [环境变量指南](/guides/environment-variables/)。

EAS CLI 使用一种不同的环境变量机制，除非它在编译和打包时调用 Expo CLI。了解更多关于 [EAS 中的环境变量](/eas/environment-variables/)。
...
通过在调用任何 Expo CLI 命令之前启用 `EXPO_NO_DOTENV` 环境变量，可以在 Expo CLI 中完全禁用 Dotenv 文件加载。

<Terminal
  cmd={[
    '# 所有用户都可以运行 cross-env，然后跟上 Expo CLI 命令',
    '$ npx cross-env EXPO_NO_DOTENV=1 expo start',
    '# 或者，macOS 和 Linux 用户可以先定义环境变量，然后运行 npx，再跟上 Expo CLI 命令',
...
带有 `EXPO_PUBLIC_` 前缀的环境变量会在构建时暴露给应用。例如，`EXPO_PUBLIC_API_KEY` 将可作为 `process.env.EXPO_PUBLIC_API_KEY` 使用。
...
> **信息** CSS 支持正在开发中，目前仅适用于 web。

Expo 支持在你的项目中使用 CSS。你可以从任何组件中导入 CSS 文件。CSS Modules 也受支持。
...
> **警告** 全局样式仅适用于 web，使用它会导致你的应用在原生平台上显示不一致。

你可以从任何组件中导入 CSS 文件。该 CSS 将应用于整页。
...
export default function App() {
  return (
    <>
      ...
      {/* 在 React Native for web 中，使用以下语法将类名附加到 `style`。 */}
      <View
        style={{
          $$css: true,
          _: 'container',
...
你也可以导入库中捆绑的样式表，就像导入任何 node 模块一样：
...
> **警告** 使用 Expo Router 时，务必在根 **\_layout.tsx** 中导入全局 CSS。Expo Router 会从根布局开始遍历依赖图。在嵌套布局中导入 CSS 会导致 **node_modules** 中的 CSS 先于你的自定义样式加载，这可能会破坏你预期的样式顺序。

### CSS Modules

> **警告** 原生平台的 CSS Modules 正在开发中，目前仅适用于 web。

CSS Modules 是一种将 CSS 作用域限定到特定组件的方法。这对于避免命名冲突以及确保样式仅应用于目标组件非常有用。

在 Expo 中，通过创建一个扩展名为 `.module.css` 的文件来定义 CSS Modules。该文件可以从任何组件中导入。导出的值是一个对象，其中类名作为键，web 专用的作用域名称作为值。可以使用导入的 `unstable_styles` 来访问对 `react-native-web` 安全的样式。
...
切换扩展名，例如 `App.ios.module.css`，将不起作用，并会导致生成一个名为 `App.ios.module` 的通用模块。
...
export default function Page() {
  return (
    <>
      <Text
        style={{
          // 这是 react-native-web 类名的应用方式
          $$css: true,
          _: styles.text,
        }}>
        你好，世界
      </Text>
      <Text style={unstable_styles.text}>你好，世界</Text>
      {/* 仅适用于 web： */}
      <p className={styles.text}>你好，世界</p>
    </>
  );
}
```
...
- CSS Modules 底层使用 [lightningcss](https://github.com/parcel-bundler/lightningcss)，请查看 [issues](https://github.com/parcel-bundler/lightningcss/issues) 了解不受支持的功能。

### PostCSS

你可以通过在项目根目录添加 `postcss.config.json` 文件来自定义 [PostCSS](https://github.com/postcss/postcss)。该文件应导出一个返回 PostCSS 配置对象的函数。例如：
...
Expo CLI 会自动处理 CSS 厂商前缀，并内置支持 [browserslist](https://browsersl.ist/)。请避免添加 `autoprefixer`，因为这会重复实现相同功能并减慢打包速度。
...
- 安装 `sass` 后，不带扩展名的模块将按以下顺序解析：`scss`、`sass`、`css`。
- 仅在 `sass` 文件中使用预期的语法。
- 目前不支持从 scss/sass 文件内部导入其他文件。

### Tailwind

> **信息** 标准 Tailwind CSS 仅支持 web 平台。若需跨平台支持，请使用诸如 [NativeWind](https://www.nativewind.dev/) 或 [Uniwind](https://uniwind.dev/) 之类的库，它们允许使用 Tailwind CSS 创建带样式的 React Native 组件。
...
Expo CLI 扩展了默认的 Metro 解析器，以添加诸如 Web、Server 和 tsconfig 别名支持等功能。你也可以通过链式调用 `config.resolver.resolveRequest` 函数，来自定义 Metro 的默认解析行为。
...
与传统的打包工具不同，Metro 在所有平台之间共享同一个解析器函数。因此，你可以使用 `context` 对象在每次请求时动态修改解析设置。

### 模块模拟

如果你希望某个模块在特定平台上为空，可以从解析器中返回一个 `type: 'empty'` 对象。下面的示例会让 web 端的 `lodash` 变为空：
...
这种技术等同于在 Webpack 或 Vite 中使用空的 externals，但额外的好处是可以针对特定平台进行处理。

### 虚拟模块

Metro 目前不支持虚拟模块。你可以使用的一种方法是：在 `node_modules/.cache/...` 目录中创建一个模块，并将解析重定向到该文件。
...
这可以用于通过自定义导入来模拟 `externals`。例如，如果你想将 `require('expo')` 重定向到类似 `SystemJS.require('expo')` 的自定义内容，你可以创建一个导出 `SystemJS.require('expo')` 的虚拟模块，并将 `expo` 的解析重定向到该文件。

## 自定义转换

> 转换在 Metro 中被高度缓存。如果你更新了某些内容，请使用 `--clear` 标志来查看更新。例如，`npx expo start --clear`。

Metro 在文件转换方面没有很强大的插件系统，因此建议改用 [**babel.config.js**](../config/babel/) 和 caller 对象来自定义转换。

```js babel.config.js
module.exports = function (api) {
  // 获取 Expo CLI 正在为其执行转换的平台。
...
如果 caller 没有 `engine`、`platform`、`bundler` 等信息，那么请确保你为转换器使用的是 `@expo/metro-config/babel-transformer`。如果你使用的是自定义转换器，那么它可能需要扩展 Expo 转换器。

如果可能，请始终尝试在 resolver 中实现自定义逻辑，因为缓存要简单得多，也更容易理解。例如，如果你需要重映射一个 import，使用 resolver 解析到静态文件比解析所有可能的导入方式再用转换器重映射它们更简单，也更快。

始终使用 `babel-preset-expo` 作为默认的 Babel 预设，这可以确保转换始终与 Expo 运行时兼容。`babel-preset-expo` 会在内部使用所有 caller 输入，以针对给定的平台、引擎和环境进行优化。

## 按需文件系统

Expo 使用 `metro-file-map` 的一个分支（[`@expo/metro-file-map`](https://github.com/expo/expo/tree/main/packages/@expo/metro-file-map)）来发现、监听并按需解析源文件。

Metro 在启动时会预先遍历 `watchFolders` 中列出的每个目录。在 [monorepo](/guides/monorepos) 中，这通常意味着会遍历每个工作区。启用按需文件系统访问后，文件映射可以在解析器请求时，按需读取 `watchFolders` 之外的文件。这样你就可以安全地减少 `watchFolders` 条目的数量，在不破坏指向项目根目录之外的导入的前提下，以文件监听覆盖范围换取更快的启动时间。

按需遍历还允许解析位于 monorepo 根目录之外的符号链接，这为 Bun 和 pnpm 等包管理器增加了 [全局虚拟存储支持](https://pnpm.io/global-virtual-store)。

此功能默认启用，可以通过在你的 [应用配置](/versions/latest/config/app/) 中将 `experiments.onDemandFilesystem` 设置为 `false` 来禁用：
...
在为服务器环境进行打包时，Expo 的 Metro 配置会根据当前的 Node.js 版本自动支持将 Node.js 内置模块（`fs`、`path`、`node:crypto` 等）外部化。如果 CLI 正在为浏览器环境打包，那么内置模块会先检查该模块是否已在本地安装，然后回退到一个空的 shim。例如，如果你安装了 `path` 以便在浏览器中使用，那么它就可以被使用；否则，该模块将自动被跳过。

## 环境设置

> **info** 这些环境变量不会在测试环境中定义。

Expo 的 Metro 配置会注入构建设置，这些设置可以通过环境变量在客户端 bundle 中使用。所有变量都会被内联，且不能动态使用。例如，`process.env["EXPO_BASE_URL"]` 将无法工作。

- `process.env.EXPO_BASE_URL` 暴露了在 `experiments.baseUrl` 中定义的基础 URL。Expo Router 会使用它，以便在部署时遵循生产环境的基础 URL。

## Bundle 分割

Expo CLI 会在生产环境中根据异步导入自动将 web bundle 拆分为多个 chunk。此功能要求在入口 bundle 的某处安装并导入 `@expo/metro-runtime`（在 Expo Router 中默认可用）。

异步 bundle 的共享依赖会合并到单个 chunk 中，以减少请求数量。例如，如果你有两个异步 bundle 都导入了 `lodash`，那么该库会合并到一个初始 chunk 中。

chunk 分割的启发式规则无法自定义。例如：
...
如果一个 bundle 使用外部 source map 导出，那么文件末尾会添加一个 [**Debug ID**](https://sentry.engineering/blog/the-case-for-debug-ids) 注释，同时在 source map 中添加一个匹配的 `debugId`，以便将对应文件关联起来。如果没有导出 source map，或者使用了内联 source map，那么就不会添加这个注释。
...
`debugId` 是 bundle 内容的确定性哈希值，但不包含外部 bundle 拆分引用。它与用于创建 chunk 文件名的值相同，只是格式化为 UUID。例如，`431b98e2-c997-4975-a3d9-2987710abd44`。
...
你可以通过环境变量 `EXPO_USE_METRO_REQUIRE=1` 选择启用自定义的 Metro `require` 实现。该运行时具有以下特性：

- 人类可读的字符串模块 ID，使缺失模块错误更容易追踪。
- 确定性的 ID，在不同运行和不同模块之间保持一致（这是开发环境中 React Server Components 所必需的）。
- 已移除对旧版 RAM bundles 的支持。
...
```js
// 手动确保 `./my-module.js` 被包含在相对于该模块的正确位置。
...
现在，随着现代 ES Modules 解析策略的引入，Metro 会先从 `node_modules` 中解析模块，然后匹配不同的 `package.json` 字段，例如 `exports`、[包公开的子路径嵌套映射](https://nodejs.org/api/packages.html#conditional-exports) 和 `main`。
...
```ts worker.ts
self.onmessage = ({ data }) => {
  const result = data * 2; // 示例：将数字乘以 2
...
```ts
// 将避免转换，并直接使用 public 路径。
const worker = new Worker('/worker.js');

// 变量会破坏转换，使其使用字面路径而不是转换后的路径。
...
> 本指南是有版本要求的，在升级/降级 Expo 时需要重新查看。或者，使用 [Expo Prebuild](/more/glossary-of-terms/#prebuild) 进行完全自动化的设置。
