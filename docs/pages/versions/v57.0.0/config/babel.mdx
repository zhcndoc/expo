---
title: babel.config.js
description: Babel 配置文件参考。
---

import { Terminal } from '~/ui/components/Snippet';

Babel 用作 JavaScript 编译器，将现代 JavaScript（ES6+）转换为与移动设备上的 JavaScript 引擎兼容的版本。

使用 `npx create-expo-app` 创建的每个新 Expo 项目都会自动配置 Babel，并使用 [`babel-preset-expo`](https://github.com/expo/expo/tree/main/packages/babel-preset-expo) 作为默认预设。除非你需要自定义 Babel 配置，否则无需创建 **babel.config.js** 文件。

## 创建 babel.config.js

如果你的项目需要自定义 Babel 配置，你需要按照以下步骤在项目中创建 **babel.config.js** 文件：

1. 导航到项目根目录，并在终端中运行以下命令。这将会在项目根目录生成一个 **babel.config.js** 文件。

<Terminal cmd={['$ npx expo customize babel.config.js']} />

2. **babel.config.js** 文件包含以下默认配置：

```js babel.config.js
module.exports = function (api) {
  api.cache(true);
  return {
    presets: ['babel-preset-expo'],
  };
};
```

3. 如果你对 **babel.config.js** 文件进行了更改，你需要重新启动 Metro bundler 以应用更改，并使用 Expo CLI 的 `--clear` 选项来清除 Metro bundler 缓存：

<Terminal cmd={['$ npx expo start --clear']} />

## babel-preset-expo

[`babel-preset-expo`](https://github.com/expo/expo/tree/main/packages/babel-preset-expo) 是 Expo 项目中使用的默认预设。它扩展了默认的 React Native 预设（`@react-native/babel-preset`），并添加了对装饰器、Web 库的 tree-shaking，以及加载字体图标的支持。
