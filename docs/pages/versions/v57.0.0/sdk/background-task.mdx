...
**如果你正在使用 [CNG](/workflow/continuous-native-generation/)**，所需的 `UIBackgroundModes` 和 `BGTaskSchedulerPermittedIdentifiers` 配置都会在预构建过程中自动应用。

<Collapsible summary="在 iOS 上手动配置 Info.plist">

如果你没有使用持续原生生成（[CNG](/workflow/continuous-native-generation/)），那么你需要将以下内容添加到 **Info.plist** 文件中：
