...
`expo-brownfield` 是一个用于向现有原生 Android 和 iOS 应用中添加 React Native 视图的工具包。它提供：

- **内置 API**，用于原生应用和 React Native 应用之间的双向通信与导航
- **配置插件**，用于在你的 Expo 项目中自动设置 brownfield 目标
- **CLI**，用于构建并将产物发布到 Maven 仓库（Android）和 XCFrameworks（iOS）
...
`expo-brownfield` 包提供了一个[配置插件](/config-plugins/introduction/)，可在使用[连续原生生成（CNG）](/workflow/continuous-native-generation/)时用于配置 brownfield 集成。此插件允许你自定义 Expo 项目的打包方式，以及它如何集成到现有的原生应用中。
  ...
  properties={[
    {
      ...
      platform: 'ios',
      description:
        'brownfield 集成的 Xcode target 名称。这用于在你的 Xcode 项目中为 React Native 代码创建一个单独的 target。',
    ...
    },
    {
      ...
      platform: 'ios',
      description:
        'brownfield target 的 bundle 标识符。它应当是唯一的，并且不同于你的主应用 bundle 标识符。',
    ...
    },
    {
      ...
      platform: 'android',
      description:
        '生成的 Android 库的 Maven group ID。将该库发布到 Maven 仓库时会使用此项。',
    ...
    },
    {
      ...
      platform: 'android',
      description:
        '生成的 Android 库的版本字符串。将其发布到 Maven 仓库时会使用此项。',
      default: '"1.0.0"',
    },
    {
...
`expo-brownfield` 库包含一个用于构建并发布到 Maven 仓库（Android）和 XCFrameworks（iOS）的 CLI。
...
构建并发布 brownfield 库及其依赖到 Maven 仓库。
...
构建 brownfield XCFramework，并将 Hermes XCFramework 复制到 artifacts 目录。
...
| `-a, --artifacts`   | artifacts 目录的路径（默认：`./artifacts`）            |
| `-s, --scheme`      | 要构建的 Xcode scheme                                  |
| `-x, --xcworkspace` | Xcode workspace 路径                                  |
| `-p, --package`     | 将 artifacts 作为 Swift Package 交付（可选指定名称）   |
| `--verbose`         | 包含子进程的所有日志                                   |
