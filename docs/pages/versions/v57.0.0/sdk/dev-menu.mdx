...
`expo-dev-menu` 可以在任何 Expo 项目中作为**独立库**使用。它在不需要完整 [`expo-dev-client`](/versions/latest/sdk/dev-client/) 启动器界面的 [brownfield 应用](/versions/latest/sdk/brownfield/) 中尤其有用。
...
- **摇动手势**：摇动你的设备
- **三指长按**：在屏幕上用三根手指长按
- **通过程序**：在你的代码中调用 `DevMenu.openMenu()`
...
> **注意：** 后续对 `registerDevMenuItems` 的调用将覆盖之前的所有条目。

## 与 expo-dev-client 一起使用

如果你正在使用[开发构建](/develop/development-builds/introduction/)，请改为安装 `expo-dev-client`。它包含 `expo-dev-menu` 以及其他开发工具：
...
有关更多信息，请查看 [`expo-dev-client` 参考文档](/versions/latest/sdk/dev-client/)。
