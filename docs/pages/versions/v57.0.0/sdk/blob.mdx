---
title: Blob
description: 一个符合 Web 标准的、适用于 React Native 的 Blob 实现。
...
`expo-blob` 为 React Native 提供了符合 Web 标准的 Blob 实现，具有更出色的性能，并且能在所有平台上一致运行。与从 `react-native` 导出的实现相比，它更加可靠；不同于 `react-native` 中的 Blob，它在 `slice()` 方法和其他 Web API 特性方面存在限制。
