...
<ContentSpotlight
  ...
  alt="iOS 设备上显示的来自 Expo Symbols 的符号集合。"
/>
...
传入一个包含各平台符号名称的对象，即可在所有平台上渲染符号。可在 [Apple SF Symbols 应用](https://developer.apple.com/sf-symbols/) 中浏览可用的 iOS 符号，也可在 [Google Material Symbols](https://fonts.google.com/icons) 中浏览 Android/web 符号。
...
在 iOS 上，直接传入一个权重字符串。在 Android 上，从 `expo-symbols/androidWeights` 导入一个权重对象：
...
可用的权重导入有：`bold`、`semiBold`、`medium`、`regular`、`light`、`extraLight`、`thin`。
