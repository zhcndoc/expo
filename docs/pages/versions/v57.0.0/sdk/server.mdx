...
`expo-server` 是一个面向 Expo Router 的服务端 API 和运行时库。它提供了一些辅助工具，可用于你的 Expo Router API 路由或其他服务端代码中，并包含用于运行 Expo Router 服务端导出的适配器。

## 安装

<APIInstallSection packageName="expo-server" hideBareInstructions />

要在你的项目中使用 `expo-server`，你需要将 Expo Router 项目配置为以 `server` 模式导出。请按照 Expo Router 的 API 路由指南中的说明进行操作：
...
`expo-server` 的运行时 API 只能在服务端代码中使用，并让你访问服务端运行时环境。该运行时 API 暴露的函数可以在请求处理器的异步上下文中调用，并为你提供当前请求的信息，或调度与传入请求并发运行的任务。
...
`expo-server` 提供了适配器，用于在不同环境或不同云提供商的无服务器函数中运行 Expo Router 的服务端导出。通常，每种运行时都需要自己的适配器才能与 `expo-server` 运行时配合工作。在部署到这些提供商之前，最好先熟悉 [`npx expo export`](/more/expo-cli#exporting) 命令的基础知识。
...
要了解如何在不同的第三方服务上托管 API 路由，请按照 Expo Router 的 API Routes 指南中的说明操作：
