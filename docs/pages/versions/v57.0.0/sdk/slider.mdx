...
> **重要** [`@expo/ui` 提供了一个可直接替换的实现](./ui/drop-in-replacements/slider)，用于 `@react-native-community/slider`，在 Android 上由 Jetpack Compose 提供支持，在 iOS 上由 SwiftUI 提供支持。

一个组件库，提供对滑块控件系统 UI 的访问，允许用户通过拖动滑块来在一系列值中进行选择。
