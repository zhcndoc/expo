---
title: RNHostView
description: 一个允许在 Jetpack Compose 中嵌入 React Native 视图的组件。
...
当 React Native 视图放置在像 [`ModalBottomSheet`](bottomsheet)、[`Card`](card)、[`Row`](row)、[`Column`](column) 等 Jetpack Compose 组件内部时，这些布局系统需要进行通信。`RNHostView` 连接了这两者之间的空白：

- **使用 [`matchContents`](#matchcontents)**：shadow node 的大小会被设置为匹配子 React Native 视图的固有大小，从而允许 Jetpack Compose 父组件根据 React Native 内容来确定自身尺寸。
...
function Example() {
  return (
    <Host style={{ flex: 1 }}>
      <Card modifiers={[fillMaxWidth()]}>
        <Column verticalArrangement={{ spacedBy: 12 }} modifiers={[padding(16, 16, 16, 16)]}>
          <Text>带有 flex: 1 子元素的 RN 组件</Text>
...
`RNHostView` 在 [`ModalBottomSheet`](/versions/latest/sdk/ui/jetpack-compose/bottomsheet) 中也能很好地工作，用于显示交互式 React Native 内容。

```tsx ModalBottomSheet 中的 RNHostView
