...
description: 用于显示内联搜索输入的 Jetpack Compose DockedSearchBar 组件。
...
Expo UI DockedSearchBar 与官方 Jetpack Compose [SearchBar API](https://developer.android.com/develop/ui/compose/components/search-bar) 保持一致，并显示一个搜索输入框，该输入框会固定在其父布局中，而不是展开到全屏。
...
<ContentSpotlight
  ...
  alt="带有搜索图标和占位文本的 Material 3 固定搜索栏"
/>
