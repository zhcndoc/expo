...
> **info** 有关跨平台使用，请参阅通用的 [`Checkbox`](../universal/checkbox) — 它会根据平台渲染相应的原生组件。
...
当父复选框需要反映其子项状态时，请使用 `TriStateCheckbox`。它支持三种状态：`'on'`、`'off'` 和 `'indeterminate'`。

将 `toggleable` 修饰符应用到每个 `Row` 上，使整行（复选框 + 标签）都可点击，并保持正确的无障碍语义。对行使用 `toggleable` 时，请省略复选框本身的 `onCheckedChange`/`onClick`，以避免重复处理。
