...
> **信息** 有关跨平台使用，请参阅通用的 [`TextInput`](../universal/textinput) —— 它会针对每个平台渲染相应的原生组件。

Expo UI 提供了三个文本字段组件，与官方 Jetpack Compose [TextField API](https://developer.android.com/develop/ui/compose/text/user-input) 保持一致：`TextField`（填充式）、`OutlinedTextField`（带轮廓边框）和 `BasicTextField`（无样式）。Material 变体 `TextField` 和 `OutlinedTextField` 共享相同的 props，并支持用于标签、占位符、图标、前缀、后缀以及辅助文本的可组合插槽子元素。`BasicTextField` 没有 Material 外观，因此需要你自行提供装饰。

| 类型     | 外观                                                   | 用途                                                                                                       |
| -------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- |
| 填充式   | 带底部指示线的实心背景。               | 遵循 Material3 设计的默认文本输入样式。适用于大多数表单和输入字段。                     |
| 轮廓式   | 带边框轮廓的透明背景。                | 提供清晰视觉边界的替代样式。当填充式字段与背景融为一体时使用。 |
| 基础     | 无容器、指示线或内边距。仅显示可编辑文本。 | 完全自定义样式的输入框。自行设置样式，并通过 `DecorationBox` 添加装饰。                     |
...
将一个 [`useNativeState`](usenativestate) 可观察对象绑定到 `value`。该字段会自行跟踪用户输入，你可以从 `text.value` 读取当前值。此处显示的填充样式是默认的 Material3 文本输入样式。
...
将一个 [`useNativeState`](usenativestate) 可观察对象作为 `value` 传入，并使用 `onValueChange` worklet 在写回之前转换或验证输入。下面的示例会在输入时将文本转换为大写。
...
当你想要一个带边框描边而不是填充背景的文本字段时，请使用 `OutlinedTextField`。
...
`BasicTextField` 是未加样式的 Compose 原语，没有容器、指示器或内边距。你可以使用 [modifiers](modifiers) 自行设置样式，并通过 `DecorationBox` 提供装饰内容，将 `InnerTextField` 放在可编辑文本应当渲染的位置。将占位符内容包裹在 `Placeholder` 中，这样它只会在字段为空时显示，并由字段文本在原生层面切换显示状态。
...
`TextField` 和 `OutlinedTextField` 都支持 7 个可组合插槽，对应 Compose API：`Label`、`Placeholder`、`LeadingIcon`、`TrailingIcon`、`Prefix`、`Suffix` 和 `SupportingText`。
...
使用 `keyboardOptions` 属性来配置键盘类型、首字母大写、自动纠错和 IME 操作。
...
使用 `keyboardActions` 属性来处理 IME 操作按钮的按下事件。触发的回调取决于 `keyboardOptions` 中设置的 `imeAction`。每个回调都会接收当前文本值。
...
使用 ref 可以以命令式方式设置文本、清空字段、更改选择范围或移动焦点。
...
当 `onValueChange` 使用 `'worklet'` 指令标记时，它会在 UI 线程上同步运行，因此在回调中对 [`useNativeState`](usenativestate) 可观察对象的写入会在下一帧之前生效。输入文本与遮罩文本之间不会出现闪烁。下面的示例会在用户输入时遮罩电话号码，并在 worklet 中同时写入 `value` 和 `selection`，以保持光标位于格式化后值的末尾。
