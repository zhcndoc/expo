---
title: 悬浮操作按钮
description: 遵循 Material Design 3 的 Jetpack Compose 悬浮操作按钮组件。
...
Expo UI 提供四种 FloatingActionButton 变体，与 Material Design 3 [`FloatingActionButton`](https://developer.android.com/develop/ui/compose/components/fab) API 相匹配：

- `SmallFloatingActionButton` — 紧凑型 FAB
- `FloatingActionButton` — 标准 FAB（默认大小）
- `LargeFloatingActionButton` — 更大的 FAB
- `ExtendedFloatingActionButton` — 带图标和文本标签的 FAB，支持动画展开/收起
...
<ContentSpotlight
  ...
  alt="小号、标准和大号 Material 3 浮动操作按钮"
/>
...
export default function StandardFABExample() {
  return (
    <Host matchContents>
      <FloatingActionButton onClick={() => console.log('FAB 按下')}>
...
使用带有 `align('bottomEnd')` 的 Compose `Box`，即可将 FAB 放置在可滚动内容之上，并且完全位于 Compose 层内。
