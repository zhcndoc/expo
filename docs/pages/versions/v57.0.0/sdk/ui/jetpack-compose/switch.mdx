---
title: 开关
description: 一个用于切换控件的 Jetpack Compose 开关组件。
...
> **info** 如需跨平台使用，请查看通用的 [`Switch`](../universal/switch) —— 它会根据平台渲染相应的原生组件。
...
<ContentSpotlight
  ...
  alt="两个 Material 3 开关，展示开启和关闭状态"
/>
...
使用 `Switch.ThumbContent` 在拇指内部渲染自定义元素。`Switch.DefaultIconSize` 会提供 M3 的默认图标大小，因此你的内容能够完美适配。
