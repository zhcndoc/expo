...
Expo UI Section 与官方 SwiftUI [Section API](https://developer.apple.com/documentation/swiftui/section) 相匹配，用于在 [`List`](list)、[`Form`](form) 或 [`Picker`](picker) 组件中对相关内容进行分组。
...
<ContentSpotlight
  ...
  alt="表单中的两个 Section，每个都有一个标题和分组的 Toggle"
/>
...
> **注意：** 可折叠部分需要 iOS 17+ 和 tvOS 17+，并且列表必须使用 `sidebar` 样式。可折叠部分不支持页脚。
