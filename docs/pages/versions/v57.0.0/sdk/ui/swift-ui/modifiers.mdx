...
允许你自定义 UI 组件外观和行为的 SwiftUI 视图修饰符。
...
修饰符通过 `modifiers` 属性并使用数组语法应用于组件。你可以组合多个修饰符来创建复杂的样式和行为。
...
> **信息** 你还可以创建适用于任何 Expo UI 组件的自定义修饰符。详情请参阅 [使用 SwiftUI 扩展](/guides/expo-ui-swift-ui/extending/) 指南。
