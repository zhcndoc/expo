...
> **信息** 如需跨平台使用，请参阅通用的 [`List`](../universal/list)——它会根据平台渲染相应的原生组件。

Expo UI List 与官方 SwiftUI [List API](https://developer.apple.com/documentation/swiftui/list) 保持一致，并支持通过 [`listStyle`](modifiers/#liststylestyle) 修饰器、各种行/分组修饰器，以及选择、重新排序和编辑功能进行样式设置。

> **信息** `List` 目前还不会延迟渲染行：React 会预先创建每一行，因此大型列表的挂载速度可能较慢。我们正在努力改进这一点。对于大型列表，我们推荐使用 [FlashList](https://shopify.github.io/flash-list/) 或 [Legend List](https://github.com/LegendApp/legend-list/)。
...
<ContentSpotlight
  ...
  alt="一个 List，包含 Favorites 和 Recents 两个分组，每一行都显示一个彩色 SF Symbol 图标和一个标签"
/>
...
使用带有 `onDelete` 和 `onMove` 属性的 [`List.ForEach`](#listforeach) 组合组件，可启用列表项的选择、删除和重新排序。

- 使用 [`environment`](modifiers/#environmentkey-value) 修饰符来启用编辑模式
- 使用 [`tag`](modifiers/#tagtag) 修饰符来标识项目
- 使用 [`selection`](#selection) 属性来控制选中的项目
- 使用 [`moveDisabled`](modifiers/#movedisableddisabled) 和 [`deleteDisabled`](modifiers/#deletedisableddisabled) 修饰符来对单个项目禁用这些操作
...
使用 [`refreshable`](modifiers/#refreshablehandler) 修饰符来启用下拉刷新功能。
...
使用 [`listRowBackground`](modifiers/#listrowbackgroundcolor)、[`listRowSeparator`](modifiers/#listrowseparatorvisibility-edges) 和 [`listRowInsets`](modifiers/#listrowinsetsparams) 修饰符来自定义单个行。
...
使用 [`scrollDismissesKeyboard`](modifiers/#scrolldismisseskeyboardmode) 修饰符来控制滚动时键盘的关闭方式。
