...
`useNativeState` 返回一个 [`ObservableState`](#observablestate)，它在原生侧映射到 SwiftUI 的 [`ObservableObject`](https://developer.apple.com/documentation/combine/observableobject)，因此对 `.value` 的读取和写入会直接被 SwiftUI 观察到，而不会经过 React 渲染周期。这使你可以从 UI 线程上的 worklet 同步更新原生视图。
...
> **注意：** 使用 worklet 需要在你的项目中安装 [`react-native-worklets`](https://docs.swmansion.com/react-native-worklets/)。`useNativeState` 本身无需它即可工作，但下面展示的同步 UI 线程更新依赖于 worklet 运行时。
