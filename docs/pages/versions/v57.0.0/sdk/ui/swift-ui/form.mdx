...
Expo UI Form 与官方 SwiftUI [Form API](https://developer.apple.com/documentation/swiftui/form) 保持一致。它提供了一个用于分组数据输入控件的容器，例如在设置或检查面板中使用。
...
<ContentSpotlight
  ...
  alt="一个包含两个分组 Section 的 Form，内有若干 Toggle 和一个页脚说明"
/>
...
使用 [`Section`](section) 组件在表单中对相关控件进行分组。
...
使用 [`scrollDisabled`](modifiers/#scrolldisableddisabled) 修饰符来防止表单滚动。
