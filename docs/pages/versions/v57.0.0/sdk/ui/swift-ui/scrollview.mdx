...
> **信息** 如需跨平台使用，请参阅通用的 [`ScrollView`](../universal/scrollview) —— 它会为每个平台渲染相应的原生组件。
...
> **信息** 需要 iOS 17 或更高版本。在较旧版本中，该修饰器不会执行任何操作。
...
> **警告** 对 `state.value` 的写入必须在 UI 运行时中执行。请使用 `react-native-worklets` 中的 `scheduleOnUI` 包裹写入，或从 `'worklet'` 函数内部调用它们。来自 JS 运行时的写入会触发 Main Thread Checker，这是 Xcode 的运行时工具，用于标记在后台线程中进行的 UIKit 调用。
