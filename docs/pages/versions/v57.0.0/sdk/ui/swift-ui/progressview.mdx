...
Expo UI ProgressView 与官方 SwiftUI [ProgressView API](https://developer.apple.com/documentation/swiftui/progressview) 保持一致，并支持通过 [`progressViewStyle`](modifiers/#progressviewstylestyle) 修饰器进行样式设置。
...
<ContentSpotlight
  ...
  alt="不确定状态的旋转指示器和带有标签的确定性 ProgressView"
/>
...
> **注意：** 当使用 `linear` 样式（这是确定性进度的默认样式）时，`ProgressView` 是一个可变宽度组件，它会扩展以填充可用的水平空间。当在 `Host` 上使用 `matchContents` 时，你应该在 `ProgressView` 上应用 [`frame`](modifiers/#frameparams) 修饰符，为其指定明确的宽度。或者，也可以使用 `style` 为 `Host` 指定明确的尺寸（例如，`style={{ width: 300 }}` 或 `style={{ flex: 1 }}`）。`circular` 样式和不确定性加载指示器具有固定尺寸，并且可以与 `matchContents` 配合使用。
