...
一个可按压的按钮，可在 Android、iOS 和 web 上保持一致渲染。支持 `filled`、`outlined` 和 `text` 三种视觉变体。
  ...
  return (
    <Host matchContents>
      <Column spacing={8}>
        ...
        <Button variant="outlined" label="描边" onPress={() => {}} />
...
export default function DisabledButtonExample() {
  return (
    <Host matchContents>
      <Button label="禁用" onPress={() => {}} disabled />
    </Host>
  );
}
```
