---
title: 底部弹出页
description: 与 @gorhom/bottom-sheet 兼容的底部弹出页。
...
一个与 `@gorhom/bottom-sheet` API 兼容的 `BottomSheet` 组件。它封装了平台特定的 `@expo/ui` 原语：Android 上使用 [Jetpack Compose ModalBottomSheet](../jetpack-compose/bottomsheet)，iOS 上使用 [SwiftUI BottomSheet](../swift-ui/bottomsheet)。在 web 上，它使用 [vaul](https://github.com/emilkowalski/vaul) 抽屉。
...
- 此实现不需要来自 `react-native-gesture-handler` 的 `GestureHandlerRootView`。如果应用的其他部分需要它，你可以保留不变。
- 不支持诸如 `BottomSheetBackdrop`、`BottomSheetHandle`、`BottomSheetFooter`、`BottomSheetDraggableView`、`BottomSheetVirtualizedList`、`BottomSheetFlashList`、`useBottomSheetModal`、`useBottomSheetSpringConfigs` 和 `useBottomSheetTimingConfigs` 等组件和 hook 导出。为兼容 API，导出了一些相关的 prop 类型。
...
| 功能                   | Android                                      | iOS                               | Web                           |
| ---------------------- | -------------------------------------------- | --------------------------------- | ----------------------------- |
| 展示方式               | Jetpack Compose 模态底部弹窗                 | SwiftUI sheet                     | vaul 抽屉                    |
| 吸附点                 | 映射为部分展开和完全展开状态                 | 支持所提供的吸附点               | 支持所提供的吸附点           |
| 无 `snapPoints`        | 适配内容                                     | 适配内容                         | 适配内容                     |
...
| 导出项                     | 支持情况    | 备注                                                     |
| -------------------------- | ----------- | --------------------------------------------------------- |
| `BottomSheet`              | <YesIcon /> | Android 和 iOS 上为模态框，web 上为抽屉                   |
| `BottomSheetModal`         | <YesIcon /> | 初始为关闭状态，并通过 `present()` 打开                  |
| `BottomSheetModalProvider` | <YesIcon /> | 为了兼容性直接渲染子组件                                 |
| `BottomSheetView`          | <YesIcon /> | 包裹底部 sheet 内容                                      |
| `BottomSheetScrollView`    | <YesIcon /> | React Native `ScrollView` 的重新导出                    |
| `BottomSheetFlatList`      | <YesIcon /> | React Native `FlatList` 的重新导出                      |
| `BottomSheetSectionList`   | <YesIcon /> | React Native `SectionList` 的重新导出                   |
| `BottomSheetTextInput`     | <YesIcon /> | React Native `TextInput` 的重新导出                     |
| `useBottomSheet`           | <YesIcon /> | 从上下文返回 sheet ref 方法                              |
| `BottomSheetBackdrop`      | <NoIcon />  | 原生 sheet 或 web 抽屉会处理背景遮罩                    |
| `BottomSheetHandle`        | <NoIcon />  | 原生 sheet 或 web 抽屉会处理拖拽指示器                  |
