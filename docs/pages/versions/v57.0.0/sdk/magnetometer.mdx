...
`expo-sensors` 中的 `Magnetometer` 提供对设备磁力计传感器的访问，以响应并测量以微特斯拉（`μT`）为单位的磁场变化。

你可以使用 `Magnetometer` 访问已校准的数值，并使用 `MagnetometerUncalibrated` 访问未经校准的原始数值。
