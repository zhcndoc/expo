...
`expo-keep-awake` 提供了一个 React hook，用于防止屏幕进入休眠状态，并提供了一对函数以命令式地启用此行为。
...
export default class KeepAwakeExample extends React.Component {
  ...
  _activate = () => {
    /* @info 屏幕在调用后将保持常亮，直到调用 <strong>deactivateKeepAwake()</strong>。 */ activateKeepAwake(); /* @end */
    alert('Activated!');
  };

  _deactivate = () => {
    /* @info 停用 KeepAwake；如果它从未被激活，则不执行任何操作。 */ deactivateKeepAwake(); /* @end */
    alert('Deactivated!');
  };
}
```
