...
`expo-cellular` 提供有关用户蜂窝服务提供商的信息，例如其唯一标识符、蜂窝连接类型，以及它是否允许在其网络上进行 VoIP 呼叫。
...
如果你没有使用 Continuous Native Generation ([CNG](/workflow/continuous-native-generation/))，或者你正在手动使用原生 **android** 项目，那么你需要向项目的 **AndroidManifest.xml** 中添加 `android.permission.READ_PHONE_STATE` 权限。此权限用于 `TelephonyManager`。
