...
`Pedometer` 来自 `expo-sensors`，它在 Android 上使用系统的 `hardware.Sensor`，在 iOS 上使用 Core Motion 来获取用户的步数，并且还允许你订阅计步器更新。
...
export default function App() {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text>过去 24 小时内走的步数：{pastStepCount}</Text>
      <Text>走起来！看看这个数字上升：{currentStepCount}</Text>
    </View>
  );
}
