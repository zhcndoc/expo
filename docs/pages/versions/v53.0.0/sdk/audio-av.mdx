---
title: 音频 (expo-av)
description: 一个提供用于在应用中实现音频播放和录音的 API 的库。
...
> **警告** **已弃用：** 本页所文档化的来自 `expo-av` 的 `Audio` 组件现已弃用，并已被 `expo-audio` 中改进后的版本替代。 [了解 `expo-audio`](./audio/)

`expo-av` 中的 `Audio` 允许你在应用中实现音频播放和录制。

请注意，如果耳机/蓝牙音频设备断开连接，音频会自动停止。

有关媒体播放 API 的示例，请参见 [播放列表示例应用](https://github.com/expo/playlist-example)；有关录音 API 的示例，请参见 [录音示例应用](https://github.com/expo/audio-recording-example)。
...
export default function App() {
  ...
  useEffect(() => {
    return sound
      ? () => {
          console.log('正在卸载声音');
          /* @info 使用后务必卸载 Sound，以防止内存泄漏。*/ sound.unloadAsync(); /* @end */
        }
      : undefined;
  }, [sound]);
...
export default function App() {
  ...
  async function stopRecording() {
    ...
    /* @info iOS 在允许录音时可能会将音频播放重定向到手机听筒，因此完成后请禁用。 */ await Audio.setAudioModeAsync(
      {
        allowsRecordingIOS: false,
      }
    ); /* @end */
    /* @info */ const uri = recording.getURI(); /* @end */
    console.log('录音已停止并存储于', uri);
  }
...
在 iOS 上，每个后台功能都需要在 **Info.plist** 文件中的 `UIBackgroundModes` 数组里添加一个特殊键。
在独立应用中，此数组默认为空，因此要使用后台功能，你需要在 **app.json** 配置中添加相应的键。

以下是一个启用后台音频播放的 **app.json** 示例：
...
- Web 浏览器要求网站通过安全方式提供服务才能监听麦克风。更多详情请参见 [MediaDevices `getUserMedia()` 安全性](https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getUserMedia#security)。
...
`Sound.Audio` 上其余的 API 与 `Video` 组件的 `ref` API 相同。更多信息请参见 [AV 文档](av/#playback)。
