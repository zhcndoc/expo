---
title: 触觉反馈
description: 一个库，提供对 Android 上系统振动效果、iOS 上触觉引擎以及 Web 上 Vibration API 的访问。
...
- 已启用低电量模式。这可以通过 [`expo-battery`](./battery/) 检测到。
...
在 Android 上，该库需要权限来控制设备的震动。`VIBRATE` 权限会自动添加。
