...
向 [`presentationDetents`](modifiers/#presentationdetentsdetents-options) 传入 `selection` 和 `onSelectionChange` 选项，可通过编程方式控制弹窗切换到哪个分段。
...
使用 [`presentationBackgroundInteraction`](modifiers/#presentationbackgroundinteractioninteraction) 修饰器，允许与弹窗背后的内容交互。
...
使用 [`interactiveDismissDisabled`](modifiers/#interactivedismissdisabledisdisabled) 修饰器，防止用户通过上滑关闭弹窗。
