...
Expo UI List 与官方 SwiftUI [List API](https://developer.apple.com/documentation/swiftui/list) 保持一致，并支持通过 [`listStyle`](modifiers/#liststylestyle) 修饰符进行样式设置，以及各种行/分区修饰符，还支持选择、重新排序和编辑功能。
...
使用带有 `onDelete` 和 `onMove` 属性的 [`List.ForEach`](#listforeach) 复合组件，可以启用列表项的选择、删除和重新排序。
  ...
  return (
    <Host style={{ flex: 1 }}>
      <List modifiers={[refreshable(handleRefresh)]}>
        <Section title="数据">
          <Text>下拉以刷新</Text>
          {lastRefresh && <Text>最后刷新时间：{lastRefresh.toLocaleTimeString()}</Text>}
        </Section>
      </List>
    </Host>
  );
    ...
    <Host style={{ flex: 1 }}>
      <List>
        <Section title="样式化的行">
          ...
          <Text modifiers={[listRowInsets({ leading: 40 })]}>额外的左侧内边距</Text>
...
使用 [`scrollDismissesKeyboard`](modifiers/#scrolldismisseskeyboardmode) 修饰符来控制滚动时键盘的收起方式。
