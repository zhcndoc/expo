...
设置 `axis="vertical"` 以允许文本输入框垂直扩展。使用 [`lineLimit`](modifiers/#linelimit) 修饰符来控制可见行数。使用 `Host matchContents` 时，添加 `fixedSize({ horizontal: false, vertical: true })`，这样文本输入框会在使用其理想高度的同时接受父容器的宽度。
...
使用 [`keyboardType`](modifiers/#keyboardtypekeyboardtype) 修饰符显示特定的键盘布局。
