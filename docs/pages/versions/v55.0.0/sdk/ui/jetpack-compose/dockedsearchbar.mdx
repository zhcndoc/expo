...
Expo UI DockedSearchBar 与官方 Jetpack Compose [SearchBar API](https://developer.android.com/develop/ui/compose/components/search-bar) 保持一致，并显示一个搜索输入框，该输入框会保持锚定在其父布局中，而不是扩展为全屏。
