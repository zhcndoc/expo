  ...
  properties={[
    {
      ...
      description: [
        ...
        '* SQLite 数据库文件：`.db`',
        '',
        '> **注意**：要导入现有数据库文件（`.db`），请参阅 [SQLite API 参考](./sqlite/#import-an-existing-database) 中的说明。对于其他文件类型（例如 `.lottie` 或 `.riv`），请参阅 [如何在 metro 配置中向 `assetExts` 添加文件扩展名](/guides/customizing-metro/#adding-more-file-extensions-to-assetexts)。',
      ].join('\n'),
      default: '[]',
    },
  ]}
...
要详细了解如何使用 `expo-asset` 配置插件将资源文件嵌入项目，请参阅[在构建时加载资源](/develop/user-interface/assets/#load-an-asset-at-build-time-with-expo-asset-config-plugin)。
