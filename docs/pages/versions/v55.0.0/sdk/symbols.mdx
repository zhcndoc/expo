...
> **重要** 此库目前处于 [beta](/more/release-statuses/#beta) 阶段，可能会发生破坏性更改。

`expo-symbols` 提供了对跨平台原生符号库的访问。在 iOS 和 tvOS 上，它使用 [SF Symbols](https://developer.apple.com/sf-symbols/)。在 Android 和 web 上，它使用 [Material Symbols](https://fonts.google.com/icons)。
...
在 iOS 上，直接传入一个字重字符串。在 Android 上，从 `expo-symbols/androidWeights` 导入字重对象：
