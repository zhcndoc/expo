---
title: '@react-native-community/datetimepicker'
description: 一个提供访问系统 UI 以进行日期和时间选择的组件。
sourceCodeUrl: 'https://github.com/react-native-community/react-native-datetimepicker'
packageName: '@react-native-community/datetimepicker'
platforms: ['android', 'ios', 'expo-go']
inExpoGo: true
---

import { BookOpen02Icon } from '@expo/styleguide-icons/outline/BookOpen02Icon';

import { APIInstallSection } from '~/components/plugins/InstallSection';
import { BoxLink } from '~/ui/components/BoxLink';
import { ContentSpotlight } from '~/ui/components/ContentSpotlight';

> **重要** [`@expo/ui` 提供了一个可直接替换的实现](./ui/drop-in-replacements/datetimepicker)，可替代 `@react-native-community/datetimepicker`，在 Android 上由 Jetpack Compose 驱动，在 iOS 上由 SwiftUI 驱动。

一个用于访问系统 UI 进行日期和时间选择的组件。

<ContentSpotlight file="sdk/datetimepicker.mp4" loop={false} />

## 安装

<APIInstallSection href="https://github.com/react-native-datetimepicker/datetimepicker#getting-started" />

## 了解更多

<BoxLink
  title="访问官方文档"
  description="获取有关 API 及其用法的完整信息。"
  Icon={BookOpen02Icon}
  href="https://github.com/react-native-datetimepicker/datetimepicker"
/>
