...
`expo-build-properties` 是一个 [config plugin](/config-plugins/introduction/)，用于在 [Prebuild](/workflow/continuous-native-generation) 期间配置 **android/gradle.properties** 和 **ios/Podfile.properties.json** 目录中的原生构建属性。

> **info** 此 config plugin 用于配置 [Prebuild command](/workflow/continuous-native-generation/#usage) 如何生成原生的 **android** 和 **ios** 目录
> 因此不能用于不会运行 `npx expo prebuild` 的项目（bare projects）。
