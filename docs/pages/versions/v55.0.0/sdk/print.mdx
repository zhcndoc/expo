...
在 iOS 上，从 HTML 源打印不支持本地资源 URL（由于 `WKWebView` 的限制）。因此，图片需要转换为 base64 并内联到 HTML 中。
...
如果 `useMarkupFormatter` 被设置为 `true`，设置边距可能会导致打印结果末尾出现空白页。为避免这种情况，请确保你的 HTML 字符串是一个格式良好的文档，并在字符串开头包含 `<!DOCTYPE html>`。

**在 Android 上**，如果你在 `printAsync` 或 `printToFileAsync` 中使用 `html` 选项，生成的打印结果可能会包含页面边距（这取决于 WebView 引擎）。
