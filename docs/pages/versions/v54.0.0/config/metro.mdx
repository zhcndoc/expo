...
除了在调用 Expo CLI 进行编译和打包时以外，EAS CLI 使用不同的环境变量机制。详细了解 [EAS 中的环境变量](/eas/environment-variables/)。
...
- CSS Modules 在底层使用 [lightningcss](https://github.com/parcel-bundler/lightningcss)，请查看 [问题](https://github.com/parcel-bundler/lightningcss/issues) 以了解不支持的功能。
...
如果某个 bundle 导出了外部源映射，则文件末尾会添加一个 [**调试 ID**](https://sentry.engineering/blog/the-case-for-debug-ids) 注解，并在对应的源映射中添加匹配的 `debugId`，以便将这些文件关联起来。如果没有导出源映射，或者使用的是内联源映射，则不会添加此注解。
...
相关的 `*.js.map` 或 `*.hbc.map` 源映射将是一个 JSON 文件，其中包含等效的 `debugId` 属性。`debugId` 会在 Hermes 字节码生成之前注入，以确保在所有情况下都能匹配。
...
对于原生平台，会添加 `react-native` 条件；对于 web 导出，会添加 `browser` 条件；对于服务器导出（例如 API 路由或 React Server 函数），会添加 `node`、`react-server` 和 `workerd` 条件。这些条件的匹配顺序并不是按照其定义顺序，而是按照 `package.json:exports` 映射中的属性顺序来匹配。
...
Web Worker 可用于将工作卸载到 Web 上的单独线程，让主线程保持响应。这对于计算开销较大的任务很有用，例如图像处理、加密或其他原本会阻塞主线程的任务。
...
另外，你还可以通过公共路径导入 Worker：先将一个经过转换的 JS 文件放入 **公共** 目录，然后在 Worker 导入中使用变量引用它：
