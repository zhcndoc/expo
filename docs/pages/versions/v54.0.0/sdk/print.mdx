...
<SnackInline label='打印示例' dependencies={['expo-print', 'expo-sharing']}>
...
在 iOS 上，从 HTML 源打印不支持本地资源 URL（由于 `WKWebView` 的限制）。作为替代，图片需要转换为 base64 并内联到 HTML 中。
...
如果 `useMarkupFormatter` 设置为 `true`，设置边距可能会导致打印输出末尾出现空白页。为避免这种情况，请确保你的 HTML 字符串是一个格式正确的文档，并在字符串开头包含 `<!DOCTYPE html>`。
