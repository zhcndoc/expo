---
title: AppIntegrity
description: 一个用于在 Android 上访问 Google Play Integrity API，以及在 iOS 上访问 Apple App Attest 服务的库。
...
如果服务器能够成功验证该证明对象，就会认为该应用实例是有效的。在这种情况下，请务必在应用中持久化存储密钥标识符，而不是证明对象，以便将来对服务器请求进行签名。
