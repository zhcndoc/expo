...
这里有一个将 `expo-live-photo` 与 `expo-image-picker` 结合使用的简单示例。
  ...
  return (
    <View style={styles.container}>
      <LivePhotoView
        ...
        onLoadComplete={() => {
          console.log('实况照片加载成功！');
        }}
        onLoadError={error => {
          console.error('加载实况照片失败：', error.message);
        }}
      />
      <View style={livePhoto ? styles.pickImageCollapsed : styles.pickImageExpanded}>
        <Button title={livePhoto ? '更换图片' : '选择一张图片'} onPress={pickImage} />
      </View>
