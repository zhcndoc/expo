---
title: KeepAwake
description: A React component that prevents the screen from sleeping while rendered.
...
`expo-keep-awake` 提供了一个 React Hook，可防止屏幕进入休眠状态，并提供一对函数以命令式方式启用此行为。
...
export default function KeepAwakeExample() {
  /* @info 只要此组件保持挂载，屏幕就不会因空闲而关闭。 */
  useKeepAwake();
  /* @end */
  return (
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
      <Text>这个屏幕将永远不会休眠！</Text>
    </View>
  );
}
```
...
export default class KeepAwakeExample extends React.Component {
  ...
  _activate = () => {
    /* @info 调用后屏幕将保持常亮，直到调用 **deactivateKeepAwake()**。 */ activateKeepAwake(); /* @end */
    alert('Activated!');
  };
