...
[`expo-notifications`](/versions/latest/sdk/notifications) 库提供了推送通知所需的全部客户端功能。Expo 还负责将推送通知发送到 FCM 和 APNs，然后它们再把通知发送到特定设备。你只需要使用通过 [`getExpoPushTokenAsync`](/versions/latest/sdk/notifications/#getexpopushtokenasyncoptions) 获取的 `ExpoPushToken` 向 Expo Push API 发送请求即可。
...
```json
{
  "data": [
    {
      "status": "error",
      "message": "\"ExponentPushToken[xxxxxxxxxxxxxxxxxxxxxx]\" 不是已注册的推送通知接收者",
      "details": {
...
- `MismatchSenderId`: 这表示你的 FCM 推送凭据存在问题。FCM 推送凭据由两部分组成：FCM server key 和 **google-services.json** 文件。两者必须与同一个 sender ID 关联。你可以在[查找 server key 的位置](/push-notifications/push-notifications-setup/#get-credentials-for-development-builds)找到 sender ID。请检查项目 EAS 控制面板中 **Credentials** > **Application identifier** > **Service Credentials** > **FCM V1 service account key** 下的 server key，以及项目 **google-services.json** > `project_number` 中的 sender ID，确认它们与 Firebase 控制台 **Project Settings** > **Cloud Messaging** 标签页 > **Cloud Messaging API (Legacy)** 中显示的内容相同。
...
| 字段                | 平台            | 类型                                                      | 描述                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------- | --------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `to`                | Android and iOS | `string \| string[]`                                      | 指定此消息接收者的 Expo 推送令牌或 Expo 推送令牌数组。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `contentAvailable`  | iOS Only        | `boolean`                                                 | 设置为 true 时，通知将导致 iOS 应用在后台启动，以运行一个[后台任务](/versions/latest/sdk/notifications/#headless-background-notifications)。你的应用需要进行[配置](/versions/latest/sdk/notifications/#background-notification-configuration)才能支持此功能。对应于 [`aps.content-available`](https://developer.apple.com/documentation/usernotifications/pushing-background-updates-to-your-app#Create-a-background-notification)。                                                                                                                                                                     |
| `data`             | Android and iOS | `Object`                                                  | 传递给你应用的 JSON 对象。其大小最多约为 4KiB；发送给 Apple 和 Google 的通知有效负载总大小必须最多为 4KiB，否则会收到“Message Too Big”错误。                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `title`             | Android and iOS | `string`                                                  | 要在通知中显示的标题，通常显示在通知正文上方。对应于 [`AndroidNotification.title`](https://firebase.google.com/docs/reference/fcm/rest/v1/projects.messages#AndroidNotification) 和 [`aps.alert.title`](https://developer.apple.com/documentation/usernotifications/generating-a-remote-notification#Payload-key-reference)。                                                                                                                                                                                                                                                                                                   |
| `body`              | Android and iOS | `string`                                                  | 要在通知中显示的消息。对应于 [`AndroidNotification.body`](https://firebase.google.com/docs/reference/fcm/rest/v1/projects.messages#AndroidNotification) 和 [`aps.alert.body`](https://developer.apple.com/documentation/usernotifications/generating-a-remote-notification#Payload-key-reference)。                                                                                                                                                                                                                                                                                                                                                |
| `ttl`               | Android and iOS | `number`                                                  | 生存时间：如果消息尚未送达，可以保留并重新投递的秒数。省略此字段则使用各提供方自己的默认值——4 周。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `expiration`        | Android and iOS | `number`                                                  | 指定消息过期时间的 Unix 纪元时间戳。效果与 `ttl` 相同（`ttl` 优先于 `expiration`）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `priority`          | Android and iOS | `'default' \| 'normal' \| 'high'`                         | 消息的投递优先级。指定 `default` 或省略此字段，以使用每个平台的默认优先级（Android 上为“normal”，iOS 上为“high”）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `subtitle`          | iOS Only        | `string`                                                  | 要在通知中标题下方显示的副标题。对应于 [`aps.alert.subtitle`](https://developer.apple.com/documentation/usernotifications/generating-a-remote-notification#Payload-key-reference)。                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `sound`             | iOS Only        | `string \| null`                                          | 接收者收到此通知时播放声音。指定 `default` 以播放设备的默认通知声音，或省略此字段以不播放声音。自定义声音需要通过配置插件进行[配置](/versions/latest/sdk/notifications/#configurable-properties)，然后指定包含文件扩展名的声音文件名。例如：`bells_sound.wav`。                                                                                                                                                                                                                                                                                                 |
| `badge`             | iOS Only        | `number`                                                  | 要在应用图标徽章上显示的数字。指定零以清除徽章。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `interruptionLevel` | iOS Only        | `'active' \| 'critical' \| 'passive' \| 'time-sensitive'` | 通知的重要性和投递时间。字符串值对应于 [`UNNotificationInterruptionLevel`](https://developer.apple.com/documentation/usernotifications/unnotificationinterruptionlevel) 枚举中的各个枚举项。                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `channelId`         | Android Only    | `string`                                                  | 用于显示此通知的通知渠道 ID。如果指定了 ID，但设备上不存在相应的渠道（且该渠道尚未由你的应用创建），通知将不会显示给用户。                                                                                                                                                                                                                                                                                                                                                                                                              |
| `icon`              | Android Only    | `string`                                                  | 通知图标。Android 可绘制资源的名称（例如：`myicon`）。默认使用[配置插件](/versions/latest/sdk/notifications/#configurable-properties)中指定的图标。                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `richContent`       | Android and iOS | `Object`                                                  | 目前支持设置通知图片。提供一个包含 `image` 键的对象，其值类型为 `string`，表示图片 URL。Android 开箱即用即可显示图片。在 iOS 上，你需要向应用添加通知服务扩展目标。请参阅[此示例](https://github.com/expo/expo/pull/36202)了解具体操作方法。                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `categoryId`        | Android and iOS | `string`                                                  | 与此通知关联的通知类别 ID。[在此了解更多有关通知类别的信息](/versions/latest/sdk/notifications/#manage-notification-categories-interactive-notifications)。                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `collapseId`        | Android and iOS | `string`                                                  | 用于合并通知的标识符。在 Android 上，这只会合并传输中的消息（如果设备处于离线状态，则只会投递具有给定 `collapseId` 的最新消息），并对应于 FCM 的 [`collapse_key`](https://firebase.google.com/docs/cloud-messaging/customize-messages/collapsible-message-types)。要在 Android 上替换已经显示的通知，请使用 `tag`。在 iOS 上，这既会合并传输中的消息，也会替换设备上已经显示的通知，并对应于 [`apns-collapse-id`](https://developer.apple.com/documentation/usernotifications/generating-a-remote-notification#Send-a-POST-request-to-APNs)。 |
| `tag`               | Android Only    | `string`                                                  | 用于替换设备上已经显示的通知的标识符。如果设备当前显示的通知具有相同的 `tag`，新通知会将其替换。这与 `collapseId` 不同，后者会合并_传输中的_消息，而 `tag` 会替换_已经显示的_通知。对应于 FCM 的 [`notification.tag`](https://firebase.google.com/docs/reference/fcm/rest/v1/projects.messages#AndroidNotification) 字段。                                                                                                                                                                                                            |
| `threadId`          | iOS Only        | `string`                                                  | 系统用于在视觉上将通知归组的标识符。共享同一个 `threadId` 的通知会堆叠到一个组中。与 `collapseId` 不同，不会替换或移除任何通知。对应于 [`aps.thread-id`](https://developer.apple.com/documentation/usernotifications/generating-a-remote-notification#Payload-key-reference)。                                                                                                                                                                                                                                                                                                              |
| `mutableContent`    | iOS Only        | `boolean`                                                 | 指定此通知是否可以被[客户端应用拦截](https://developer.apple.com/documentation/usernotifications/modifying-content-in-newly-delivered-notifications?language=objc)。默认为 `false`。                                                                                                                                                                                                                                                                                                                                                                                                                                            |

**关于 `contentAvailable` 的说明**：此字段替代了已弃用的 `_contentAvailable` 字段，后者仍会出于向后兼容目的而被接受。如果同时指定二者，`contentAvailable` 优先。

**关于 `ttl` 的说明**：在 Android 上，我们会尽最大努力立即投递零 TTL 的消息，并且不会对其进行限流。但是，将 TTL 设置为较低的值（例如零）可能会导致普通优先级通知永远无法到达处于休眠模式的 Android 设备。要确保通知能够送达，TTL 必须足够长，以便设备从休眠模式唤醒。两者同时指定时，此字段优先于 `expiration`。
