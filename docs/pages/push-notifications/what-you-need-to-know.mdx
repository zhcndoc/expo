...
通知是向用户传达新信息或事件的提醒，即使应用当前并未活跃使用。通知的适用范围很广，不同平台之间的差异也会让实现通知变得令人望而生畏。
...
| 通知类型                                                                                                                                                                                                                 | 应用处于前台                                                                                                                                                                                         | 应用处于后台                                                                          | 应用已终止                                                                             |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| [通知消息](/push-notifications/what-you-need-to-know/#notification-message)和[包含数据负载的通知消息](/push-notifications/what-you-need-to-know/#notification-message-with-data-payload) | 送达时会运行 [`NotificationReceivedListener`](/versions/latest/sdk/notifications/#addnotificationreceivedlistenerlistener) 和 [JS 任务](/versions/latest/sdk/notifications/#registertaskasynctaskname) | 操作系统显示通知                                                                  | 操作系统显示通知                                                                  |
| [无头后台通知](/push-notifications/what-you-need-to-know/#headless-background-notifications)                                                                                                        | 送达时会运行 [`NotificationReceivedListener`](/versions/latest/sdk/notifications/#addnotificationreceivedlistenerlistener) 和 [JS 任务](/versions/latest/sdk/notifications/#registertaskasynctaskname) | 送达时会运行 [JS 任务](/versions/latest/sdk/notifications/#registertaskasynctaskname) | 送达时会运行 [JS 任务](/versions/latest/sdk/notifications/#registertaskasynctaskname) |
...
> **信息** 当应用未运行或已被杀死，并且通过点击通知启动时，请尽早在 iOS 上注册 `NotificationResponseReceivedListener`（在模块顶层）。为了在应用启动后处理初始通知响应，我们建议在启动期间同时检查 `useLastNotificationResponse` 或 `getLastNotificationResponse`，而不是仅依赖监听器。这同样也是让应用进入前台的操作按钮的推荐做法。
...
在 iOS 上，额外数据可以作为常规通知消息请求的一部分。Apple 并不区分带数据和不带数据的通知消息。
...
无头后台通知能够在应用已终止时，仍因通知触发而运行自定义 JavaScript。这很强大，但也有一个限制：即使通知已送达设备，操作系统也不能保证它一定会送达你的应用。这可能由多种原因导致，例如在 Android 上启用了 [Doze 模式](https://developer.android.com/training/monitoring-device-state/doze-standby)，或者你发送了过多的后台通知——Apple 建议不要 [每小时发送超过两到三条](https://developer.apple.com/documentation/usernotifications/pushing-background-updates-to-your-app#overview)。

当你使用 Expo Push Service，并且仅指定 `data` 和 `contentAvailable: true`（以及其他非交互字段，例如 `ttl`）时，生成的推送通知请求会产生无头后台通知。
...
你也可能会遇到“静默通知”这个术语，它是另一种不向用户展示任何内容的通知名称——我们将其描述为 [无头后台通知](#headless-background-notifications)。
