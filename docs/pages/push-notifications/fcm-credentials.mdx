...
> **警告**：如果 **google-services.json** 中的 API 密钥（`client.api_key.current_key` 字段）受到限制，请在 [Google Cloud 控制台](https://console.cloud.google.com/apis/credentials)中检查该密钥。在 **API 限制** 下，允许使用 **FCM Registration API** 和 **Firebase Installations API**，或者取消对该密钥的限制。在 **应用限制** 下，使用 Google Play 控制台中 **发布** > **设置** > **应用完整性** > **应用签名密钥证书** 中的 SHA-1，而不是你的[上传密钥](/app-signing/app-credentials/#app-signing-by-google-play)。如果两者不匹配，Firebase Installations API 会返回 `403 PERMISSION_DENIED: Requests from this Android client application are blocked`，并且你的应用将永远无法收到推送令牌。
...
<ContentSpotlight
  alt="在 Google Cloud 控制台的 IAM 管理页面中编辑 Principal"
...
<Step label="2">
点击 **Add Role**，然后从下拉菜单中选择 **Firebase Cloud Messaging API Admin** 角色。点击 **Save**。
