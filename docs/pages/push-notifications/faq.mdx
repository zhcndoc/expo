...
你可以将任何推送通知服务用于 Expo 项目。[`expo-notifications` 中的 `getDevicePushTokenAsync` 方法](/versions/latest/sdk/notifications/#getdevicepushtokenasync)允许你获取原生设备推送令牌，然后将其用于其他服务，或者甚至[直接通过 FCM 和 APNs 发送通知](/push-notifications/sending-notifications-custom)。
...
推送通知涉及很多环节，因此原因可能很多。为了缩小问题范围，请查看 [推送票据](/push-notifications/sending-notifications/#push-tickets) 和 [推送回执](/push-notifications/sending-notifications/#push-receipts) 中的错误信息。

你还可以通过在应用中测试[本地通知](/versions/latest/sdk/notifications/#schedulenotificationasyncrequest)来进一步缩小问题范围。这样可以确保客户端逻辑全部正确，并将问题定位到服务器端或应用凭证。
...
当你为应用商店构建应用时，需要生成并使用自己的凭证。在 Android 上，请参照[本指南](/push-notifications/fcm-credentials)。在 iOS 上，这由你的 [推送密钥](/app-signing/app-credentials/#push-notification-keys)处理（吊销与你的应用关联的推送密钥会导致通知投递失败。要修复它，请使用 `eas credentials` 添加新的推送密钥）。
...
如需可视化指南，请查看 [使用 EAS 配置 Expo 通知的视频](https://youtu.be/BCCjGtKtBjE?t=2123)。

### 发送通知时的错误信息

查看返回的推送票据或回执的 `details` 属性以获取更多信息。[在此处阅读](/push-notifications/sending-notifications/#errors)常见错误码响应及其对应的解决方案。

### 在 iOS 上获取推送令牌耗时很长

`getDevicePushTokenAsync` 和 `getExpoPushTokenAsync` 在 iOS 上有时可能需要很长时间才能完成。这不在 `expo-notifications` 的控制范围内，正如 Apple 的[《排查推送通知问题》](https://developer.apple.com/library/archive/technotes/tn2265/_index.html)技术说明中所述：
...
- 确保设备已可靠连接到互联网（尝试关闭 Wi-Fi 或切换到其他网络，并按[此 SO 回答](https://stackoverflow.com/a/34332047/1123156)所述，关闭防火墙对 5223 端口的阻止）。
- **[现有的 React Native 项目](/bare/overview/)** 必须[手动启用 **Push Notifications** 能力](/build-reference/ios-capabilities#manual-setup)。如果你在设置时遇到问题，请参考[此 Stack Overflow 回答](https://stackoverflow.com/a/10791240/1123156)。你也可以按照[此 Stack Overflow 回答](https://stackoverflow.com/a/8036052/1123156)的说明，通过记录持久连接调试信息进一步排查。
...
如果出现此问题的设备尚未使用 SIM 卡完成设置，根据[此 Stack Overflow 回答](https://stackoverflow.com/a/19432504/1123156)的说法，配置它可能有助于缓解此问题。
...
如果您不使用 [Expo 推送通知服务](/push-notifications/sending-notifications)，而是希望直接与 Google 和 Apple 通信，请参阅[使用 FCM 和 APNs 发送通知](/push-notifications/sending-notifications-custom)。

### Android 上的通知图标显示为灰色或白色方块

这表明您提供的图像资源存在问题。图像应为纯白色，并带有透明背景（这是 Google 的要求，由 Google 强制执行，而非 Expo）。如需更多信息，请参阅[这篇文章](https://clevertap.com/blog/fixing-notification-icon-for-android-lollipop-and-above/)。
