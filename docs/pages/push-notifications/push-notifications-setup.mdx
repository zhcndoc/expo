...
<Prerequisites>
  <Requirement title="支持推送功能的设备或模拟器/仿真器">
    你可以在实体 Android 或 iOS 设备上测试推送通知，也可以在带有
    Google Play 服务的 Android Emulator 上测试，或者在运行 Xcode 14 或更高版本（macOS 13+、iOS 16+）的 iOS Simulator 上测试。
  </Requirement>
</Prerequisites>

本指南中的以下步骤使用 [EAS Build](/build/introduction/)。这是设置通知最简单的方法，因为你的 EAS 项目还会包含[通知凭据](#get-credentials-for-development-builds)。不过，你也可以不使用 EAS Build，而通过[在本地构建项目](/guides/local-app-development/)来使用 `expo-notifications` 库。

<Step label="1">

## 安装库

运行以下命令来安装 `expo-notifications` 和 `expo-constants` 库：

<Terminal
  cmd={{
    npm: ['$ npx expo install expo-notifications expo-constants'],
    yarn: ['$ yarn expo install expo-notifications expo-constants'],
    pnpm: ['$ pnpm expo install expo-notifications expo-constants'],
    bun: ['$ bun expo install expo-notifications expo-constants'],
  }}
/>
...
async function registerForPushNotificationsAsync() {
  ...
  try {
    /* @info 这里会获取 Expo 推送令牌（如果之前未获取过），该令牌对该设备和项目 ID 是唯一的。 */
...
<Tabs tabs={["Android", "iOS"]}>
  ...
  <Tab>
    > **警告** 生成凭据需要付费的 Apple Developer 账户。

    对于 iOS，请确保在首次运行 `eas build` 命令之前，已经[注册要用于测试的 iOS 设备](/develop/development-builds/introduction/?buildenv=build-with-eas#how-would-you-like-to-build-your-development-build)。
