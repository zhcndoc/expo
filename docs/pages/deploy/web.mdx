...
<Prerequisites>
  <Requirement
    title={
      <>
        在 app.json 中设置 <CODE>expo.web.output</CODE>
      </>
    }>
    在你项目的 **app.json** 中，确保
    [`expo.web.output`](/versions/latest/config/app/#output) 属性的值为 `static` 或
    `server`。
  </Requirement>
</Prerequisites>
...
了解有关工作流常见模式的更多信息，请参阅 [工作流示例指南](/eas/workflows/examples/introduction)。

### 面向 AI 代理的 Expo Skills

如果你使用 AI 代理，请安装 [Expo Skills](/skills/)，教它如何部署你的 Web 应用：

<RelatedSkills names={['eas-hosting']} />

## 了解更多

你可以了解更多关于设置 [部署别名](/eas/hosting/deployments-and-aliases/)、使用 [自定义域名](/eas/hosting/custom-domain/) 或 [部署 API 路由](/router/web/api-routes/#deployment) 的信息。
