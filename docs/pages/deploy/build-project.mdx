...
生产构建会提交到应用商店，用于向广大用户发布，或者作为应用商店支持的测试流程的一部分，例如 TestFlight。本指南将说明如何使用 [EAS](#production-builds-using-eas) 和 [本地](#release-builds-locally) 创建生产构建。对于 Expo 应用，也可以使用任何能够编译 Android 和 iOS 应用的 CI 服务来创建生产构建。
