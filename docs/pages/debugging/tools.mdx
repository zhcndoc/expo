...
VS Code 是一款广受欢迎的代码编辑器，内置调试器。这个调试器使用与 React Native DevTools 相同的系统——inspector protocol。
...
启动 React Native 调试器后，你需要将端口（快捷键：macOS 上为 <kbd>Cmd ⌘</kbd> + <kbd>T</kbd>，Linux/Windows 上为 <kbd>Ctrl</kbd> + <kbd>T</kbd>）设置为 `8081`。之后，使用 `npx expo start` 运行你的项目，并在开发者菜单中选择 `Debug remote JS`。调试器应会自动连接。
