...
- **隔离抛出错误的代码**。这一步对于修复晦涩的错误至关重要。可以这样做：
...
通过运行以下命令在 Xcode 中打开项目，该命令是一个快捷方式，用于将项目 **ios** 目录中的 `.xcworkspace` 文件打开到 Xcode。
...
打开 Xcode 应用，然后按 <kbd>Shift</kbd> + <kbd>Cmd ⌘</kbd> + <kbd>2</kbd> 打开 **设备与模拟器** 窗口。

</Step>

<Step label="2">

如果你连接了实体设备，请在 **设备** 下选择它。否则，如果你正在使用模拟器，请在 **模拟器** 下选择它。

<ContentSpotlight
  alt="Xcode 中的设备与模拟器窗口。"
...
点击窗口中显示的 **打开控制台** 按钮以打开 console 应用。

<ContentSpotlight
  alt="Xcode 中设备与模拟器窗口里的打开控制台按钮。"
...
- **尝试在本地使用生产模式。** 在本地以 **生产模式** 运行应用会显示通常不会抛出的错误。为此，你可以运行 `npx expo start --no-dev --minify`。`--no-dev` 标志告诉服务器以生产模式运行，而 `--minify` 用于像生产 JavaScript bundle 一样对代码进行压缩。
