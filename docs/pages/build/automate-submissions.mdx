...
EAS Build 通过 `--auto-submit` 标志开箱即用地提供自动提交功能。此标志会告知 EAS Build，在构建完成后，使用适当的提交配置文件将构建传递给 EAS Submit。有关如何设置和配置提交的更多信息，请参阅 [EAS Submit 文档](/deploy/submit-to-app-stores/)。
...
- 如果你在 App Store Connect 中启用了“启用自动分发”，TestFlight 将自动创建一个群组，并邀请所有内部 TestFlight 用户测试该构建。
- 你还可以在 **eas.json** 提交配置文件中使用 [`groups`](/eas/json/#groups) 字段指定其他 TestFlight 群组。
- 使用 [TestFlight](/submit/testflight/)，你可以发布应用版本以供内部和外部测试。TestFlight 支持与最多 100 名内部测试者共享，并提供一个公开链接，可与最多 10,000 名外部测试者共享。
- 提交至 Apple App Store 审核是一个手动过程。将构建提交到 TestFlight 后，你必须手动将其推送到 App Store。
