...
<Prerequisites>
  <Requirement title="从本地机器成功构建一次">
    ...
    如果你还没有完成这些，请先查看 [创建你的第一个构建](/build/setup/) 指南，准备好后再返回这里。
...
[EAS 工作流](/eas/workflows/get-started) 是 Expo 提供的一项 CI/CD 服务，允许你在 EAS 上运行构建以及许多其他类型的任务。你可以使用 EAS 工作流来自动化开发和发布流程，例如创建开发构建或自动构建并提交到应用商店。
...
如果你在 CI 上使用 ad hoc provisioning 运行 [内部分发](/build/internal-distribution/) 构建，请刷新 ad hoc provisioning profile，以便包含上次构建之后添加的已注册设备。对于 `eas build`，请在 `--non-interactive` 一起传入 [`--refresh-ad-hoc-provisioning-profile`](/eas/cli/#eas-build)。对于 [EAS Workflows](/eas/workflows/get-started)，请在构建任务的 `params` 中将 `refresh_ad_hoc_provisioning_profile: true`（[构建任务参数](/eas/workflows/pre-packaged-jobs#build)）。有关要求和示例命令，请参阅 [CI 上的自动化](/build/internal-distribution/#automation-on-ci-optional)。
