---
title: 使用图片选择器
description: 在本教程中，了解如何使用 Expo Image Picker。
hasVideoLink: true
---
...
<VideoBoxLink
  videoId="iEQZU58naS8"
  title="观看：在你的通用 Expo 应用中使用图片选择器"
...
> **info** **提示：** 每当我们在项目中安装新库时，都先在终端中按 <kbd>Ctrl</kbd> + <kbd>C</kbd> 停止开发服务器，然后运行安装命令。安装完成后，再运行 `npx expo start` 重新启动开发服务器。
...
export default function Index() {
  /* @tutinfo 将图片选择器选项传递给 `launchImageLibraryAsync()` */
...
import { View, StyleSheet } from 'react-native';
import * as ImagePicker from 'expo-image-picker';
/* @tutinfo 从 `react` 中导入 `useState` 钩子。*/
...
export default function Index() {
  ...
  const pickImageAsync = async () => {
    ...
    if (!result.canceled) {
      /* @tutinfo 从 `assets` 数组中取出第一个 uri。另外，由于一次只能选择一张图片，所以你不需要更改这一点。 */
  ...
  return (
    <View style={styles.container}>
      <View style={styles.imageContainer}>
        /* @tutinfo 将所选图片 URI 传递给 `ImageViewer` 组件。 */
...
export default function ImageViewer({ imgSource, /* @tutinfo 传递 `selectedImage` 属性。*/selectedImage/* @end */ }: Props) {
  ...
  /* @tutinfo `imgSource` 已替换为 `imageSource`。 */
