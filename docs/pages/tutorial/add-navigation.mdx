...
- 一个 **路由** 文件会将 React 组件作为默认导出。它可以使用 `.js`、`.jsx`、`.ts` 或 `.tsx` 扩展名。
...
export default function RootLayout() {
  return (
    /* @tutinfo 将屏幕包裹在 `Stack` 导航器中。为 `/index` 路由添加 `options` 属性。再添加一个 `Stack.Screen` 组件以添加 `/about` 路由。 */
    <Stack>
...
import { Text, View, StyleSheet } from 'react-native';
/* @tutinfo 从 `expo-router` 中导入 `Link` 组件。 */ import { Link } from 'expo-router'; /* @end */

export default function Index() {
  return (
    <View style={styles.container}>
      <Text style={styles.text}>Home screen</Text>
      /* @tutinfo 在 `Text` 组件后添加 `Link` 组件，并传入指向 `/about` 路由的 `href` 属性。 */
...
const styles = StyleSheet.create({
  ...
  /* @tutinfo 为 `Link` 组件添加 `fontSize`、`textDecorationLine` 和 `color` 样式。 */
  button: {
    fontSize: 20,
...
export default function RootLayout() {
  return (
    <Stack>
      /* @tutinfo 这是将标签导航器嵌套在堆栈导航器中的方式，尤其是在根布局由父级堆栈导航器组成时。我们还将 `headerShown` 选项设为 `false`，以隐藏标签导航器的标题。否则，每个屏幕上都会显示两个标题。 */
...
export default function TabLayout() {
  return (
    <Tabs>
      /* @tutinfo `Tabs.Screen` 组件的工作方式类似，并且接受与 `Stack.Screen` 组件相同的属性。唯一的区别是设备上的导航模式。 */
...
2. 为 `index` 和 `about` 两个路由都添加 `tabBarIcon`。这个函数接收 `focused` 和 `color` 作为参数，并渲染图标组件。通过图标集，我们可以指定自定义的图标名称。
3. 为 `Tabs` 组件添加 `screenOptions.tabBarActiveTintColor`，并将其值设为 `#ffd33d`。这样会在标签处于激活状态时更改其图标和标签的颜色。
...
export default function TabLayout() {
  return (
    <Tabs
      /* @tutinfo 我们可以使用许多 `screenOptions` 来自定义标签栏。这里我们将激活标签的颜色更改为自定义值，稍后也会在应用中继续使用这个颜色。 */
      screenOptions={{
      ...
      <Tabs.Screen
        ...
        options={{
          ...
          tabBarIcon: ({ color, focused }) => (
            /* @tutinfo `focused` 参数允许我们在标签处于激活和未激活状态时改变其图标和标签的行为。*/
...
<ProgressTracker name="GET_STARTED" currentChapterSlug="/tutorial/add-navigation" />
