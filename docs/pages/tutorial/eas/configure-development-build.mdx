...
该命令会启动 Metro 打包器。在终端窗口中，我们会看到二维码，随后是 `Metro waiting on...` 和一个清单 URL：
...
由于我们在任何设备或模拟器/仿真器上还没有安装开发构建，所以目前还无法运行项目。
...
我们需要在本地机器上将 EAS 命令行界面（CLI）工具作为全局依赖安装。运行以下命令：
...
<Terminal
  cmd={[
    '# 运行 eas init 后的输出',
    '✔ 应由哪个账户拥有此项目？ > your-username',
    '✔ 是否要为 @your-username/sticker-smash 创建一个项目？ … 是',
    '✔ 已创建 @your-username/sticker-smash',
    '✔ 项目已成功链接（ID: XXXX-XX-XX-XXXX）（已修改 app.json）',
  ]}
/>
...
- 提示选择平台：**Android**、**iOS** 或 **全部**。由于我们要创建 Android 和 iOS 应用，选择 **全部**。
