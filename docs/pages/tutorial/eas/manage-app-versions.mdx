...
Google Play 商店和 Apple App Store 都依赖面向开发者的值来识别每个唯一构建。例如，如果我们上传一个应用版本为 `1.0.0 (1)` 的应用（这是面向用户的值和面向开发者的值的组合），我们就无法再向应用商店提交另一个具有相同应用版本的构建。提交重复应用版本号的构建会导致提交失败。

下面通过 **app.config.js** 中的 `android.versionCode` 和 `ios.buildNumber` 展示了手动管理面向开发者的值的示例。**我们不需要手动添加或管理这些值，因为 EAS Build 已经为我们自动完成了这件事**。
...
默认情况下，EAS Build 会帮助自动化管理面向开发者的版本值。它使用 [远程版本源](/build-reference/app-versions/#remote-version-source) 在每次创建新的生产发布时自动递增面向开发者的值。
...
```json eas.json
{
  "cli": {
    /* @hide ... */ /* @end */
    /* @info `appVersionSource` 设置为 `remote`。 */
    "appVersionSource": "remote"
    /* @end */
  },
  "build": {
    "production": {
      /* @info `autoIncrement` 设置为 `true`，以便自动递增 `versionCode` 或 `buildNumber`。 */
      "autoIncrement": true
      /* @end */
    }
  }
...
- 当提示 **是否要现在将应用版本来源设置为 remote？** 时，选择 **yes**。这将把 **eas.json** 中的 `cli.appVersionSource` 设置为 `remote`。
- 当提示 **你想用哪个版本号来初始化它？** 时，输入你在应用商店中设置的最后一个版本号。
