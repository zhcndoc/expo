...
- **是否要登录到你的 Apple 账户？** 按 <kbd>Y</kbd>。这将登录到我们的 Apple Developer 账户。
...
- **生成新的 Apple Provisioning Profile？** 按 <kbd>Y</kbd>。这将作为生产应用的 provisioning profile。
...
要提交我们通过最新的 EAS 构建创建的应用二进制文件，请运行 [`eas submit`](/submit/ios/) 命令：
...
- **按照提示登录我们的 Apple 账户。** 当提示 **是否复用此 App Store Connect API Key？** 时，按 <kbd>Y</kbd>。
...
> **Note**：与内部测试类似，我们也可以使用 TestFlight 创建一个用于邀请外部测试者的测试组。内部测试限制为 100 名用户，而 TestFlight 允许向外部分享测试版本，最多支持 10,000 名测试者，并提供可公开分享的链接。有关分步说明，请参阅 [使用 TestFlight 分发 iOS 应用](/submit/testflight/)。
...
> **完成 App Store 列表**：要为应用商店列表做准备，请参阅 [创建应用商店资源](/guides/store-assets/)，了解如何创建截图和预览。
...
> **注意：** 此命令会自动将你的构建上传到 TestFlight 进行内部测试，但不会自动将你的应用提交到 App Store 审核。等你准备公开发布时，仍然需要手动将构建从 TestFlight 提交到 App Store。更多信息请参阅 [应用商店的默认提交行为](/build/automate-submissions/#default-submission-behavior-for-app-stores)。
