...
- **Google 服务账号密钥：** 我们需要一个 Google 服务账号邮箱和 JSON 密钥来自动化应用提交流程。**请先按照我们的指南中的详细说明，在 [创建 Google 服务账号密钥或从现有账号下载它](https://expo.fyi/creating-google-service-account)，然后再返回本指南。** 这是可选项，但在[自动化发布流程](#automated-release)时是必需的。
