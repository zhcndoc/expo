...
<Prerequisites>
  ...
  <Requirement title="Google 服务账号密钥（可选）">
    自动化 [release 流程](#automated-release) 需要一个 Google 服务账号邮箱和 JSON 密钥。请按照 [创建 Google 服务账号密钥或从现有账号下载它](https://expo.fyi/creating-google-service-account) 中的详细说明操作，然后返回本指南。
  </Requirement>
</Prerequisites>
