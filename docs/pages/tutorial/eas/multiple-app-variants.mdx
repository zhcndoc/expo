...
export default ({ config }) => ({
  /* @info `config` 对象会复制 app.json 文件中所有现有的配置。*/
  ...config,
  /* @end */
});
```
...
export default ({ config }) => ({
  ...
  android: {
    /* @info 包含来自 app.json 的现有 `android` 配置 */
    ...config.android,
    /* @end */
...
```json eas.json|collapseHeight=440
{
  "build": {
    "development": {
      ...
      /* @info 添加 `env.APP_VARIANT` 以访问构建配置文件的环境变量*/
      "env": {
