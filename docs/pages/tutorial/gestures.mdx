...
// ... 其余导入语句保持不变
/* @tutinfo 从 `react-native-gesture-handler` 导入 `GestureHandlerRootView`。 */import { GestureHandlerRootView } from 'react-native-gesture-handler';/* @end */
...
/* @tutinfo 从 `react-native-reanimated` 导入 `useAnimatedStyle`、`useSharedValue` 和 `withSpring`。 */import Animated, { useAnimatedStyle, useSharedValue, withSpring } from 'react-native-reanimated';/* @end */

export default function EmojiSticker({ imageSize, stickerSource }: Props) {
  /* @tutinfo 创建一个 `scaleImage` 共享值，并将 `imageSize` 设为其初始值。 */const scaleImage = useSharedValue(imageSize);/* @end */
    ...
    <View style={{ top: -350 }}>
      /* @tutinfo 使用 `GestureDetector` 包裹 `Animated.Image` 组件。*/ <GestureDetector gesture={doubleTap}>/* @end */
        <Animated.Image
          source={stickerSource}
          resizeMode="contain"
          style={/* @tutinfo 修改 `Animated.Image` 上的 `style` 属性以传入 `imageStyle`。 */[imageStyle, { width: imageSize, height: imageSize }]/* @end */}
        />
      /* @tutinfo */
      </GestureDetector>
      /* @end */
...
export default function EmojiSticker({ imageSize, stickerSource }: Props) {
  ...
  return (
    /* @tutinfo 将所有组件包裹在 `GestureDetector` 内。 */<GestureDetector gesture={drag}>/* @end */
      <Animated.View style={/* @tutinfo 将 `containerStyle` 添加到 `Animated.View` 的 style 属性。 */[containerStyle, { top: -350 }]/* @end */}>
        <GestureDetector gesture={doubleTap}>
          <Animated.Image
            source={stickerSource}
            resizeMode="contain"
...
<ProgressTracker
  ...
  name="GET_STARTED"
  summary="We have successfully implemented pan and tap gestures."
  nextChapterDescription="In the next chapter, we will learn how to take screenshots of images and stickers, and save them to the device's photo album."
