...
安装完成后，请务必重启开发服务器，并在终端中按 <kbd>W</kbd>。
...
/* @tutinfo 导入 `domtoimage` 库。 */import domtoimage from 'dom-to-image';/* @end */
...
<ProgressTracker
  ...
  name="GET_STARTED"
  summary="The app has now completed all the features we initially set out to build, so it’s time to shift focus to aesthetics alone.."
  nextChapterDescription="In the next chapter, we’ll customize the app’s status bar, splash screen, and app icon."
  nextChapterTitle="Configure the status bar, splash screen, and app icon"
