...
选择构建镜像时，你可以使用下方提供的完整名称，或使用以下别名之一：`auto`、`latest`，或者某个特定 SDK 的别名，例如 `sdk-56`。

- 使用特定名称可确保环境一致，仅会有少量更新。
- 使用 `auto` 别名时，构建镜像会根据项目配置、Expo SDK 版本和 React Native 版本来选择。你可以在构建日志的 **启动构建环境** 部分查看此次构建使用了哪个镜像。
- `latest` 别名会指向包含最新软件版本的镜像。
...
- SDK 别名会随着每个新的 SDK 发布而更新。
- `latest` 别名会随着每个新的镜像版本发布而更新。
