...
每个平台的镜像都包含一个特定版本的 Node.js、Yarn、CocoaPods、Xcode、Ruby、Fastlane 等。你可以在[eas.json](/build/eas-json)中覆盖其中部分版本。如果你要找的配置项不存在，你可以使用[npm hooks](/build-reference/npm-hooks)通过 `apt-get` 或 `brew` 安装或更新任何系统依赖。请注意，这些自定义配置会在构建过程中应用，并且会增加构建耗时。

在选择构建镜像时，你可以使用下面提供的完整名称，或使用以下别名之一：`auto`、`latest`，或者针对特定 SDK 的别名，例如 `sdk-57`。

- 使用特定名称可保证环境一致，仅会有少量更新。
- 使用 `auto` 别名时，构建镜像将根据项目配置、Expo SDK 版本以及 React Native 版本来选择。你可以在构建日志的 **启动构建环境** 部分查看本次构建使用了哪个镜像。
- `latest` 别名将指向软件版本最新的镜像。
- `sdk-57` 别名将指向最适合 SDK 57 构建的镜像。
- `sdk-56` 别名将指向最适合 SDK 56 构建的镜像。
- `sdk-55` 别名将指向最适合 SDK 55 构建的镜像。
- `sdk-54` 别名将指向最适合 SDK 54 构建的镜像。
- `sdk-53` 别名将指向最适合 SDK 53 构建的镜像。
- `sdk-52` 别名将指向最适合 SDK 52 构建的镜像。
- SDK 别名会随着每个新 SDK 版本的发布而更新。
- `latest` 别名会随着每个新镜像版本的发布而更新。

> **信息** **注意：** 如果你没有在 **eas.json** 中提供 `image`，默认会使用 `auto` 别名。
...
- [npm 缓存已通过 Kubernetes 部署](/build-reference/caching/#javascript-dependencies)
- [Maven 缓存已通过 Kubernetes 部署](/build-reference/caching/#android-dependencies)
- 构建环境预配时，会通过 `GRADLE_OPTS` 环境变量注入 Gradle JVM 参数。请参见下方的 [Gradle JVM 参数](#gradle-jvm-args)。
- **~/.npmrc** 中的全局 npm 配置：
...
EAS Build 会在 Gradle 运行前，将 `GRADLE_OPTS` 环境变量设置到构建 VM（worker）上。其值取决于你选择的[资源类别](/eas/json/#resourceclass)：
...
除了 `-Xmx` 之外，worker 还会通过 `-Dorg.gradle.jvmargs` 向 Gradle 构建 JVM 传递以下 JVM 参数：

- `-XX:MaxMetaspaceSize=1g`
- `-XX:+HeapDumpOnOutOfMemoryError`
- `-Dfile.encoding=UTF-8`

worker 还会通过 `GRADLE_OPTS` 设置这些顶层 Gradle 属性：

- `-Dorg.gradle.parallel=true`
- `-Dorg.gradle.daemon=false`

> **警告** worker 会通过 `GRADLE_OPTS` 设置 `org.gradle.jvmargs`，这会覆盖你项目 **gradle.properties** 中定义的任何 `org.gradle.jvmargs`。

#### 覆盖 `GRADLE_OPTS`

你可以在 **eas.json** 的构建配置文件 [`env`](/eas/json/#env) 中、在[工作流文件](/eas/workflows/syntax/#jobsjob_idenv)中，或通过 [EAS 环境变量](/eas/environment-variables/)设置 `GRADLE_OPTS`，以替换 worker 的默认值。项目环境变量优先于 worker 的默认值。
...
我们的目标是尽快支持所有稳定版 Xcode 发布版本，只要它们可用于构建流程，你就可以将应用提交到 App Store Connect。

这通常意味着我们会支持最新的稳定版 Xcode，以及前一个版本（直到 Apple 引入新的[最低 Xcode 版本要求](https://developer.apple.com/news/upcoming-requirements/?id=04292024a)）。
