...
<ContentSpotlight
  alt="运行 eas build 命令的终端，其中提供了 Android 和 iOS 平台选项"
...
如果你有一个[现有的 React Native 项目](/bare/overview/)，它的内容会略有不同。
...
<ContentSpotlight
  alt="eas build:configure 中显示初始化步骤完成的提示"
...
这就是将项目配置为与 EAS Build 兼容的全部内容了。还有一个步骤：如果你在 **eas.json** 中将 `cli.requireCommit` 设置为 `true`，系统会提示你提交我们为你所做的所有更改。你可以选择在提交前查看这些更改，也可以指定 git 提交信息，或者使用默认信息。
