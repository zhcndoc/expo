...
You can configure the [`web.output`](/versions/latest/config/app/#output) target in [App Config](/workflow/configuration/) to set the export mode for your Web app:
...
| ------------------- | ----------- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `single` (default)  | <YesIcon /> | <NoIcon />  | Outputs a single-page application (SPA) with only one **index.html** in the output directory, and no HTML that can be statically indexed.                                 |
| `server`            | <YesIcon /> | <YesIcon /> | Creates **client** and **server** directories. Client files are output as separate HTML files. API routes are output as separate JavaScript files for hosting with a custom Node.js server. |
| `static`            | <YesIcon /> | <NoIcon />  | Outputs separate HTML files for each route in the **app** directory.                                                                                                       |

> **info** **Note**: For `static` and `server` output modes, you can configure [global HTTP headers](/router/web/server-headers) that apply to all route responses via the `expo-router` plugin.
...
<BoxLink
  title="使用 EAS 立即部署"
  description="EAS 托管是部署 Web 应用的最佳方式，支持自定义域名、SSL 等更多功能。"
  href="/eas/hosting/get-started"
  Icon={Cloud01Icon}
/>
