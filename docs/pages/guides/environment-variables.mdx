...
`NODE_ENV` 是标准的 Node.js 变量，用于标识你的代码正在以哪种模式运行（通常是 `development`、`production` 或 `test`）。像模块解析器、打包器和测试运行器这样的工具会读取它，以决定如何构建、安装或运行你的代码。

我们不建议使用 `NODE_ENV` 在不同的 **.env** 文件之间切换（例如 **.env.test** 和 **.env.production**）。虽然从技术上讲这是可行的（`NODE_ENV=test npx expo start` 会加载 **.env.test**）——但它的行为可能并不符合你的预期。例如，`npx expo export` 会始终强制将 `NODE_ENV` 设为 `production`，因此 `NODE_ENV=test npx expo export` 实际上不会以 `test` 作为 `NODE_ENV` 运行该命令。

其他基于 Expo CLI 命令构建的工具也会表现出相同的行为——例如，`eas update` 会调用 `npx expo export`，因此，`NODE_ENV=test eas update` 也不会以 `test` 作为 `NODE_ENV` 运行（它会是 `production`）。`NODE_ENV` 环境变量在不同工具中有不同用途（例如，如果你运行 `NODE_ENV=production npm install`，那么你的 `devDependencies` 将不会被安装），我们发现，对于 React Native 项目，最好不要为了这个用例进一步过度使用它。
...
之前使用 `direnv` 时，你需要使用一个读取 [`process.env`](https://nodejs.org/dist/latest/docs/api/process.html#process_process_env) 的[动态应用配置](/workflow/configuration/#dynamic-configuration)，将环境变量设置到 `extra` 字段中，以便在 JavaScript 代码中通过 [`expo-constants`](/versions/latest/sdk/constants) 使用。请将这些引用直接移到代码中，并添加 `EXPO_PUBLIC_` 前缀：
