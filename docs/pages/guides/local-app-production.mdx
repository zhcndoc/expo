...
<Prerequisites>
  <Requirement title="已安装 OpenJDK">
    安装一个 [OpenJDK
    发行版](/get-started/set-up-your-environment/?mode=development-build&buildEnv=local#install-watchman-and-jdk)
    以便使用 `keytool` 命令。
  </Requirement>
...
<Prerequisites>
  <Requirement title="Apple Developer 会员资格">
    付费的 Apple Developer 会员资格是签名和提交 iOS 应用所必需的。
  </Requirement>
  <Requirement title="已安装 Xcode">
    在你的电脑上 [安装
