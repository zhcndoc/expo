...
[Sentry](https://getsentry.com/) 是一个崩溃报告平台，可为你提供**对生产部署的实时洞察，并附带用于重现和修复崩溃的信息**。
...
请确保在你的构建环境中设置了具有[sensitive visibility](/eas/environment-variables/#visibility-settings-for-environment-variables)的 `SENTRY_AUTH_TOKEN`。Sentry 会自动为你上传 source maps。如果你使用的是环境变量而不是 app config 中的属性，也请确保这些变量同样已设置。
...
<Terminal
  cmd={{
    npm: [
      '# 将 `eas update` 生成的 "dist" 目录传入上传脚本',
...
Sentry 与 Expo 的集成让你可以直接在 EAS 仪表板中查看 Expo 应用部署的崩溃报告和会话回放。此集成提供了到 Sentry 堆栈跟踪的直接链接，并包含完整上下文、会话回放和调试能力。

### 安装

> **info** 安装此集成需要 Sentry 的 owner、manager 或 admin 权限。

1. 登录你的 Expo 账号，并打开 [**账户设置 > 概览**](https://expo.dev/accounts/[your-account]/settings)。
2. 在 **连接** 下，点击 Sentry 旁边的 **连接**。
3. 登录你的 Sentry 账号，并接受将该集成加入你的组织。之后你会被重定向回 **账户设置**。

### 关联你的项目

连接账号后，你需要将你的 EAS 项目关联到你的 Sentry 项目：

1. 在 EAS 中打开 **项目 > [你的项目] > 配置 > 项目设置**。
2. 点击 **关联**，并从下拉菜单中选择你的 Sentry 项目。

### 使用

要在 EAS 仪表板中查看你的 Sentry 数据，请打开 **项目 > [你的项目] > 更新 > 部署 > [部署]**，以查看来自某个 Release 的 Sentry 数据。

通过此集成，你可以：

- 直接在 EAS 仪表板中查看崩溃报告
- 访问会话回放，准确查看错误发生前发生了什么
- 获取包含完整上下文的详细堆栈跟踪
- 在 EAS 和 Sentry 之间无缝切换以进行调试

## 了解更多关于 Sentry 的信息

Sentry 不仅仅能捕获致命错误，还可以通过他们的 [JavaScript 使用文档](https://docs.sentry.io/platforms/javascript/) 了解更多关于如何使用 Sentry 的信息。
