...
> **info** **SDK 55 and later run entirely on the new architecture.** The new architecture is always enabled and cannot be disabled. If you need to use the old architecture, use SDK 54 or earlier.

The new architecture is the name we use to describe a comprehensive internal overhaul of React Native. It also addresses limitations discovered over years of production use of the original React Native architecture at Meta and elsewhere.

In this guide, we'll cover how to use the new architecture in your Expo project.

<BoxLink
  title="The new architecture is here"
  description="A blog post from the React Native team at Meta outlining the features of the new architecture and the motivation for building it."
  href="https://reactnative.dev/blog/2024/10/23/the-new-architecture-is-here"
  Icon={BookOpen02Icon}
/>
<BoxLink
  title="React Native 0.82 - A new era"
  description="React Native 0.82 is the first version to run entirely on the new architecture. SDK 55 uses React Native 0.83 and inherits this behavior."
...
**The new architecture is the present and future of React Native**. Starting with React Native 0.82, the new architecture is always enabled and cannot be disabled. SDK 55 uses React Native 0.83 and inherits this behavior. [The old architecture was frozen in June 2025](https://github.com/reactwg/react-native-new-architecture/discussions/290), which means it will no longer receive new features or bug fixes.

**New React and React Native features will only ship for the new architecture**. For example, the new architecture includes [full support for Suspense](https://reactnative.dev/blog/2024/10/23/the-new-architecture-is-here#full-support-for-suspense) and [new styling capabilities](https://reactnative.dev/blog/2025/01/21/version-0.77#new-css-features-for-better-layouts-sizing-and-blending), neither of which are implemented in the old architecture. Many popular libraries now also support only the new architecture.

**If you're on SDK 54 or earlier**, you can still use the old architecture by setting `newArchEnabled` to `false`. However, you'll need to migrate to the new architecture before upgrading to SDK 55 or later.
      ...
      <Tabs tabs={['Android', 'iOS']}>
        <Tab>
          <Terminal
            cmd={{
              npm: [
                '# 如果你愿意，可以先运行一次干净的 prebuild 并启动本地构建',
                '$ npx expo prebuild --clean && npx expo run:android',
                '# 如果你更喜欢，也可以使用 EAS 运行构建',
                '$ eas build -p android',
              ],
              yarn: [
                '# 如果你愿意，可以先运行一次干净的 prebuild 并启动本地构建',
...
<Collapsible summary="你是否正在为现有的 React Native 项目禁用新架构（SDK 54 及更早版本）？">
...
Meta 和 Expo 正在努力让新架构成为所有新应用的默认架构，并尽可能让现有应用的迁移变得简单。然而，新架构不只是一个名称——React Native 的许多内部实现都已从底层重新设计和重建。因此，在你的应用中启用新架构时，可能会遇到一些问题。以下是一些排查这些问题的建议。
