...
> **信息** **SDK 55 及更高版本完全运行在新架构上。** 新架构始终处于启用状态，无法禁用。如果需要使用旧架构，请使用 SDK 54 或更早版本。

新架构是我们用于描述 React Native 全面内部改造的名称。它还解决了 Meta 及其他地方多年来在生产环境中使用原始 React Native 架构时发现的局限性。

在本指南中，我们将介绍如何在 Expo 项目中使用新架构。

<BoxLink
  title="新架构已到来"
  description="React Native Meta 团队发布的一篇博文，介绍了新架构的功能以及构建新架构的动机。"
  href="https://reactnative.dev/blog/2024/10/23/the-new-architecture-is-here"
  Icon={BookOpen02Icon}
/>
<BoxLink
  title="React Native 0.82——新时代"
...
**新架构是 React Native 的现在和未来**。从 React Native 0.82 开始，新架构始终处于启用状态，无法禁用。SDK 55 使用 React Native 0.83，并继承了这一行为。[旧架构于 2025 年 6 月被冻结](https://github.com/reactwg/react-native-new-architecture/discussions/290)，这意味着它将不再获得新功能或错误修复。

**新的 React 和 React Native 功能将仅针对新架构发布**。例如，新架构包含对 [Suspense 的完整支持](https://reactnative.dev/blog/2024/10/23/the-new-architecture-is-here#full-support-for-suspense)以及[新的样式功能](https://reactnative.dev/blog/2025/01/21/version-0.77#new-css-features-for-better-layouts-sizing-and-blending)，而旧架构均未实现这些功能。许多热门库现在也仅支持新架构。

**如果你使用的是 SDK 54 或更早版本**，仍可以通过将 `newArchEnabled` 设置为 `false` 来使用旧架构。但是，在升级到 SDK 55 或更高版本之前，你需要迁移到新架构。

</Collapsible>

## Expo 工具与新架构

从 SDK 53 开始，[Expo SDK](/versions/latest/) 中的所有 `expo-*` 软件包都支持新架构（包括[无桥接模式](https://github.com/reactwg/react-native-new-architecture/discussions/154)）。[了解 Expo 库中的已知问题](#known-issues-in-expo-libraries)。
...
<Collapsible summary="你是否正在现有的 React Native 项目中启用新架构？">
