...
你可以在本地开发服务器中使用 Expo Atlas。这种方法允许你在修改项目中的任何代码时，Atlas 自动更新。

当你的应用在 Android、iOS 和/或 web 上通过本地开发服务器运行后，你可以使用 <kbd>Shift</kbd> + <kbd>M</kbd> 通过 [开发工具插件菜单](/debugging/devtools-plugins/#using-a-dev-tools-plugin) 打开 Atlas。

<Terminal
  cmd={{
    npm: ['# 使用 Atlas 启动本地开发服务器', '$ EXPO_ATLAS=true npx expo start'],
    yarn: ['# 使用 Atlas 启动本地开发服务器', '$ EXPO_ATLAS=true yarn expo start'],
    pnpm: ['# 使用 Atlas 启动本地开发服务器', '$ EXPO_ATLAS=true pnpm expo start'],
    bun: ['# 使用 Atlas 启动本地开发服务器', '$ EXPO_ATLAS=true bun expo start'],
  }}
/>
...
你也可以在为应用或 EAS Update 生成生产包时使用 Expo Atlas。Atlas 会在导出期间生成 **.expo/atlas.jsonl** 文件，你可以在无法访问项目的情况下共享和打开该文件。此文件包含每个打包模块的原始源代码和转换后的源代码，包括内联的 [`EXPO_PUBLIC_` 环境变量](/guides/environment-variables/)。请将其视为你的源代码，只与信任的人共享。
...
Lighthouse 是了解你的网站速度、可访问性和性能表现的绝佳方式。你可以在 Chrome 的 **审核** 选项卡中测试你的项目，或者使用 [Lighthouse CLI](https://github.com/GoogleChrome/lighthouse#using-the-node-cli)。
