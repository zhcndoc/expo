...
[Clerk](https://clerk.com/expo-authentication) 是一个身份验证和用户管理平台，提供注册、登录、多因素认证、社交登录、组织以及托管用户数据库。[`@clerk/expo`](https://www.npmjs.com/package/@clerk/expo) SDK 为你提供 React hooks、控制组件，以及预构建的原生 UI 组件，这些组件会在 Android 上使用 Jetpack Compose 渲染，在 iOS 上使用 SwiftUI 渲染。
