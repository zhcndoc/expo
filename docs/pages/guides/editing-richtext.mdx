...
Many apps need to let users enter text. For example, if you want to build a messaging or social media app, you’ll likely rely heavily on text input. React Native includes the `<TextInput>` component out of the box, which makes this easy in many simple scenarios.

However, sometimes you need a bit more flexibility. Think of long social media posts, note-taking apps, or document editors. Ideally, you’d want support for different text styles, lists, headings, inline images, and more. This is called a rich text editor, and it’s a hard problem to solve well everywhere, including in React Native.

At the moment, there isn’t a default solution in the React Native ecosystem. However, this guide will explore some options and promising approaches, each with its own trade-offs.

<VideoBoxLink
  videoId="CxORa1tXMjw"
  title="Watch: How to build a rich text editor with DOM components"
  description="Use Expo DOM components to render a web-based editor in a native app, enabling a rich text editor in React Native."
/>
...
- 对于 markdown 内容，你可以使用 markdown 渲染器，例如 [`react-native-enriched-markdown`](https://github.com/software-mansion/react-native-enriched-markdown) 或其他类似工具。
