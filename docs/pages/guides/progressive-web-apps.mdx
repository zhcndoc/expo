...
> **警告** 添加 service worker 时请务必小心，因为它们已知会在 Web 上引发意外行为。如果你不小心发布了一个激进缓存你网站的 service worker，用户将无法轻松请求更新。若想获得最佳的移动端离线体验，请使用 Expo 创建原生应用。与带有 service worker 的网站不同，原生应用可以通过应用商店更新来清除缓存体验。这类似于重置用户的原生浏览器（如果 service worker 足够激进，他们可能不得不这样做）。更多信息请参见 [为什么 service worker 不是最佳选择](https://github.com/facebook/create-react-app/issues/2398)。
