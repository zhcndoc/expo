...
maxHeadingDepth: 4
description: 了解可自定义的不同 Metro 打包器配置。
---
...
有关更多信息，请参阅 [**metro.config.js** 文档](/versions/latest/config/metro)。

> **info** Expo 会锁定某些 Metro 配置选项，以防止项目损坏。并非上游 Metro 接受的每个选项都可以自定义或受支持。Expo 也不支持从 YAML 文件（上游已弃用）加载 Metro 配置，或者加载位于仓库之外的任何 Metro 配置。
...
从 **SDK 56** 开始，Expo 的文件映射支持按需文件系统访问。实际上，这意味着你的 `watchFolders` 不再需要包含应用打包所需的每个模块，并且会符号链接到项目根目录之外依赖项的项目现在也能正确解析。

按需文件系统由 [应用配置](/workflow/configuration/) 中的 [`experiments.onDemandFilesystem`](/versions/v56.0.0/config/app/#ondemandfilesystem) 标志控制，并且默认启用。

<BoxLink
  title="按需文件系统"
  description="了解 Expo 的文件映射如何发现、监视并延迟解析源文件。"
...
<BoxLink
  title="压缩 JavaScript"
  description="了解如何使用 Expo CLI 中的 Metro 打包器自定义 JavaScript 压缩过程。"
  href="/guides/minify/"
  Icon={BookOpen02Icon}
/>
...
此功能在[现有的 React Native 项目](/bare/overview/)中需要额外设置。有关更多信息，请参阅 [Metro 设置指南](/versions/latest/config/metro#existing-react-native-apps)。
