...
<Prerequisites>
  <Requirement title="Expo SDK 55 及更早版本：react-native-webview">
    <Tabs>
      <Tab label="SDK 56 及更高版本">
        DOM 组件默认使用 [`@expo/dom-webview`](https://www.npmjs.com/package/@expo/dom-webview)，无需额外安装。

        <Collapsible summary="如何退出使用 @expo/dom-webview 并改用 react-native-webview？">
        如果你想改用 `react-native-webview`，请先安装它，然后通过 `dom` 属性选择退出：
  ...
  <Requirement title="Expo CLI 和 Expo Metro 配置">
    如果你已经使用 `npx expo [command]` 运行项目（例如，你是通过 `npx create-expo-app` 创建的项目），那么就已经全部配置好了。

    如果你的项目中还没有 `expo` 包，请通过运行下面的命令进行安装，并 [选择使用 Expo CLI 和 Metro 配置](/bare/installing-expo-modules/#configure-expo-cli-for-bundling-on-android-and-ios)：
  ...
  <Requirement title="Expo Metro Runtime、React DOM 和 React Native Web">
...
如果你使用 AI 代理，请安装 [Expo Skills](/skills/)，教会它何时使用 DOM 组件，以及如何将 Web 代码迁移到原生环境：

<RelatedSkills names={['expo-dom', 'expo-web-to-native']} />

## 功能

- Web、原生和 DOM 组件之间共享打包器配置。
- DOM 组件启用了 React、TypeScript、CSS 以及 Metro 的所有其他功能。
- 支持在终端中记录日志，以及使用 Safari/Chrome 进行调试。
- 支持快速刷新和 HMR。
- 支持嵌入式导出，以实现离线支持。
- Web 和原生平台之间统一资源。
- DOM 组件的包可以在 [Expo Atlas](/guides/analyzing-bundles/#analyzing-bundle-size-with-expo-atlas) 中进行检查，以便调试。
- 无需重新构建原生应用即可访问所有 Web 功能。
- 开发环境中的运行时错误叠加层。
- 支持 Expo Go。
...
你可以在 DOM 组件中使用 `useDOMImperativeHandle` hook，以接受来自原生端的 ref 调用。这个 hook 类似于 React 的 [`useImperativeHandle`](https://react.dev/reference/react/useImperativeHandle) hook，但不需要向其传递 ref 对象。
...
> **warning** EAS Update 不支持公共资源。请改用 `require()` 加载本地资源。
...
export default function App() {
  return <WebView source={{ html: '<h1>你好，世界！</h1>' }} />;
}
```
...
<Collapsible summary="如何在 DOM 组件中获取安全上下文？">
