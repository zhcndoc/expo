...
- 修改 **tsconfig.json** 后，重启 Expo CLI 以更新路径别名。别名变更时无需清除 Metro 缓存。
- 如果不使用 TypeScript，**jsconfig.json** 可以作为 **tsconfig.json** 的替代方案。
- 定义路径别名后，会增加额外的解析时间。
- 路径别名仅由 Metro（包括 Metro web）支持，不受已弃用的 `@expo/webpack-config` 支持。
- [现有的 React Native 项目](/bare/overview/) 需要为此功能进行额外设置。更多信息请参阅 [Metro 设置指南](/versions/latest/config/metro#existing-react-native-apps)。
...
- 如果定义了 `compilerOptions.paths`，它们会相对于 `compilerOptions.baseUrl` 进行解析；否则，它们会相对于项目根目录进行解析。
- `compilerOptions.baseUrl` 会在 node modules 之前解析。这意味着如果你有一个名为 `./path.ts` 的文件，它可能会被导入，而不是名为 `path` 的 node 模块。
...
- 绝对导入仅由 Metro（包括 Metro web）支持，不受 `@expo/webpack-config` 支持。
- 现有的 React Native 项目需要为此功能进行额外设置。更多信息请参阅 [版本化 Metro 设置指南](/versions/latest/config/metro#existing-react-native-apps)。
