...
- 使用 `WebBrowser.maybeCompleteAuthSession()` 来关闭 Web 弹出窗口。如果忘记添加这一行，弹出窗口将不会关闭。
...
- **环境：**
  - 现有的 React Native 应用
    - `npx expo prebuild`
  - App Store 或 Play Store 中的独立版构建，或本地测试
    - Android：`eas build` 或 `npx expo run:android`
    - iOS：`eas build` 或 `npx expo run:ios`

- **创建：** 在正确的环境中运行时，使用 `AuthSession.makeRedirectUri({ native: '<YOUR_URI>' })` 来选择原生 URI。
  ...
  - 此链接通常会自动创建，但我们建议你至少定义 `scheme` 属性。通过传递 `native` 属性，可以在应用中覆盖整个 URL。Google 或 Okta 等要求使用自定义原生 URI 重定向的提供商通常会使用此方式。你可以使用 `npx uri-scheme` 添加、列出和打开 URI scheme。
  - 如果更改了 `expo.scheme`，请运行 `npx expo prebuild --clean`，使用新的 scheme 重新生成原生项目，然后使用 `npx expo run:android` 和 `npx expo run:ios` 重新构建。
- **使用：** `promptAsync({ redirectUri })`

## 改善用户体验

“登录流程”是整个过程中的重要环节，在许多情况下，用户会在此再次_下定决心_使用你的应用。糟糕的体验可能会导致用户甚至还没真正开始使用应用，就选择放弃。

以下是一些可以让用户身份验证更快速、更简单且更安全的建议：

### 预热浏览器

在 Android 上，你可以选择在使用浏览器之前对其进行预热。这样可以让浏览器应用在后台预先完成初始化。这样做可以显著加快向用户显示身份验证提示的速度。
