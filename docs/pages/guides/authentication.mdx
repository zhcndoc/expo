...
- **创建：** 在正确的环境中运行时，使用 `AuthSession.makeRedirectUri({ native: '<YOUR_URI>' })` 来选择 native。
  ...
  - 这个链接通常可以自动创建，但我们建议你至少定义 `scheme` 属性。整个 URL 都可以通过传入 `native` 属性在应用中覆盖。通常这会用于 Google 或 Okta 等提供商，它们要求你使用自定义的原生 URI 重定向。你可以使用 `npx uri-scheme` 来添加、列出和打开 URI scheme。
  - 如果你更改了 `expo.scheme`，请运行 `npx expo prebuild --clean` 以使用新 scheme 重新生成原生项目，然后再使用 `npx expo run:android` 和 `npx expo run:ios` 重新构建。
...
function App() {
  useEffect(() => {
    /* @info <strong>仅限 Android：</strong>在后台启动默认浏览器应用的预热，以改善切换时间。 */
