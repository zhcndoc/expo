...
local-first 软件的另一个特性是它具有协作性——多个设备可以处理同一份数据，且更改会同步到所有设备上。当你在 [Figma](https://www.figma.com/) 中协作设计时，这可以实时发生；或者当你离线时在 Linear 中创建任务，并在你重新联网后同步时，这可以异步发生。

### 开发者体验优势

你不再需要为每个网络请求管理应用的各种状态——“已加载”“加载中”“错误”等，以及它们对应的 UI 状态和其他逻辑。只需写入本地数据库，应用就会自动将更改同步到服务器。这意味着你可以专注于构建应用，而不必过多担心网络和离线状态。
...
[Jazz](https://jazz.tools/) 是一个具有实时同步、离线支持和行级权限的 local-first 关系型数据库。它是开源的，为 Expo 和 React Native 提供一流支持，你可以自行托管它以快速启动。要了解更多，请查看 [示例](https://github.com/garden-co/jazz/tree/main/examples) 或阅读 [入门指南](https://jazz.tools/docs/install/client) 以获取详细说明。
