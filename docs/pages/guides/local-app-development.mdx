...
要使用你的机器在本地将项目构建为应用，你必须在测试、调试构建或创建用于提交到应用商店的生产构建之前手动生成原生代码。你可以通过两种方式在本地构建应用。本指南简要介绍这两种方法，并提供创建此工作流所需的其他指南链接。
...
然后在终端中按 <kbd>A</kbd> 启动 Android，或按 <kbd>I</kbd> 启动 iOS，以打开已安装的应用。Metro 会提供你更新后的 JavaScript 包，而无需重新编译原生代码，因此应用的加载时间只需几秒，而不是几分钟。
