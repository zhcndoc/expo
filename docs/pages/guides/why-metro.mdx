...
世界上几乎每个 React Native 应用都使用 Metro，这使它成为经过实战验证、并针对大规模项目优化的解决方案。因此，它适合各种规模的开发者，从个人爱好者到大型公司都适用。Metro 专为处理 Meta 的大规模应用而设计，这也是它具备 delta bundling 和[共享远程缓存](https://metrobundler.dev/docs/caching)等功能的原因。
...
- 文件监视默认使用 JS 爬虫，在安装 Watchman（C++）时也可选择使用它。
- AST 使用 Hermes parser（WebAssembly）解析为与 Babel 兼容的格式。
- AST 转换使用 Babel。这最大限度地提升了开发者的自定义能力。
- 压缩在原生平台上使用 Hermes，在 Web 上使用 Terser（可选支持 ESBuild）。
- CSS 解析和压缩使用 [LightningCSS](https://lightningcss.dev/)（Rust）。
