...
```js babel-preset-expo 之后（为服务器打包）
if (true) {
  console.log('Hello on the server!');
}
```
...
如果星号导出引入了具有歧义的导出，例如 `module.exports.ArrowUp` 或 `exports.ArrowDown`，那么优化过程将不会展开星号导出，并且不会从聚合文件中移除任何导出。你可以使用 [Expo Atlas](/guides/analyzing-bundles/#analyzing-bundle-size-with-expo-atlas) 检查展开后的导出。
