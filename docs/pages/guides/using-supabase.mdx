---
title: 使用 Supabase
description: 使用 Supabase 为您的 React Native 应用添加 Postgres 数据库和用户身份验证。
---
...
[Supabase](https://supabase.com/?utm_source=expo&utm_medium=referral&utm_term=expo-react-native) 是一个后端即服务（BaaS）应用开发平台，提供托管的后端服务，例如 Postgres 数据库、用户身份验证、文件存储、边缘函数、实时同步，以及向量和 AI 工具包。它是 Google Firebase 的开源替代方案。
...
<Prerequisites>
  <Requirement title="一个 Supabase 项目">
    前往 [database.new](https://database.new?utm_source=expo&utm_medium=referral&utm_term=expo-react-native) 创建一个新的 Supabase 项目。
  </Requirement>
  <Requirement title="项目 URL 和可发布密钥">
    从 API 设置中获取**项目 URL**，并从 API 密钥中获取**可发布密钥**：

    1. 前往控制台中的 [API 设置](https://supabase.com/dashboard/project/_/settings/api) 页面。
    2. 在此页面找到你的项目 `URL` 和 `service_role` 密钥。
    3. 然后前往 [API 密钥](https://supabase.com/dashboard/project/_/settings/api-keys)。
    4. 在 API 密钥选项卡下找到你的项目**可发布密钥**。
...
创建一个辅助文件来初始化 Supabase 客户端（`@supabase/supabase-js`）。你需要此前复制的 API URL 和 [`Publishable` 密钥](#prerequisites)。由于 Supabase 已在数据库中启用[行级安全性](https://supabase.com/docs/guides/auth/row-level-security?utm_source=expo&utm_medium=referral&utm_term=expo-react-native)，因此这些变量可以安全地公开在你的 Expo 应用中。
