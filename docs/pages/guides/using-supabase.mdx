...
<Prerequisites>
  ...
  <Requirement title="Project URL 和 Publishable key">
...
创建一个辅助文件来初始化 Supabase 客户端（`@supabase/supabase-js`）。你需要之前复制的 API URL 和 `Publishable` key [前文](#get-the-api-keys)。由于 Supabase 已在数据库中启用 [行级安全性](https://supabase.com/docs/guides/auth/row-level-security?utm_source=expo&utm_medium=referral&utm_term=expo-react-native)，这些变量在你的 Expo 应用中公开是安全的。
