...
要创建启动画面图标，你可以使用这个 [Figma 模板](https://www.figma.com/community/file/1637141012732584189)。它为 Android 和 iOS 的图标及启动图片提供了一个最基本的设计。
...
要创建应用图标，你可以使用这个 [Figma 模板](https://www.figma.com/community/file/1637141012732584189)。它为 Android 和 iOS 的图标及启动图片提供了一个最基本的设计。
...
Android 自适应图标由两个独立图层组成——前景图像以及背景颜色或背景图像。这使操作系统可以将图标裁切为不同形状，同时也支持视觉效果。对于 Android 13 及更高版本，操作系统支持主题化应用图标，它会使用壁纸和主题来确定由设备主题设置的颜色。

你提供的设计应遵循用于启动器图标的 [Android 自适应图标指南](https://developer.android.com/develop/ui/compose/system/icon_design_adaptive)。你还应该：
...
- 使用 **.png** 文件。
- 1024x1024 是一个不错的尺寸。如果你有使用 `npx create-expo-app` 创建的 Expo 项目，[EAS Build](/build/setup/) 会为你生成其他尺寸。若是[现有的 React Native 项目](/bare/overview/)，请自行生成图标。EAS Build 生成的最大尺寸是 1024x1024。
- 图标必须是严格的正方形。例如，1023x1024 的图标无效。
- 确保图标填满整个正方形，没有圆角或其他透明像素。操作系统会在适当时候对你的图标进行遮罩处理。
- 可以使用 `ios.icon` 为不同的系统外观（例如深色和有色调）提供不同图标。如果指定此项，将覆盖应用配置文件中的顶层 icon 键。请参见下面的示例：
