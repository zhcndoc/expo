...
> **信息** **assets/fonts** 目录路径是在 React Native 应用中放置字体文件的一种常见约定。如果你遵循自定义约定，也可以把这些文件放在其他位置。

在项目中使用本地字体文件有两种方式：

- 使用 [`expo-font` 配置插件](/versions/latest/sdk/font/#configuration-in-app-config)嵌入字体文件（仅限 Android 和 iOS）。
- 在运行时使用 [`useFonts`](/versions/latest/sdk/font/#usefonts) hook 加载字体文件（支持 Android、iOS 和 web）。

### 使用 `expo-font` 配置插件

`expo-font` 配置插件允许将一个或多个字体文件嵌入到项目的原生代码中。它支持 Android 和 iOS 的 `ttf` 和 `otf`，而 `woff` 和 `woff2` 仅在 iOS 上受支持。

> **注意：** 配置插件仅在原生平台（Android 和 iOS）上运行。对于 web，请改用 [`useFonts` hook](#with-usefonts-hook)。
...
- 由于此方法要求[创建开发构建](/develop/development-builds/introduction/#how-would-you-like-to-build-your-development-build)，因此无法与 Expo Go 一起使用。
...
对于 Android，你可以指定 `fontFamily`、`weight`，以及可选的 `style`（默认为 `"normal"`），这样字体会被嵌入为原生 [XML 资源](https://developer.android.com/develop/ui/views/text-and-emoji/fonts-in-xml)。如果你在数组中只提供字体文件路径，那么文件名会成为 Android 上的字体族名称。iOS 总是从字体文件本身提取字体族名称。
...
使用配置插件嵌入字体后，创建一个[新的开发构建](/develop/development-builds/introduction/#how-would-you-like-to-build-your-development-build)，并将其安装到你的设备、Android Emulator 或 iOS Simulator 上。
...
_Font Book 应用在 macOS 上的截图。_

<ContentSpotlight
  alt="macOS 上的 Font Book 应用显示了 Inter 字体的标识符，其中 PostScript 名称“Inter-Black”被突出显示。"
...
/* @info 导入 `SplashScreen`，这样在字体尚未加载时，我们就可以继续显示 `SplashScreen`。 */ import * as SplashScreen from 'expo-splash-screen'; /* @end */
import {useEffect} from 'react';

/* @info 这可以防止 `SplashScreen` 在字体加载期间自动隐藏。 */
SplashScreen.preventAutoHideAsync();
/* @end */

export default function RootLayout() {
  /* @info 使用 `useFonts` hook 映射字体文件。 */
...
在项目中使用 Google 字体有两种方式：

- 使用 [`expo-font` 配置插件](/versions/latest/sdk/font/#configuration-in-app-config)嵌入已安装的字体。
- 在运行时通过 [`useFonts`](/versions/latest/sdk/font/#usefonts) hook 异步加载已安装的字体。

### 使用 `expo-font` 配置插件

> **注意：** 使用 `expo-font` 配置插件嵌入 Google 字体，与自行嵌入自定义字体具有相同的优点和限制。更多信息请参见[使用 `expo-font` 配置插件加载本地字体文件](#with-expo-font-config-plugin)。
...
使用配置插件嵌入字体后，创建一个[新的开发构建版本](/develop/development-builds/introduction/#how-would-you-like-to-build-your-development-build)，并将其安装到你的设备、Android 模拟器或 iOS 模拟器上。
...
> **注意：** 使用 `useFonts` hook 加载 Google 字体，与自行嵌入自定义字体具有相同的优点和限制。更多信息请参见[使用 `useFonts` hook 加载本地字体文件](#with-usefonts-hook)。
...
```tsx src/app/_layout.tsx
// 其余导入语句
/* @info 从 `@expo-google-fonts/inter` 导入 `Inter_900Black` 和 `useFonts` hook */
import { Inter_900Black, useFonts } from '@expo-google-fonts/inter';
/* @end */
/* @info 导入 `SplashScreen`，这样在字体尚未加载时，我们就可以继续显示 `SplashScreen`。 */
...
当 `@expo/vector-icons` 库中的图标首次加载时，它们在你的应用中会显示为不可见图标。加载完成后，它们会被缓存，以供应用后续使用。为了避免在应用首次加载时显示不可见图标，请在初始加载屏幕期间使用 [`useFonts`](/versions/latest/sdk/font/#usefonts) 进行预加载。例如：
