...
只有第一阶段——前置条件——是其他阶段所必需的。按照其说明完成后，你可以继续查看与自己在采用 Expo 过程中目标最相关的工具和服务。
...
大多数 React Native 开发者在构建应用时都会解决常见问题，例如实现导航、访问原生 API、升级到新版本等等。这需要使用一组特定的工具和库来构建和维护你的应用——这意味着你正在创建自己的框架。
