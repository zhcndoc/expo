...
- 通过按下 <kbd>J</kbd> 键即可立即访问 Hermes 调试器。
- 调试器自带 [React Native DevTools](/debugging/tools/#debugging-with-react-native-devtools)。
- 支持 [Continuous Native Generation (CNG)](/workflow/continuous-native-generation/)，可通过 [`expo prebuild`](/more/glossary-of-terms/#prebuild) 用于升级、白标化、轻松设置第三方包，以及通过缩小代码范围提升代码库的可维护性。
- 支持使用 [`expo-router`](/router/introduction/) 进行基于文件的路由。
  - 开发环境中的 [异步打包](/router/web/async-routes)。
- 内置 [环境变量支持](/guides/environment-variables) 和 **.env** 文件集成。
- 可在终端中直接查看原生日志，并与 JavaScript 日志并列显示。
- 使用 Expo CLI 专为 React Native 应用构建的 `xcpretty` 风格工具，可改进原生构建日志格式。例如，在编译 Pod 时，你可以看到它是由哪个 Node 模块引入的。
- [一流的 TypeScript 支持](/guides/typescript)。
- 支持带有 `paths` 和 `baseUrl` 的 **tsconfig.json** 别名，并且[已内置于 Metro](/guides/typescript/#path-aliases-optional)。
- 通过 Metro 提供 [Web 支持](/guides/customizing-metro/#adding-web-support-to-metro)。对 React Native Web 提供完整类型支持。
- 现代 [CSS 支持](/versions/latest/config/metro#css)，支持 Tailwind、PostCSS、CSS Modules、SASS 等。
- 通过 Expo Router 和 Metro web 实现静态站点生成。
- 开箱即用的 [monorepo 支持](/guides/monorepos)。
- 支持 Expo 工具链，例如 [`expo-dev-client`](/develop/development-builds/introduction)、[Expo Updates 协议](/technical-specs/expo-updates-1) 和 [EAS Update](/eas-update/introduction)。
- 使用 `npx expo run:ios` 时会自动执行 `pod install`。
- `npx expo install` 会为常见包选择兼容的依赖版本。
- 运行 `npx expo run:[android|ios]` 和 `npx expo start` 时会自动检测端口。如果默认端口上已有其他应用运行，则会使用其他端口。
- 在交互式提示中，可使用 <kbd>Shift</kbd> + <kbd>A</kbd> 或 <kbd>Shift</kbd> + <kbd>I</kbd> 快捷键选择启动 Android 或 iOS 设备。
- 内置通过 [ngrok 隧道](/develop/development-builds/development-workflows/#tunnel-urls) 提供应用服务的支持。
...
<Terminal
  cmd={{
    npm: ['# Android', '$ npx expo run:android', '', '# iOS', '$ npx expo run:ios'],
    yarn: ['# Android', '$ yarn expo run:android', '', '# iOS', '$ yarn expo run:ios'],
    pnpm: ['# Android', '$ pnpm expo run:android', '', '# iOS', '$ pnpm expo run:ios'],
    bun: ['# Android', '$ bun expo run:android', '', '# iOS', '$ bun expo run:ios'],
  }}
/>
