...
以下说明适用于在 React Native 0.86 中安装最新版本的 Expo 模块。对于早期版本，请查看 [native upgrade helper](/bare/upgrade)，了解这些文件是如何定制的。
...
另外，你也可以在 **AppDelegate.swift** 中添加额外的 delegate 方法。某些库可能需要这些方法，因此除非你有充分理由不添加它们，否则建议添加。[在 AppDelegate.swift 中查看 delegate 方法](https://github.com/expo/expo/blob/sdk-57/templates/expo-template-bare-minimum/ios/HelloWorld/AppDelegate.swift#L34-L51)。

保存所有更改，并在 Xcode 中将你的 iOS Deployment Target 更新为 <code>iOS 16.4</code>：

- 在 Xcode 中打开 **your-project-name.xcworkspace**，然后在左侧边栏中选择你的项目。
- 选择 **Targets** > **your-project-name** > **Build Settings** > **iOS Deployment Target**，并确保其设置为 <code>iOS 16.4</code>。
...
- [`expo-asset`](/versions/latest/sdk/asset) - 一个仅 JavaScript 的包，它构建于 `expo-file-system` 之上，并为所有 Expo 模块中的资源提供通用基础。
- [`expo-constants`](/versions/latest/sdk/constants) - 提供对清单的访问。
- [`expo-file-system`](/versions/latest/sdk/filesystem) - 与设备文件系统交互。由 `expo-asset` 和许多其他 Expo 模块使用。开发者通常也会在应用代码中直接使用它。
- [`expo-font`](/versions/latest/sdk/font) - 在运行时加载字体。此模块是可选的，可以安全移除。不过，如果你在开发中使用 `expo-dev-client`，则建议保留，并且 `@expo/vector-icons` 也依赖它。
- [`expo-keep-awake`](/versions/latest/sdk/keep-awake) - 在开发应用时防止设备进入睡眠状态。此模块是可选的，可以安全移除。
