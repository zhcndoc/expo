...
在某些托管服务上，例如 [AWS](https://aws.amazon.com/) 和 [Firebase](https://firebase.google.com/)，你需要显式将 JavaScript 文件的 `"Content-Type"` 头设置为 `"application/javascript"`，这样 [updates](./configuring-updates) 才能正常工作。否则 [Updates.checkForUpdateAsync()](/versions/latest/sdk/updates/#updatescheckforupdateasync) 将会报错 _"Failed to fetch new update"_。
