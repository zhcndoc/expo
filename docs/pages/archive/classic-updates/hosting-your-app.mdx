...
<Terminal
  cmd={[
    ...
    '$ git add * && git commit -m "使用此 JS bundle 更新我的应用"',
...
在某些托管服务上，例如 [AWS](https://aws.amazon.com/) 和 [Firebase](https://firebase.google.com/)，你需要显式将 JavaScript 文件的 `"Content-Type"` 头设置为 `"application/javascript"`，这样 [更新](./configuring-updates) 才能正常工作。否则 [Updates.checkForUpdateAsync()](/versions/latest/sdk/updates/#updatescheckforupdateasync) 将会报错 _"Failed to fetch new update"_。

下面是一个 **firebase.json** 配置示例，其中部署目标（[部署目标](https://firebase.google.com/docs/cli/targets)）名为 "native"。
...
<Terminal
  cmd={[
    ...
    '',
    '# 将应用部署到 firebase',
    '$ firebase deploy --only hosting:native -m "部署我的应用"',
  ]}
/>
...
1. 运行 `expo export --dump-sourcemap --public-url <your-url>`。这也会将你的 bundle source map 导出到 **bundles** 目录中。
