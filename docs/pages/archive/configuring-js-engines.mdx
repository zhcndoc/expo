...
我们推荐使用 Hermes，因为它专为 React Native 应用构建并针对其进行了优化，同时还提供了最佳的调试体验。如果你熟悉不同 JavaScript 引擎之间的权衡，并希望从 Hermes 切换到其他引擎，则可以在[应用配置](/workflow/configuration/)中的 [`jsEngine`](/versions/v54.0.0/config/app/#jsengine) 字段指定应用所使用的 JavaScript 引擎。默认值为 `hermes`。
